...
Citizen.CreateThread(function()
    exports["qb-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=false,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("qb-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == ghostChoppVehiclee then
        ...
        ghostStartChoppingBruh()
        exports['qb-interface']:showInteraction('[E] Start Chopping')
    end
end)

function ghostStartChoppingBruh()
	Citizen.CreateThread(function()
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["qb-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["qb-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["qb-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
    ...
    TriggerEvent('qb-chop:cooldown')
    return success
end

RegisterNetEvent('qb-chop:cooldown')
AddEventHandler('qb-chop:cooldown', function()
    CoolDownActive = true
    Citizen.Wait(math.random(60000, 120000))
    CoolDownActive = false
    if ClockedIntoChop then
        TriggerEvent('qb-phone:chop_shop_offer')
        CreateChopCar()
    end
end)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("qb-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('qb-chopshop:clock-in')
AddEventHandler('qb-chopshop:clock-in', function()
    if CoolDownActive then
        TriggerEvent('DoLongHudText', 'You have a active cooldown!', 2)
    elseif ClockedIntoChop then
        TriggerEvent('qb-chopshop:clock-out')
    else
        ClockedIntoChop = true
        TriggerEvent('qb-phone:signInInfoChopshop')
        Citizen.Wait(math.random(60000, 120000))
        TriggerEvent('qb-phone:chop_shop_offer')
    end
end)

RegisterNetEvent('qb-chopshop:clock-out')
AddEventHandler('qb-chopshop:clock-out', function()
    if ClockedIntoChop then
...
function CreatePickupCarBlip(x, y ,z)
  ...
  RegisterNetEvent('qb-chopshop:acceptOffer')
  AddEventHandler('qb-chopshop:acceptOffer', function()
    CreateChopCar()
  end)

function CreateChopCar()
	if DoesEntityExist(ghostChoppVehiclee) then
	    SetVehicleHasBeenOwnedByPlayer(ghostChoppVehiclee,false)
		SetEntityAsNoLongerNeeded(ghostChoppVehiclee)
		DeleteEntity(ghostChoppVehiclee)
	end

    local ghostChopveh = GetHashKey(ChopCars[math.random(#ChopCars)])
    RequestModel(ghostChopveh)
    while not HasModelLoaded(ghostChopveh) do
        Citizen.Wait(0)
    end
    ...
    ghostChoppVehiclee = CreateVehicle(ghostChopveh, x, y, z, h, true, false)

	local pos = GetEntityCoords(ghostChoppVehiclee, false)
    Citizen.CreateThread(function()
        while true do
          Citizen.Wait(5)
          if GetVehiclePedIsIn(PlayerPedId(), false) == ghostChoppVehiclee then
