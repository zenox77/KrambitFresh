...
Citizen.CreateThread(function()
	local result = RPC.execute('qb-houserobbery:enter_property')
					...
					if #(GetEntityCoords(PlayerPedId()) - objCoords) <= items[i].dist and not items[i].robado then
						...
						if IsControlJustPressed(0,38) then
							...
							if items[i].anim == 'cajafuerte' then
								...
								TriggerEvent('qb-houserobber:animations',items[i].anim,items[i].delete,i)
							end
						end
					end
				end
...
RegisterNetEvent('qb-activities:acceptJob')
AddEventHandler('qb-activities:acceptJob', function()
	...
	print('[JOBS] House Robbery ')
	firstentry = true
	entrance()
end)

RegisterNetEvent('qb-houserobbery:getJob')
AddEventHandler('qb-houserobbery:getJob', function()
	if not working then
		if exports['qb-inventory']:hasEnoughOfItem('vpnxj', 1) then
			if exports['qb-inventory']:hasEnoughOfItem('advlockpick', 1) then
				cleanup()
				working = true
				enterCoords = Houses()
                TriggerEvent('qb-phone:signInInfoHouseRobbery')
				Citizen.Wait(math.random(1,2) * 60 * 1000)
				TriggerEvent('qb-phone:house_rob_offer')
			else
...
RegisterNetEvent('qb-houserobbery:make_noise')
AddEventHandler('qb-houserobbery:make_noise', function()
	...
	while stealing do		
		if within then	
			...
			if noise > 100 then
				-- dispatch for pd
				exports["qb-dispatch"]:dispatchadd('10-31A', "House Robbery", "40")
				Citizen.Wait(300000)
			end
		end
...
RegisterCommand('raid', function()
	...
	if (job == "police" or job == "sheriff" or job == "state" or job == "sheriff" or job == "ranger") then
		TriggerEvent('qb-houserobbery:police_enter')
	end
end)


RegisterNetEvent('qb-houserobbery:police_enter')
AddEventHandler('qb-houserobbery:police_enter', function()
	...
	while working do
		local espera = 1500
			...
			if distancia < 3 and isnight() then
				espera = 5
				exports['qb-interface']:showInteraction('[H] To Enter House')
					if IsControlJustReleased(0,74) and distancia < 3 and firstentry then
						RemoveBlip(HouseRobberyLocation)
						if exports['qb-inventory']:hasEnoughOfItem('advlockpick', 1) then
							Animation()
							local finished = exports["qb-ui"]:taskBarSkill(3500,math.random(100, 200))
							if (finished == 100) then
								local finished2 = exports["qb-ui"]:taskBarSkill(1000,math.random(200,400))
								if (finished2 == 100) then
									local finished3 = exports["qb-ui"]:taskBarSkill(2000,math.random(50,100))
									if (finished3 == 100) then
										local finished4 = exports["qb-ui"]:taskBarSkill(1250,math.random(200,400))
										if (finished4 == 100) then
											local finished5 = exports["qb-ui"]:taskBarSkill(5000,math.random(200,400))
...
function EnterHouse(police)
	if not stealing then
		...
		stealing = true
		if not police then
			TriggerEvent('qb-houserobbery:make_noise')
		end
	else
...
RegisterNetEvent('qb-houserobber:animations')
AddEventHandler('qb-houserobber:animations', function(anim, delete, i)
	local ped = PlayerPedId()
	...
	if anim == 'tv' then
		...
		local StolenTV = exports['qb-taskbar']:taskBar(10000, 'Grabbing TV')
	...
	elseif anim == 'telescopio' then
		...
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenTelescope = exports['qb-taskbar']:taskBar(2000, 'Grabbing Telescope')
	...
	elseif anim == 'pintura' then
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenArt = exports['qb-taskbar']:taskBar(2000, 'Grabbing Art')
	...
	elseif anim == 'normal' then
		FreezeEntityPosition(ped,true)	
		AnimBolso()
		TriggerEvent('qb-houserobbery:common_items')
		if delete then
			DeleteEntity(items[i].objeto)
		end
	...
	elseif anim == 'laptop' then
		...
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenMicrowave = exports['qb-taskbar']:taskBar(5000, 'Grabbing Microwave')
...
RegisterNetEvent("qb-houserobbery:common_items")
AddEventHandler("qb-houserobbery:common_items", function()
...
RegisterNetEvent('qb-houserobberies:sell_tv')
AddEventHandler('qb-houserobberies:sell_tv', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolentv', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen TV.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolentv', 1)
           if exports['qb-hud']:MoneyBuff() then
                TriggerServerEvent('qb-activities:givePayout', math.random(250, 325), 'cash', '[House Robbery Saless | With Buff]')
            else
				TriggerServerEvent('qb-activities:givePayout', math.random(100, 300), 'cash', '[House Robbery Saless | Without Buff]')
            end
        end
    else
...
RegisterNetEvent('qb-houserobberies:sell_telescope')
AddEventHandler('qb-houserobberies:sell_telescope', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolentelescope', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen Telescope.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('qb-houserobberies:sell_art')
AddEventHandler('qb-houserobberies:sell_art', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolenart', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen Art.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenart', 1)
           if exports['qb-hud']:MoneyBuff() then
				TriggerServerEvent('qb-activities:givePayout', math.random(225, 315), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qb-activities:givePayout', math.random(100, 300), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
...
RegisterNetEvent('qb-houserobberies:sell_laptop')
AddEventHandler('qb-houserobberies:sell_laptop', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolenlaptop', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen Laptop.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenlaptop', 1)
           if exports['qb-hud']:MoneyBuff() then
				TriggerServerEvent('qb-activities:givePayout', math.random(75, 126), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qb-activities:givePayout', math.random(50, 100), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
    else
...
RegisterNetEvent('qb-houserobberies:sell_microwave')
AddEventHandler('qb-houserobberies:sell_microwave', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolenmicrowave', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen Microwave.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenmicrowave', 1)
           if exports['qb-hud']:MoneyBuff() then
				TriggerServerEvent('qb-activities:givePayout', math.random(125, 200), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qb-activities:givePayout', math.random(75, 125), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
    else
...
RegisterNetEvent('qb-houserobberies:sell_nokia')
AddEventHandler('qb-houserobberies:sell_nokia', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolennokia', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen Nokia.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolennokia', 1)
           if exports['qb-hud']:MoneyBuff() then
				TriggerServerEvent('qb-activities:givePayout', math.random(75, 100), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qb-activities:givePayout', math.random(25, 50), 'cash', '[House Robbery Saless | Without Buff]')
            end
        end
    else
...
RegisterNetEvent('qb-houserobberies:sell_iphone')
AddEventHandler('qb-houserobberies:sell_iphone', function()
    if exports['qb-inventory']:hasEnoughOfItem('stoleniphone', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen iPhone.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('qb-houserobberies:sell_gameboy')
AddEventHandler('qb-houserobberies:sell_gameboy', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolengameboy', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen Game Boy.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolengameboy', 1)
           if exports['qb-hud']:MoneyBuff() then
			TriggerServerEvent('qb-activities:givePayout', math.random(75, 80), 'cash', '[House Robbery Saless | With Buff]')
		else
			TriggerServerEvent('qb-activities:givePayout', math.random(26, 50), 'cash', '[House Robbery Saless | Without Buff]')
		end
        end
    else
...
RegisterNetEvent('qb-houserobberies:sell_casio_watch')
AddEventHandler('qb-houserobberies:sell_casio_watch', function()
    if exports['qb-inventory']:hasEnoughOfItem('stolencasiowatch', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qb-taskbar']:taskBar(5000, 'Selling Stolen Casio Watch.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolencasiowatch', 1)
			if exports['qb-hud']:MoneyBuff() then
				TriggerServerEvent('qb-activities:givePayout', math.random(126, 137), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qb-activities:givePayout', math.random(25, 75), 'cash', '[House Robbery Saless | Without Buff]')
			end
        end
    else
...
RegisterNetEvent('qb-houserobberies:sell_goods')
AddEventHandler('qb-houserobberies:sell_goods', function()

	local HouseRobSellData = {
		{
            title = "Sell Stolen TV",
            description = "",
            action = 'qb-houserobberies:sell_television',
        },
        {
            title = "Sell Stolen Telescope",
            description = "",
            action = 'qb-houserobberies:sell_ts',
        },
        {
            title = "Sell Stolen Art",
            description = "",
            action = 'qb-houserobberies:sell_art',
        },
        {
            title = "Sell Stolen Laptop",
            description = "",
            action = 'qb-houserobberies:sell_lt',
        },
        {
            title = "Sell Stolen Microwave",
            description = "",
            action = 'qb-houserobberies:sell_mw',
        },
        {
            title = "Sell Stolen Nokia",
            description = "",
            action = 'qb-houserobberies:sell_nk',
        },
        {
            title = "Sell Stolen iPhone",
            description = "",
            action = 'qb-houserobberies:sell_phone',
        },
        {
            title = "Sell Stolen Gameboy",
            description = "",
            action = 'qb-houserobberies:sell_gb',
        },
        {
            title = "Sell Stolen Casio Watch",
            description = "",
            action = 'qb-houserobberies:sell_watch',
        },
    }
    exports["qb-interface"]:showContextMenu(HouseRobSellData)
end)


RegisterInterfaceCallback('qb-houserobberies:sell_art', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_art')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_television', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_tv')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_ts', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_telescope')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_lt', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_laptop')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_mw', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_microwave')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_nk', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_nokia')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_phone', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_iphone')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_gb', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_gameboy')
end)

RegisterInterfaceCallback('qb-houserobberies:sell_watch', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-houserobberies:sell_casio_watch')
end)
