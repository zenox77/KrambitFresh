Config = {
    ...
    local bliptable = {}
        ...
        RegisterNetEvent('start-oxy', function()
            if exports['qb-inventory']:hasEnoughOfItem('darkmarketdeliveries', 1) and exports['qb-inventory']:hasEnoughOfItem('vpnxj', 1) then
                TriggerEvent('qb-phone:oxyRunNoti', 'Wait for a job offer !')
                Citizen.Wait(math.random(60000, 120000))
                TriggerEvent('qb-phone:oxyrun:offer')
            else
                TriggerEvent('DoLongHudText', 'You need a delivery list and a VPN to work here !', 2)
            end
        end)

        RegisterNetEvent('qb-oxy:Accept:Offer')
        AddEventHandler('qb-oxy:Accept:Offer', function()
            TriggerServerEvent('started-job')
            TriggerEvent('qb-phone:oxyRunNoti', 'Head towards the location on your GPS.')
        end)
        ...
        RegisterNetEvent('generate-ped-loc', function()
            ...
            exports["qb-interact"]:AddPeekEntryByModel(GetHashKey('a_m_m_bevhills_01'), {{
        ...
        RegisterNetEvent('collect-package', function()
            if toplampaket < 10 then
                ...
                if not exports['qb-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
                    runStarted = true
                    TriggerEvent("player:receiveItem", "darkmarketpackage", 1, false, nil)
                    toplampaket = toplampaket+1
                    TriggerEvent('qb-phone:oxyRunNoti', '('..toplampaket..'/10) Packages Collected.')

                    if toplampaket == 10 then
                        TriggerEvent('qb-phone:oxyRunNoti', 'All Packages Collected.')
        ...
        RegisterNetEvent('generate-new-corner-loc', function()
            ...
            jobs = jobs+1
            TriggerEvent('qb-phone:oxyRunNoti', 'Drive to the next location.')
        ...
        RegisterNetEvent('handoff-package', function()
            ...
            if exports['qb-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
                if licensePlate == pPlate then
                    ...
                    if handoff < 5 then
                        ...
                        handoff = handoff+1
                        TriggerEvent('qb-phone:oxyRunNoti', '('..handoff..'/5) Packages Handed Off.')
                        ...
                        if exports['qb-inventory']:hasEnoughOfItem('darkmarketpackage', 1, true) then
                            local rnd = math.random(1, 2)
                            if rnd == 1 then
                                exports["qb-dispatch"]:dispatchadd('10-37', "Investigate Suspicious Activity", "102")
                                TriggerEvent("player:receiveItem", "oxy", math.random(1, 5))
                            end
                            TriggerEvent("inventory:removeItem", "darkmarketpackage", 1)
                            TriggerEvent('qb-oxyruns:clean')
                            Wait(500)
                            TaskVehicleDriveWander(ped, veh, 20.0, 16777216)
                            SetDriverAggressiveness(ped, 0.0)
                            Wait(15000)
        ...
        AddEventHandler("qb-oxyruns:clean", function(pRandomChance)
            local payment = 0
            for typ, conf in pairs(billsCleaningStuff) do
              ...
              if math.random() < randomChance and exports["qb-inventory"]:hasEnoughOfItem(typ, totalAmount, false) then
                ...
                if exports['qb-hud']:MoneyBuff() then
                    TriggerServerEvent('qb-activities:givePayout', payment*2, 'cash', '[Oxy Run | With Buff | Cleaned Bands]')
                else
                    TriggerServerEvent('qb-activities:givePayout', payment, 'cash', '[Oxy Run | Without Buff | Cleaned Bands]')
                end
              end
            end
            if payment == 0 then
                TriggerServerEvent('qb-activities:givePayout', math.random(150, 350), 'cash', '[Oxy Run | Without Buff | Reg Payout]')
            end
          end)
          ...
          function thread()
            ...
            if Config.TargetName == 'q' then
                ...
                exports["qb-polytarget"]:AddCircleZone("oxystart", vector3(-1564.06, -440.33, 37.39), 0.55, {
                    useZ=true,
                })
                
                exports["qb-interact"]:AddPeekEntryByPolyTarget("oxystart", {
                    {
                        event = "start-oxy",
                        id = "oxystart1",
                        icon = "circle",
...
-- local function strtTimer ()
--     local time = 120
--     while time > 0 do
--         Citizen.Wait(1000) --every sec recude timer
--         time = time - 1
--         if time <= 0 then 
--             DeleteEntity(ped)
--             DeleteEntity(veh)
