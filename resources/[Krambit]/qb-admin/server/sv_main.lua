...
function sendLog(link, color, title, description, footer)
    ...
    PerformHttpRequest(link, function(err, text, headers) end, 'POST', json.encode({username = "Nopixel",  avatar_url = "https://cdn.discordapp.com/attachments/ENTER_HERE/ENTER_HERE.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end
...
AddEventHandler("np:admin:searchPlayerInventory", function(pTarget)
    local user = exports["qb-base"]:getModule("Player"):GetUser(tonumber(pTarget))
    local cid = user:getCurrentCharacter().id
    TriggerClientEvent("server-inventory-open", source, "1", 'ply-'..cid)
end)

RegisterServerEvent('qb-admin:insertPrio')
AddEventHandler('qb-admin:insertPrio', function()
    local src = source
...
RegisterNetEvent("qb-admin:server:play-sound", function(ServerId, SoundId)
    local src = source
    TriggerClientEvent('qb-admin:cl:play-sound', ServerId, SoundId)
end)