function DPX.Core.LoginPlayer(self, args, src, callback)
    TriggerEvent("qb-base:playerAttemptLogin", src)
    ...
    local function fetchData(_err)
        ...
        DPX.DB:FetchPlayerData(src, function(data, err)
            ...
            if not err then TriggerEvent("qb-base:playerLoggedIn", user) TriggerClientEvent("qb-base:playerLoggedIn", src) end
        end)
    end
	...
	DPX.DB:PlayerExistsDB(src, function(exists, err)
		...
		if not exists then
			DPX.DB:CreateNewPlayer(src, function(created)
				...
				TriggerClientEvent('qb-admin:InsertPrioCL', src)
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.LoginPlayer, "qb-base:loginPlayer")
...
function DPX.Core.FetchPlayerCharacters(self, args, src, callback)
	...
	DPX.DB:FetchCharacterData(user, function(data, err)
		if err then
			...
			TriggerEvent("qb-base:charactersLoaded", user, data)
			TriggerClientEvent("qb-base:charactersLoaded", src, data)
		end

		callback(data)
	end)
end
DPX.Events:AddEvent(DPX.Core, DPX.Core.FetchPlayerCharacters, "qb-base:fetchPlayerCharacters")
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.CreateCharacter, "qb-base:createCharacter")
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.DeleteCharacter, "qb-base:deleteCharacter")
...
function DPX.Core.SelectCharacter(self, id, src, callback)
	...
	TriggerClientEvent('qb-base:setcontrols', src)
	TriggerClientEvent('updatepasses', src)

	TriggerEvent("qb-base:characterLoaded", user, selectedCharacter)
	TriggerClientEvent("qb-base:characterLoaded", src, selectedCharacter)

	callback({loggedin = true, chardata = selectedCharacter})
end
DPX.Events:AddEvent(DPX.Core, DPX.Core.SelectCharacter, "qb-base:selectCharacter")