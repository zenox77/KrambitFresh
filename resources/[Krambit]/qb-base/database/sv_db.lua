...
function DPX.DB.CreateNewCharacter(self, src, data2, hexid, ph, callback)
    ...
    callback = callback and callback or function() return end
    local data = data2
    local jailTime , prisonTime

    if data.type == 'lifer' then
        jailTime = 'jail'
        prisonTime = 9999999 * 9999
    ...
    local q = [[INSERT INTO characters (owner, first_name, last_name, dob, gender, phone_number, story,jail,jail_time)
                VALUES(@owner, @first_name, @last_name, @dob, @gender, @phone_number, @story,@jail,@jail_time);]]
    local v = {
        ["owner"] = hexid,
