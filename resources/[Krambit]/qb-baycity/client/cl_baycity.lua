...
-- -105.12893676758, 6471.46484375, 31.626705169678 Heading: 40.885322570801
            if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge",1,false) then
                FreezeEntityPosition(PlayerPedId(),true)
                    ...
                    function() -- Success
                        ...
                        TriggerEvent('qb-vaultDoor3',1)
                        TriggerEvent("inventory:removeItem","thermitecharge",1)
                        TriggerEvent('DoLongHudText', 'You bypassed the system!', 2)
...
AddEventHandler('BayCitySecondDoor', function()
    ...
    if distance < 1.5 then
        if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge",1,false) then
            FreezeEntityPosition(PlayerPedId(),true)
            local FirstDoorBigBank = exports["qb-taskbar"]:taskBar(9000,"Preparing For Hack")
            if FirstDoorBigBank == 100 then
                FreezeEntityPosition(PlayerPedId(),false)
                Wait(1000)
                exports["memorygame"]:thermiteminigame(1, 3, 4, 10,
                 function() -- Success
                     TriggerEvent("qb-BayCityTriggerSecondDoor")
                     TriggerEvent("inventory:removeItem","thermitecharge",1)
                     TriggerEvent('DoLongHudText', 'You bypassed the system!', 2)
...
RegisterNetEvent('qb-BayCityTriggerSecondDoor')
AddEventHandler('qb-BayCityTriggerSecondDoor', function()
	anim66()
	Citizen.Wait(3500)
end)

RegisterNetEvent('qb-Seconddoorsbaycity') 
AddEventHandler('qb-Seconddoorsbaycity', function()
    TriggerServerEvent("qb-doors:change-lock-state", 524, false)
end)
    ...
    if distance < 1.5 then
        if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge",1,false) then
            ...
            if FirstDoorBigBank == 100 then
                ...
                exports["memorygame"]:thermiteminigame(1, 3, 4, 10,
                function() -- Success
                    TriggerEvent("qb-BayCityTriggerFirstDoor")
                    TriggerEvent("inventory:removeItem","thermitecharge",1)
                    TriggerEvent('DoLongHudText', 'You bypassed the system!', 2)
...
RegisterNetEvent('qb-BayCityTriggerFirstDoor')
AddEventHandler('qb-BayCityTriggerFirstDoor', function()
	anim69()
	Citizen.Wait(3500)
end)

RegisterNetEvent('qb-firstdoorsbaycity') 
AddEventHandler('qb-firstdoorsbaycity', function()
    TriggerServerEvent("qb-doors:change-lock-state", 525, false)
end)

RegisterCommand('adkasjdlkajdaj',function()
    TriggerEvent('qb-vaultDoor3',1)
end)

RegisterNetEvent('qb-vaultDoor3')
AddEventHandler('qb-vaultDoor3', function(method)
    local obj = GetClosestObjectOfType(-1306.7009277344, -815.44757080078, 17.308031082153, 2.0, 961976194, false, false, false)
    local count = 0
    print('[BAYCITY] Rotate door')
    if (method == 1) then

        repeat
            local rot = GetEntityHeading(obj) - 0.3
...
RegisterNetEvent("qb-baycity:getDoorCheckCL")
AddEventHandler("qb-baycity:getDoorCheckCL", function(result)
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('qb-baycity:getGetDoorStateSV')
...
RegisterNetEvent("qb-ptfxparticlebaycity")
AddEventHandler("qb-ptfxparticlebaycity", function()
    local ptfx
...
-- 252.99769592285, 220.96379089355, 101.68360137939 Heading: 166.93214416504
    ...
    TriggerServerEvent("qb-particleserverbaycity")
    SetPtfxAssetNextCall("scr_ornate_heist")
    local effect = StartParticleFxLoopedAtCoord("scr_heist_ornate_thermal_burn", ptfx, 0.0, 0.0, 0.0, 1.0, false, false, false, false)
    ...
    DeleteObject(bomba)
    StopParticleFxLooped(effect, 0)
	TriggerEvent("qb-Seconddoorsbaycity")
end

RegisterNetEvent("qb-ptfxparticlebaycity1")
AddEventHandler("qb-ptfxparticlebaycity1", function()
    local ptfx
    RequestNamedPtfxAsset("scr_ornate_heist")
    while not HasNamedPtfxAssetLoaded("scr_ornate_heist") do
        Citizen.Wait(1)
...
-- 252.99769592285, 220.96379089355, 101.68360137939 Heading: 166.93214416504
    ...
    DetachEntity(bomba, 1, 1)
    FreezeEntityPosition(bomba, true)
    TriggerServerEvent("qb-particleserverbaycity1", method)
    SetPtfxAssetNextCall("scr_ornate_heist")
    local effect = StartParticleFxLoopedAtCoord("scr_heist_ornate_thermal_burn", ptfx, 0.0, 0.0, 0.0, 1.0, false, false, false, false)
