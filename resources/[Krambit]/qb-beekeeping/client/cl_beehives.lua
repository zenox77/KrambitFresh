        ...
        if inZone > 0 then
            ...
            placeObject("np_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    local finished = exports["qb-taskbar"]:taskBar(1500, "Installing beehive", false, true, false, false, nil, 5.0, PlayerPedId())
                    ClearPedTasks(PlayerPedId())
                    if finished == 100 then
                        RPC.execute("qb-beekeeping:installHive", pCoords, pHeading)
...
AddEventHandler("qb-beekeeping:checkBeehive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    showHiveMenu(hiveId)
end)

RegisterUICallback('qb-beekeeping:addQueen', function (data)
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["qb-taskbar"]:taskBar(15000, "Adding Queen bee", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("qb-beekeeping:addQueen", data.key.id)
        TriggerEvent("inventory:removeItem", "beequeen", 1)
    end
end)

RegisterUICallback('qb-beekeeping:removeHive', function (data)
    ...
    if finished == 100 then
        ...
        local success = RPC.execute("qb-beekeeping:removeHive", data.key, getReady)
        if not success then
...
AddEventHandler("qb-beekeeping:harvestHive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["qb-taskbar"]:taskBar(5000, "Harvesting", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("qb-beekeeping:harvestHive", hiveId)
    end
end)

AddEventHandler("qb-polyzone:enter", function(zone, data)
    if zone == "qb-beekeeping:bee_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            local result = RPC.execute("qb-beekeeping:getBeehives")
        end
    end
end)
...
RegisterNetEvent('qb-beekeeping:trigger_zone')
AddEventHandler('qb-beekeeping:trigger_zone', function (type, pData)
    --Update all hives
    if type == 1 then
...
function showHiveMenu(pHiveId)
    ...
    if getStageFromPercent(harvest) < 2 and not tobool(hive.has_queen) then
        context[#context+1] = {
            title = 'Add Queen bee',
            key = { id = pHiveId },
            action = 'qb-beekeeping:addQueen',
            description = 'Make the hive happy.',
            disabled = not exports["qb-inventory"]:hasEnoughOfItem("beequeen", 1, false)
        }
    end
    ...
    if harvest >= 95 or myJob == "police" or myJob == "sheriff" or myJob == "state"  then
        context[#context+1] = {
            title = 'Destroy Hive',
            key = { id = pHiveId },
            action = 'qb-beekeeping:removeHive',
        }
    end

    exports['qb-ui']:showContextMenu(context)
end
