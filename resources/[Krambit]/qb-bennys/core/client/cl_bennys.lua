...
local function saveVehicle()
    ...
    -- TriggerEvent("player:receiveItem", "bennysorder", 1, false, vehicleMods)
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qb-bennys:attemptPurchase",cheap, type, upgradeLevel)
...
function RepairVehicle()
    ...
    ExitBennys()
    local finished = exports["qb-taskbar"]:taskBar(10000 + (missingBodyHealth / 50), "Repairing vehicle...", false, true)
...
function ExitBennys()
    ...
    TriggerServerEvent("qb-bennys:removeFromInUse", currentBennys)
    if currentBennys:find('^devbennys') then
        --
    else
        exports["qb-interface"]:showInteraction("Bennys")
    end
    currentBennys = nil
end
...
function IsTunerWorker()
    local mypasses = exports['qb-business']:IsEmployedAt('tuner_shop')
    print('Mypasses' , mypasses)
    if mypasses then
        print('Return true')
        return true
    end
...
function IsHayesWorker()
    local mypasses = exports['qb-business']:IsEmployedAt('hayes_autos')
        if mypasses then
            return true
        end
    return false
...
--#[Event Handlers]#--
RegisterNetEvent("qb-bennys:purchaseSuccessful")
AddEventHandler("qb-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("qb-bennys:purchaseFailed")
AddEventHandler("qb-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    --[[local isBennysInUse = RPC.execute("qb-bennys:checkIfUsed", currentBennys)
    ...
    ::hasAccess::
    TriggerServerEvent("qb-bennys:addToInUse", currentBennys)
    print(currentBennys)
    ...
    elseif currentBennys == "bennyshayes" then
        if exports['qb-business']:IsEmployedAt('hayes_autos') then
            enterLocation(bennys, repair)
        end
    ...
    elseif currentBennys == "bennysharmony" then
        if exports['qb-business']:IsEmployedAt('harmony_repairs') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "ottosbay1" then
        if exports['qb-business']:IsEmployedAt('ottos_autos') then
            enterLocation(bennys, repair)
        end
    ...
    elseif currentBennys == 'bennyspaleto' then
        enterLocation(bennys, repair)
...
RegisterNetEvent("qb-bennys:repairVeh")
AddEventHandler('qb-bennys:repairVeh', function()
    ...
    if car ~= 0 then
        ...
        local finished = exports["qb-taskbar"]:taskBar(4000,"Repairing Engine",false,false,playerVeh)
        Citizen.Wait(1000)
        local finished = exports["qb-taskbar"]:taskBar(3000,"Repairing Body",false,false,playerVeh)
        if (finished == 100) then
...
RegisterNetEvent('qb-public-bennys')
AddEventHandler('qb-public-bennys', function()
    TriggerServerEvent('qb-bennys:payPublicBennys')
end)

Citizen.CreateThread(function()
    exports["qb-polyzone"]:AddBoxZone("pubbennys", vector3(-33.2, -1053.37, 28.4), 8.0, 9.4, {
        heading=340,
        minZ=25.72,
        maxZ=29.52
    })
