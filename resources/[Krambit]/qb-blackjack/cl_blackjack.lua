...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
   -- dwRefreshChipCount = false
    dwCurrentChipCount = RPC.execute("qb-casino:getCurrentChipCount")
  end
  ...
  exports["qb-interface"]:sendAppEvent("status-hud", {
    show = true,
    title = title,
    values = values,
  })
--   SendNUIMessage({ -- If you don't have qb-interface
--     app = "status-hud",
--     show = false,
--     data = {
--         show = true,
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['qb-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                        ...
                        -- exports["qb-interface"]:showInteraction(
                        --   "[E] Play! " .. dwGetLimitTitleString(),
                        --   background
                        -- )
                        exports["qb-interface"]:showInteraction("[E] Play! " .. dwGetLimitTitleString() , background)
                        uiIntShown = true
                    end
                end
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['qb-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['qb-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
...
RegisterInterfaceCallback("qb-interface:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("qb-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("qb-casino:winChips", currentBetAmount / 2)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "qb-interface:blackjackHitStand",
...
RegisterInterfaceCallback("qb-interface:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("qb-interface:application-closed", function(name, data)
...
function dwWaitForBet(act)
  ...
  if waitingForBetState then
    ...
    if sittingAtBlackjackTable then
      ...
      local data = {}
      data[#data + 1] = {
        action = "qb-interface:blackJackBet",
        title = "Place bet",
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("qb-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("qb-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
