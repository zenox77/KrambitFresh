...
AddEventHandler("blindfold:blindfold", function(pArgs, pEntity)
  ...
  isBlindfolded = exports["qb-flags"]:HasPedFlag(tPed, "isBlindfolded")
  ...
  local blindfoldtask = exports["qb-taskbar"]:taskBar(time, string)

  if blindfoldtask == 100 then
    if isBlindfolded then
      exports['qb-flags']:SetPedFlag(tPed, 'isBlindfolded', false)
    else
      exports['qb-flags']:SetPedFlag(tPed, 'isBlindfolded', true)
    end
  end
