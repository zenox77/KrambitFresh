...
function AddGNE(cid, pAmount)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
        print('Adding [GNE]')
        if result[1].gne ~= nil then
        exports.oxmysql:execute("UPDATE boosting_users SET `gne` = @gne WHERE identifier = @cid", {
            ['gne'] = result[1].gne + pAmount,
            ['cid'] = cid
        })
    else
        error( '[MAJOR]' .. 'CID ' .. cid .. ' ' .. 'Not in DB? ')
    end
    end)
end

RegisterServerEvent('qb-boosting:removeGNE')
AddEventHandler('qb-boosting:removeGNE', function(amount, cid)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
...
RegisterNetEvent("boosting:spawn:playerspawned")
AddEventHandler("boosting:spawn:playerspawned" , function()
    local src = source
    local query = GetContractToDB(GetIdentifier(src))
    InitBoostingUser(src)
    print('^1 [Boosting] Player ' .. src .. ' Just loaded in handle contracts..')
...
RegisterServerEvent('qb-boosting:pay_gne')
AddEventHandler('qb-boosting:pay_gne', function()
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
    local gnes = GetGNE(cid)
...
RegisterServerEvent('qb-boosting:giveGNE')
AddEventHandler('qb-boosting:giveGNE', function(amount)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
