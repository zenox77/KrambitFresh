...
RegisterCommand("creationmenu", function()
	...
	local hasPerm = RPC.execute("qb-business:hasPermission", "ace_industries", "craft_access")
	
	if employed and hasPerm then
		TriggerEvent("qb-business:stash:creationMenu")
	end
end)
...
RegisterInterfaceCallback("qb-business:stash:enteredAuthCode", function(data, cb)
...
function getPDAuthCode()
  ...
  exports["qb-interface"]:openApplication("textbox", {
    callbackUrl = "qb-business:stash:enteredAuthCode",
    key = "",
    items = {
...
function openInventory(type, data)
  ...
  if type == "stash" then
    local success = RPC.execute("qb-business:hasPermission", biz, "stash_access")
  ...
  elseif type == "craft" then
    local success = RPC.execute("qb-business:hasPermission", biz, "craft_access")
    if not success and isPD() then
      success = getPDAuthCode()
    end
    if success then
...
function enterPoly(name, data)
  ...
  exports["qb-interface"]:showInteraction("[E] " .. name)
end
...
local creationMenu = {
  key = 1,
  show = true,
  callbackUrl = "qb-business:client:createStash",
  items = { }
}

Citizen.CreateThread(function()
  exports["qb-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      id = "business_stash",
      event = "qb-business:client:openStash",
      icon = "archive",
      label = "Open Storage",
    },
  }, 
  { 
    ...
    isEnabled = function(pEntity)
      local objInfo = exports["qb-objects"]:GetObjectByEntity(pEntity)
      return objInfo ~= nil
    end
  })
  exports["qb-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      ...
      event = "qb-business:client:removeStash",
      icon = "trash",
      label = "Remove Storage",
    },
    {
      id = "business_stash_update",
      event = "qb-business:client:changeStashModel",
      icon = "archive",
...
RegisterNetEvent("qb-business:stash:creationMenu")
AddEventHandler("qb-business:stash:creationMenu", function ()
  ...
  exports["qb-interface"]:openApplication("textbox", creationMenu)
end)

AddEventHandler("qb-business:client:removeStash", function (pData, pEntity)
  local objInfo = exports["qb-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end

	TriggerEvent("animation:PlayAnimation", "hammering")

  local finished = exports["qb-taskbar"]:taskBar(15000, "Destroying Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local deleted = exports["qb-objects"]:DelObject(objInfo.id)
  if not deleted then
...
AddEventHandler("qb-business:client:openStash", function (pData, pEntity)
  local objInfo = exports["qb-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil or not objInfo or rawequal(next(objInfo), nil) then print('No information on this object..') return end
  ...
  local success = RPC.execute("qb-business:hasPermission", businessCode, "stash_access")
  if not success and isPD() then
    success = getPDAuthCode()
  end
...
AddEventHandler("qb-business:client:changeStashModel", function (pData, pEntity)
  print('pEntity we are at :',pEntity)
  local objInfo = exports["qb-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local menuData = {
    key = { id = objInfo.id },
    show = true,
    callbackUrl = "qb-business:client:updateStashModel",
    items = {
      { icon = "object-ungroup", _type = "select", label = "New Unit Model", name = "stashProp", options = unitOptions },
    }
  }
  exports["qb-interface"]:openApplication("textbox", menuData)
end)

RegisterInterfaceCallback("qb-business:client:updateStashModel", function(pData, pCb) 
  ...
  local finished = exports["qb-taskbar"]:taskBar(15000, "Changing Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local updated = exports["qb-objects"]:UpdateObject(pData.key.id, stashProp)
end)

RegisterInterfaceCallback("qb-business:client:createStash", function(pData, pCb) 
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  exports["qb-interface"]:closeApplication("textbox")
  ...
  local result = exports["qb-objects"]:PlaceAndSaveObject(
    stashProp, 
    metaData, 
    { groundSnap = true }
  )
