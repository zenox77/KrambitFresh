...
local function explodeVehicle(pEntity)
  exports['qb-sync']:SyncedExecution('NetworkExplodeVehicle', pEntity, 1, 0, 0)
  TriggerServerEvent('qb-miscscripts:carbombs:removeBomb', NetworkGetNetworkIdFromEntity(pEntity))
end
...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['qb-taskbar']:taskBar(2500, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = RPC.execute("qb-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
...
--[[   local remoteBomb = RPC.execute('qb-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))

  if remoteBomb and remoteBomb.planted then
    local number = remoteBomb.bomb.number
    TriggerServerEvent('qb-usableprops:defusePhoneBomb', number)
    TriggerEvent('qb-police:client:showTextPopup', { show = true, text = 'Cell Phone Found, Number: ' .. number })
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    TriggerServerEvent('qb-miscscripts:carbombs:foundBomb', NetworkGetNetworkIdFromEntity(pEntity), carBombMeta)
...
RegisterNetEvent("qb-miscscripts:carbombs:foundBombAll", function(pNetTable)
...
AddEventHandler("qb-inventory:itemUsed", function(name, info)
  ...
  local progress = exports['qb-taskbar']:taskBar(2500, 'Planting car bomb...', true)
  ...
  local added = RPC.execute('qb-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, 0, timeToComplete)
  TriggerEvent('inventory:removeItem', 'car_bomb', 1)
  
  return TriggerEvent('DoLongHudText', 'Successfully added car bomb to vehicle', 1)
end)

AddEventHandler('qb-miscscripts:carBombs:removeBomb', function (pData, pEntity)
    local metaData = RPC.execute("qb-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
    local carBombMeta = metaData.carBombData or false
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    TriggerEvent('animation:PlayAnimation', 'kneel')
    TriggerServerEvent('qb-miscscripts:carbombs:removeBomb', myNetIdentifier)
...
--       gameFinishedEndpoint = 'qb-miscscripts:carBombs:completeHacking',
