...
function drawCarForWins()
  ...
  --   exports['qb-vehicles']:SetVehicleAppearance(spinningCar, carOnShowAppearence)
  -- end
  -- if carOnShowMods then
  --   exports['qb-vehicles']:SetVehicleMods(spinningCar, carOnShowMods)
  -- end
...
AddEventHandler("qb-casino:elevatorEnterCasino", function()
  enterCasino(true)
end)
AddEventHandler("qb-casino:elevatorExitCasino", function()
  enterCasino(false)
end)
...
function enterCasino(pIsInCasino)
  ...
  if not inCasino then
    ...
    TriggerEvent("qb-casino:casinoExitedEvent")
    inCasino = false
    postGateTriggered = false
    return
  end
  doInitStuff()
  TriggerEvent("qb-casino:casinoEnteredEvent")
end
    ...
    while inCasino do
      ...
      if inCasino and postGateTriggered then
        if hasMembership() then
          --RPC.execute("qb-casino:generateRaffleTicket")
        end
      end
    end
...
function doSomeTextureReplacesBruv(apply)
  if not apply then
    ...
    exports["qb-lib"]:releaseDui(wheelDui.id)
    wheelDui = nil
    return
  end
  if wheelDui == nil then
    wheelDui = exports["qb-lib"]:getDui(wheelUrl, 1024, 1024)
    AddReplaceTexture('vw_prop_vw_luckywheel_01a', 'script_rt_casinowheel', wheelDui.dictionary, wheelDui.texture)
  else
    exports["qb-lib"]:changeDuiUrl(wheelDui.id, wheelUrl)
  end
end
...
function showThe6StrImage(apply)
  if not apply then
    ...
    exports["qb-lib"]:releaseDui(tvDui.id)
    tvDui = nil
    return
  end
  if tvDui == nil then
    tvDui = exports["qb-lib"]:getDui(tvUri, 512, 256)
    AddReplaceTexture('vw_prop_vw_cinema_tv_01', 'script_rt_tvscreen', tvDui.dictionary, tvDui.texture)
  else
    exports["qb-lib"]:changeDuiUrl(tvDui.id, tvUri)
  end
end
...
-- chips
RegisterInterfaceCallback("qb-interface:casinoPurchaseChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local amount = data.values.amount 
  RPC.execute("qb-casino:purchaseChips", amount)
  exports['qb-interface']:closeApplication('textbox')
end)

RegisterInterfaceCallback("qb-interface:transferChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local amount = data.values.amount
  local stateId = data.values.state_id
  RPC.execute("qb-casino:transferChips", stateId, amount)
  exports['qb-interface']:closeApplication('textbox')
end)

AddEventHandler("qb-casino:purchaseChipsAction", function(pArgs)
  ...
  --     callbackUrl = 'qb-interface:casinoPurchaseChipsAmount',
  --     key = 1,
  --     header = "",
  --     items = {
  --       {
  ...
  if action == "purchase:dirty" then
    ...
    if exports["qb-inventory"]:hasEnoughOfItem("markedbills", 20, false, true) then
      TriggerEvent("inventory:removeItem", "markedbills", 20)
      payment = payment + (250 * 20) -- $5k / $250 per
    end
    if exports["qb-inventory"]:hasEnoughOfItem("rollcash", 5, false, true) then
      TriggerEvent("inventory:removeItem", "rollcash", 5)
      payment = payment + (30 * 5) -- $150 / $30 per
    end
    if exports["qb-inventory"]:hasEnoughOfItem("band", 5, false, true) then
      TriggerEvent("inventory:removeItem", "band", 5)
      payment = payment + (300 * 5) -- $1500, / $300 per
    end
    RPC.execute("qb-casino:purchaseChipsDirty", payment)

  elseif action == "withdraw:cash" then
    RPC.execute("qb-casino:withdrawChips", "cash")
  elseif action == "withdraw:bank" then
    RPC.execute("qb-casino:withdrawChips", "bank")
  -- elseif action == "transfer" then
  --   Wait(100)
  --   exports['qb-interface']:openApplication('textbox', {
  --     callbackUrl = 'qb-interface:transferChipsAmount',
  --     key = 1,
  --     items = {
  --       {
  --         icon = "user",
...
AddEventHandler("qb-casino:purchaseMembership", function()
  ...
  exports['qb-interface']:openApplication('textbox', {
    callbackUrl = 'qb-interface:casinoGetMembership',
    key = 1,
    header = '',
    items = {
      {
...
RegisterInterfaceCallback("qb-interface:casinoGetMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['qb-interface']:closeApplication('textbox')
  local state_id = {
...
AddEventHandler("qb-casino:purchaseMembershipCard", function()
  RPC.execute("qb-casino:purchaseMembershipCard")
end)

AddEventHandler("qb-casino:getLoyaltyCard", function()
  local characterId = exports["isPed"]:isPed("cid")
...
AddEventHandler("qb-casino:purchaseVipMembership", function()
  local characterId = exports["isPed"]:isPed("cid")
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "casino"} })
  if not jobAccess then
    TriggerEvent("DoLongHudText", "Please talk to a member of floor staff", 2)
  ...
  exports['qb-interface']:openApplication('textbox', {
    callbackUrl = 'qb-interface:casinoGetVipMembership',
    key = 1,
    header = '',
    items = {
      {
...
RegisterInterfaceCallback("qb-interface:casinoGetVipMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['qb-interface']:closeApplication('textbox')
  TriggerEvent("player:receiveItem", "casinovip", 1, false, { state_id = data.values.state_id }, { state_id = data.values.state_id })
end)

AddEventHandler("qb-casino:purchaseDrinks", function(data)
  TriggerEvent("server-inventory-open", "46", "Shop")
end)

function hasMembership(hrOnly)
  local characterId = exports["isPed"]:isPed("cid")
  local info = exports["qb-inventory"]:GetInfoForFirstItemOfName("casinomember")
  if not info then
    return false
  end
...
-- end, false)
-- Citizen.CreateThread(function()
--   exports["qb-keybinds"]:registerKeyMapping("", "Casino", "Add Coords", "+addCasinoCoords", "-addCasinoCoords")
-- end)

-- Citizen.CreateThread(function()
--   for _, v in pairs(exports["qb-casino"]:getPedCoordsC()) do
