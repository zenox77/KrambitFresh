...
Citizen.CreateThread(function()
    Elevators = RPC.execute('qb-doors:elevators:fetch')
    ...
    for _, elevator in ipairs(Elevators) do
        ...
        for floorId, floor in ipairs(floors) do
            ...
            local data, lib = zone.options.data or {}, zone.target and 'qb-polytarget' or 'qb-polyzone'

            data.floorId = floorId
            data.elevatorId = elevator.id

            exports[lib]:AddBoxZone("qb-doors:elevator", zone.center, zone.width, zone.length, zone.options)
        end
    end

    exports['qb-interact']:AddPeekEntryByPolyTarget('qb-doors:elevator', {
        {
            id = "elevatorPrompt",
            event = "qb-doors:elevator:prompt",
            icon = "chevron-circle-up",
            label = "Elevator"
        }}, { distance = { radius = 1.5 } })
end)

AddEventHandler('qb-doors:elevator:prompt', function(pParameters, pEntity, pContext)
    local data = pContext.zones and pContext.zones['qb-doors:elevator']

    if not data or not Elevators[data.elevatorId] then return end

    local hasHackingDevice = exports["qb-inventory"]:hasEnoughOfItem('elevatorhackingdevice', 1, false, true)
...
function OpenElevatorMenu(pElevator, pCurrentFloor, pBreachingDevice)
    ...
    for floorId, floor in ipairs(elevator.floors) do
        ...
        elements[#elements+1] = {
            ...
            action = (not isCurrentFloor and not isRestricted) and 'qb-doors:elevator:teleport' or '',
            key = floor.teleport,
        ...
        if hasAccess then
            access[#access+1] = {
                ...
                action = 'qb-doors:elevator:access',
    ...
    if not hasAccess and pBreachingDevice then
        elements[#elements+1] = {
            ...
            action = 'qb-doors:elevator:hackControlPanel',
            key = { type = "elevator", id = pElevator }
        }
    end

    exports['qb-interface']:showContextMenu(reverse(elements))
end
...
RegisterNetEvent('qb-doors:elevators:updateState')
AddEventHandler('qb-doors:elevators:updateState', function (pElevatorId, pFloorId, pRestricted, pForceUnlock)
...
RegisterInterfaceCallback('qb-doors:elevator:teleport', function (data, cb)
    ...
    local finished = exports["qb-taskbar"]:taskBar(time, "Waiting for the Elevator", false)
...
RegisterInterfaceCallback('qb-doors:elevator:access', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    TriggerServerEvent('qb-doors:change-elevator-state', elevatorId, floorId, not locked)
end)

RegisterInterfaceCallback('qb-doors:elevator:hackControlPanel', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local id = data.key.id
    local accessType = data.key.type

    local hackId = RPC.execute('qb-doors:startAccessHack', accessType, id)

    if not hackId then return TriggerEvent('DoLongHudText', 'Unable to establish connection', 2) end

    exports['qb-interface']:openApplication('memorygame', {
        gameFinishedEndpoint = 'qb-doors:elevator:completeHacking',
...
RegisterInterfaceCallback('qb-doors:elevator:completeHacking', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local success = data.success
    local hackId = data.parameters.hackId

    RPC.execute('qb-doors:completeAccessHack', success, hackId)

    clearAccessCache()

    Citizen.Wait(3000)

    exports['qb-interface']:closeApplication('memorygame')
end)