...
RegisterNetEvent('qb-doors:initial-lock-state')
AddEventHandler('qb-doors:initial-lock-state', function(pDoors)
    doors = pDoors
...
RegisterNetEvent('qb-doors:change-lock-state')
AddEventHandler('qb-doors:change-lock-state', function(pDoorId, pDoorLockState, pDoorForceUnlock)
...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState then
            exports["qb-interface"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess then
                        exports["qb-interface"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    else
                        exports["qb-interface"]:showInteraction((newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    end
                else
                    idle = 100
                end
            ...
            if currentDoorId ~= nil and hasAccess and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                TriggerServerEvent("qb-doors:change-lock-state", currentDoorId, not currentDoorLockState)
            end

            Wait(idle)
        end

        exports["qb-interface"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
AddEventHandler("qb-doors:doorKeyFob", function()
    ...
    if currentZone ~= nil and bollards[currentZone].inside then
        ...
        local entity = exports['qb-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    ...
    TriggerServerEvent("qb-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("qb-doors:request-lock-state")
end)

RegisterCommand("addnewdoor", function()
    local pEntity = exports['qb-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    TriggerServerEvent("qb-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
end, false)

RegisterNetEvent("qb-doors:add")
AddEventHandler("qb-doors:add", function(pArgs, pEntity, pContext)
...
RegisterNetEvent("qb-doors:detCoord")
AddEventHandler("qb-doors:detCoord", function ()
  ...
  Citizen.CreateThread(function()
    ...
    ClearPedTasks(ped) 
    DeleteObject(bomba)
    TriggerServerEvent("qb-doors:change-lock-state", targetDoor, 0)
