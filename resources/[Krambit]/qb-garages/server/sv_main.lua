...
RegisterServerEvent("qb-garages:resetpdgarage")
AddEventHandler("qb-garages:resetpdgarage",function ()
    ResetPDGarages()
end)



RegisterServerEvent("qb-garages:resetcartoimpound")
AddEventHandler("qb-garages:resetcartoimpound",function (plate)
...
RPC.register("qb-garages:select",function(pSource,garname)
	local src = source
	if garname.param == "Impound Lot" then
		exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE vehicle_state = @state', {['@state'] = "Out"}, function(vehicles)
			for k, v in ipairs(vehicles) do
				TriggerClientEvent("qb-garages:resetcartoimpound", src, v.license_plate)
			end
		end)
	end
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE cid = @cid AND current_garage = @garage ', { ['@cid'] = char.id, ["garage"] = garname.param }, function(result)
		if (result and result[1]) then
			for ind, value in pairs(result) do
				...
				TriggerClientEvent('qb-context:sendMenu', src, {
					{
						id = value.id,
						header = value.name,
						txt = 'Plate: ' .. value.license_plate,
						params = {
							event = "qb-garages:takeout",
							args = value.id
						}
					},
				})
...
RPC.register("qb-garages:selectShared", function(pSource, garname)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE current_garage = @garage ', { ["garage"] = garname.param }, function(result)
        if (result[1]) then
            ...
            -- Check if "Shared Garage" item already exists in the menu
            local sharedGarageExists = false
            for i, item in ipairs(menu) do
                if item.id == 1 then
                    sharedGarageExists = true
                    break
                end
            end

            -- Add "Shared Garage" item to menu if it doesn't exist
            if not sharedGarageExists then
                table.insert(menu, {
                    id = 1,
            ...
            -- Add other items to menu with dynamic values for id
            for ind, value in pairs(result) do
                enginePercent = value.engine_damage / 10
                bodyPercent = value.body_damage / 10
                curGarage = value.current_garage
                ...
                table.insert(menu, {
                    ...
                    txt = 'Plate: ' .. value.license_plate,
                    params = {
                        event = "qb-garages:takeout2",
                        args = value.id
                    }
                })
            end

            TriggerClientEvent('qb-context:sendMenu', src, menu)
        else
            TriggerClientEvent('DoLongHudText', src, "There are no parked vehicles in this garage.", 2)
        end
    end)
end)


RPC.register("qb-garages:attempt:sv",function(pSource,pdata)
	local src = source
	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
		...
		for ind, value in pairs(result) do
			TriggerClientEvent('qb-context:sendMenu', src, {
				...
				},
				{
					...
					params = {
						event = "qb-garages:attempt:spawn",
						args = result
					}
				},
				{
...
RPC.register("qb-garages:attempt:sv2",function(pSource,pdata)
	local src = source
	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
		...
		for ind, value in pairs(result) do
			TriggerClientEvent('qb-context:sendMenu', src, {
				...
				},
				{
					...
					txt = 'Spawn Selected Vehicle',
					params = {
						event = "qb-garages:attempt:spawn2",
						args = result
					}
				},
				{
...
RPC.register("qb-garages:spawned:get",function(pSource,valid)
	local src = source
	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE cid = @cid AND id = @id', { ['@cid'] = char.id, ['@id'] = valid.param }, function(result)
		if result[1].vehicle_state == "In" then
			TriggerClientEvent("qb-garages:attempt:spawn", src, result, true)
		else
...
RPC.register("qb-garages:spawned:getShared",function(pSource,valid)
	local src = source
	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE id = @id', { ['@id'] = valid.param }, function(result)
		if result[1].vehicle_state == "In" then
			TriggerClientEvent("qb-garages:attempt:spawn2", src, result, true)
		else
			TriggerClientEvent('DoLongHudText', src, 'Vehicle is not in the garage. Check in your phone maybe.', 2)
		end
    end)
...
RPC.register("qb-garages:getonlyimpound",function(pSource)
	local src = source
	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
