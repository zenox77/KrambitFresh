RPC.register("qb-gopros:addSecCamera", function(pSource, pName, pCoords, pRotation)
...
RPC.register("qb-gopros:getSecurityCameraById", function(pSource, pCameraID)
    local result = MySQL.query.await([[
...
RPC.register("qb-gopros:getSecurityCameraByStateID", function(pSource, pCid)
...
RPC.register("qb-gopros:getSecurityCameraAccessByCameraID", function(pSource, pCamera)
...
RPC.register("qb-gopros:addUserToCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})

    print(pCid)
    local pCid = exports['qb-phone']:getCIDFromPhoneNumber(pCid)
    print(pCid)
...
RPC.register("qb-gopros:removeUserFromCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})
