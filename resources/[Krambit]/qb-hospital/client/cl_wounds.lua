...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('qb-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('qb-hospital:client:SyncBleed')
AddEventHandler('qb-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('qb-hospital:client:FieldTreatLimbs')
AddEventHandler('qb-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('qb-hospital:client:FieldTreatBleed')
AddEventHandler('qb-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('qb-hospital:client:ReduceBleed')
AddEventHandler('qb-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('qb-hospital:client:ResetLimbs')
AddEventHandler('qb-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('qb-hospital:client:RemoveBleed')
AddEventHandler('qb-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
AddEventHandler('baseevents:onPlayerKilled', function(killedBy, data)
    TriggerEvent('qb-hospital:client:ResetLimbs')
    TriggerEvent('qb-hospital:client:RemoveBleed')
end)
