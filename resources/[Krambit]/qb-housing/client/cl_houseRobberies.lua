...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["qb-taskbar"]:taskBar(math.random(5000, 10000), "Breaking in")
...
function alarm(clearAlarm)
    if Housing.alarm then
        local alarmOffset = exports["qb-build"]:getModule("func").getAlarm()
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["qb-textui"]:showInteraction("[E] Take Object")
        else
            exports["qb-textui"]:showInteraction("[E] Look for some belongings.")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["qb-inventory"]:getCurrentWeight()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["qb-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["qb-taskbar"]:taskBar(15000,"Procurando")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("qb-heists:complete", math.random(30, 50))
...
RegisterNetEvent("qb-weathersync:currentTime", function(pHour, pMinute)
...
RegisterNetEvent("qb-housing:alarmFail")
AddEventHandler("qb-housing:alarmFail", function()
...
RegisterNetEvent("qb-housing:alarmEnter")
AddEventHandler("qb-housing:alarmEnter", function()
    insideGame = true
    local successevrp
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("qb-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("qb-housing:alarm")
AddEventHandler("qb-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["qb-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("qb-housing:beingRobbed", function(pPropertyId)
