...
function canRent()
    ...
    if isComplete and dist <= 3.0 then
        ...
        local canRent = RPC.execute("qb-housing:canRent", propertyId)
...
AddEventHandler("property:menuAction", function(pData)
    ...
    if isComplete and dist <= 3.0 then
        ...
        elseif action == "forfeit" then
            ...
            exports["qb-interface"]:showContextMenu(MenuData["property_check"])
        end
    else
...
AddEventHandler("qb-housing:handler", function(data, cb)
...
Citizen.CreateThread(function()
    exports["qb-keybinds"]:registerKeyMapping("","Housing", "Interact", "+propertyInteract", "-propertyInteract", "E")
