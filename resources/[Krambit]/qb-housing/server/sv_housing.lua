...
GetUserByCid = function(cid)
    for index, value in pairs(GetPlayers()) do
        local xPlayer = exports['qb-base']:getModule("Player"):GetUser(tonumber(value))
...
RPC.register("qb-housing:isThisPropertyOwned", function(src, propertyId)
    local result = MySQL.query.await([[
...
RegisterNetEvent("qb-housing:enterHouse")
AddEventHandler("qb-housing:enterHouse", function(pPropertyId)
    local src = source
    Housing.Players[src] = pPropertyId
end)

RegisterNetEvent("qb-housing:leftHouse")
AddEventHandler("qb-housing:leftHouse", function(pPropertyId)
    local src = source
...
RPC.register("qb-phone:getHouseKeys", function(src, pHouseId)
    local user = exports['qb-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return {} end
...
RPC.register("qb-phone:giveKey", function(src, pHouseId, pPlayerId)
    local user = exports['qb-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false, "ID não encontrado" end

    local user2 = exports['qb-base']:getModule("Player"):GetUser(pPlayerId)
    ...
    TriggerClientEvent("qb-phone:notification", pPlayerId, "fas fa-key", "Keys", "Você recebeu a chave da propriedade " .. Housing.info[pHouseId]["street"], 5000)
    TriggerClientEvent("qb-housing:refresh", pPlayerId)

    return true, "Chave recebida"
end)

RPC.register("qb-phone:removeKey", function(src, pHouseId, pPlayerId)
    local user = exports['qb-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
...
RPC.register("qb-phone:payHouse", function(src, pHouseId)
  ...
  --  local tax = price
   ...
   -- local accountId = exports["qb-base"]:getChar(src, "bankid")
   -- local bank = exports["qb-financials"]:getBalance(accountId)
   -- if bank < priceTotal then
   --     return false, "Você não tem $" .. priceTotal .. " em sua conta"
    --end

    --local groupBank = exports["qb-groups"]:groupBank("real_estate")

    --local comment = "Pagamento de aluguel da propriedade " .. Housing.info[pHouseId]["street"]
    --local success, message = exports["qb-financials"]:transaction(accountId, groupBank, price, comment, cid, 5)
    --if not success then
      --  return false, message
    --end

    --exports["qb-financials"]:addTax("Propertys", tax.tax)
...
RPC.register("qb-phone:removeSharedKey", function(src, pHouseId)
    local user = exports['qb-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false end
    ...
    if affectedRows and affectedRows > 0 then
        TriggerClientEvent("qb-housing:refresh", src)
        return true
    end
...
RegisterServerEvent('qb-housing:buyLog')
AddEventHandler('qb-housing:buyLog', function(pHouse, price)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** FM | Nopixel [Housing] | Purchase **",
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE", function(err, text, headers) end, 'POST', json.encode({username = "np | House Bought Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)