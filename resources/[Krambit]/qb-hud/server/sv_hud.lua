...
AddEventHandler('carfill:checkmoney', function(costs)
	...
	if exports['qb-base']:getModule('GetCash')(src) >= costs then
		TriggerClientEvent("RefuelCarServerReturn", src)
		exports['qb-base']:getModule('RemoveCash')(src, costs)
	else
		moneyleft = costs - exports['qb-base']:getModule('GetCash')(src)
		TriggerClientEvent('DoLongHudText', src, "Requires $" .. costs)
	end
end)

RegisterServerEvent('qb-hud:UpdateStress_SV')
AddEventHandler('qb-hud:UpdateStress_SV',function(amount)
	local src = source
