...
function StartPeekin()
    ...
    UpdateRequired = true
    NearZones = {}
    nearbyZones = exports["qb-polytarget"]:GetZones(GetEntityCoords(PlayerPedId())) or {}
...
RegisterNetEvent("qb-target:changed", function(pTarget, pEntityType, pEntityOffset)
...
RegisterNetEvent('qb-polyzone:enter', function(zoneName, zoneData, zoneCenter)
...
RegisterNetEvent('qb-polyzone:exit', function(zoneName)
    if not PeekEntries['polytarget'][zoneName] then return end
...
AddEventHandler("qb-interact:doPettingAnimal", function()
  ...
  if lastStressTime == 0 or lastStressTime + (60000 * 15) < GetGameTimer() then
    ...
    TriggerEvent("client:newStress", false, 15)
  end
end)