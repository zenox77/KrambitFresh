...
function GenerateBurnerPhoneNumber () {
    let length = 12
    let phoneNumber = "796"
    let number;
    for (let i = 1 ; i < length ; i++) {
            let nGen = Math.floor(Math.random() * 6) + 1
            if (nGen != number) {
                if (length == 3) {
...
RegisterServerEvent("qb-inventory:exploiterKick")
onNet("qb-inventory:exploiterKick", async() => {
    // let connect = {
    //     {
    //         ["color"] = color,
    //         ["title"] = "** FM | Nopixel [Inventory] | Exploiter **",
    //         ["description"] = "Steam Name: "..source.." Was kicked for attempting to use an inventory exploit.",
    //     }
    // },
    // PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE-", function(color, title, headers){ end }, 'POST', json.encode({username = "np | Exploiter Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] : 'application/json' })
});
...
onNet("server-inventory-open", async(coords, player, secondInventory, targetName, itemToDropArray, sauce) => {
    ...
    exports.ghmattimysql.execute(string, {}, function(inventory) {
        ...
        if (secondInventory == "1") {
            ...
            var shopAmount = 3;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "42070") {
            var targetinvname = targetName;
            var shopArray = wineryCraftBottle();
            var shopAmount = 2;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
...
function removecash(src, amount) {
    emit('qb-inventory:cash:remove', src, amount)
}
