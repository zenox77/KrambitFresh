...
function getQuantity(pServerId, pItemId)
    local user, amount = exports["qb-base"]:getModule("Player"):GetUser(pServerId), 0
    ...
    if user then
        ...
        if checkQuality then
            decayDate = exports['qb-inventory']:GetDecayDate(pItemId)
...
function removeItem(pServerId, pItemId, pAmount, pShouldNotify)
    local user = exports["qb-base"]:getModule("Player"):GetUser(pServerId)
