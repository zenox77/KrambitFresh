...
RegisterServerEvent('qb-jacksons_autos:CarTable')
AddEventHandler('qb-jacksons_autos:CarTable', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('qb-jacksons_autos:ReturnCarTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('qb-jacksons_autos:FinaceEnabledSV')
AddEventHandler('qb-jacksons_autos:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qb-jacksons_autos:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('qb-jacksons_autos:BuyEnabledSV')
AddEventHandler('qb-jacksons_autos:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qb-jacksons_autos:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('qb-jacksons_autos:RequestCarTable')
AddEventHandler('qb-jacksons_autos:RequestCarTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('qb-jacksons_autos:ReturnCarTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('qb-jacksons_autos:ChechMoney')
AddEventHandler('qb-jacksons_autos:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('qb-jacksons_autos:FailedPurchase', src)
    end
end)

RegisterServerEvent('qb-jacksons_autos:BuyVehicle')
AddEventHandler('qb-jacksons_autos:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
