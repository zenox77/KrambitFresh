...
function DPX.Jobs.CanBecomeJob(self, user, job, callback)
    ...
    if not hexId or not characterId or not src then callback(false, "Id's don't exist") return end
        ...
        TriggerEvent("qb-jobmanager:attemptBecomeJob", src, characterId, function(allowed, reason)
...
function DPX.Jobs.SetJob(self, user, job, notify, callback)
    ...
    DPX.Jobs:CanBecomeJob(user, job, function(allowed, reason, rank) 
        ...
        TriggerClientEvent("qb-jobmanager:playerBecameJob", src, job, name, false)
        TriggerClientEvent("qb-jobmanager:playerBecomeEvent", src, job, name, notify)
...
AddEventHandler("qb-base:characterLoaded", function(user, char)
...
AddEventHandler("qb-base:exportsReady", function()
    exports["qb-base"]:addModule("JobManager", DPX.Jobs)
end)
                ...
                for src,data in pairs(tbl) do
                    ...
                    if user then
                        ...
                        if tonumber(curTime) == tonumber(data.lastPayCheck) or tonumber(data.lastPayCheck) >= 480 then
                            ...
                            --exports["qb-log"]:AddLog("Job Pay", user, "User recieved paycheck, amount: " .. tostring(payCheck))
                        else
...
RPC.register("qb-jobmanager:jobCount", function(pSource, pJob)
