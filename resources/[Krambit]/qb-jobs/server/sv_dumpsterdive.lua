...
RegisterServerEvent('np:startDumpsterTimer')
AddEventHandler('np:startDumpsterTimer', function(dumpster)
    startTimer(source, dumpster)
end)

RegisterServerEvent('np:giveDumpsterReward')
AddEventHandler('np:giveDumpsterReward', function()
...
function startTimer(id, object)
    ...
    while timer > 0 do
        ...
        timer = timer - 1000
        if timer == 0 then
            TriggerClientEvent('np:removeDumpster', id, object)
        end
    end
end