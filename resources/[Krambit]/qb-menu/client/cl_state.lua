...
AddEventHandler('qb-polyzone:entered:garages', polyZoneEnter)
AddEventHandler('qb-polyzone:exited:garages', polyZoneExit)
...
RegisterNetEvent("menu:send:qb-housing:garages", function(pTable, pHid, pHeading)
        exports["qb-polyzone"]:AddBoxZone("housingGarage-"..pHid, pTable, 10, 10, {
            heading = pHeading,
            minZ = -25,
            maxZ = 100,
          })
...
AddEventHandler('qb-polyzone:enter', function(name)
    ...
    for k, v in pairs (sharedGarages) do
        if name == v then
            local garageData = exports["qb-garages"]:getGarageData(name)
