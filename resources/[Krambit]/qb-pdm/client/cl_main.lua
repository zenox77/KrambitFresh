...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("qb-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("qb-pdm:testdrive")
AddEventHandler("qb-pdm:testdrive", function()
	if exports["qb-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qb-pdm:EnableBuy")
AddEventHandler("qb-pdm:EnableBuy", function()
	if #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["qb-business"]:IsEmployedAt("pdm") then
		return
	end
	...
	TriggerServerEvent("qb-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("qb-pdm:BuyEnabledCL")
AddEventHandler("qb-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("qb-pdm:PDMCommission")
AddEventHandler("qb-pdm:PDMCommission", function(newAmount)
	if exports["qb-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qb-pdm:ReturnPDMTTable")
AddEventHandler("qb-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('qb-pdm:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["qb-business"]:IsEmployedAt("pdm") then
				isNearDealerShip = true
				if buyPlate[addplate] ~= nil then
					exports['qb-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['qb-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['qb-interface']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['qb-interface']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
exports('isNearDealerShip', function()
	return isNearDealerShip 
end)
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("qb-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			local VehicleProps = exports['qb-bennys']:FetchVehProps(personalvehicle)
			...
			TriggerServerEvent('qb-pdm:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('qb-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'PDM')
			TriggerServerEvent('qb-financials:business_money', price / 10, 'pdm', 'add')
			exports['qb-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("qb-pdm:FailedPurchase")
AddEventHandler("qb-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "a-class" or this == "b-class" or this == "s-class" or this == "pdm-tuners" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('qb-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["qb-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
AddEventHandler('pdmvehshop:spawnVehicle', function(v)
	...
	if playerPed and playerPed ~= -1 then
		...
		exports['qb-vehicles']:SetVehicleIdentifier(veh, netid)
	end
end)
...
RegisterCommand('commissionpdm', function(source, args, raw)
	if exports["qb-business"]:IsEmployedAt("pdm") then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('qb-pdm:PDMCommission', amount)
		else
...
RegisterCommand('enableBuypdm', function(source, args, raw)
	if exports["qb-business"]:IsEmployedAt("pdm") then
		TriggerEvent('qb-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)

RegisterNetEvent('qb-fuel:pdmmenushit', function()
...
RegisterInterfaceCallback('qb-pdm:pdmcom', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
	exports['qb-interface']:openApplication('textbox', {
		callbackUrl = 'qb-pdm:send_pdm',
		key = 1,
		items = {
		  {
			icon = "address-card", 
...
RegisterInterfaceCallback('qb-pdm:send_pdm', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerServerEvent("qb-pdm:wtf", data.values.pPDMPayCom)
end) 



RegisterNetEvent("qb-pdm:commissionpdm") 
AddEventHandler("qb-pdm:commissionpdm", function(args)
	print("YO")
	if exports["qb-business"]:IsEmployedAt("pdm") then
		print("IM PDM WORKER")
		local amount = args
...
RegisterNetEvent("qb-pdm:testdrivepdm")
AddEventHandler("qb-pdm:testdrivepdm", function()
	if exports["qb-business"]:IsEmployedAt("pdm") then
		TriggerEvent('qb-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
...
RegisterCommand('enablebuy', function(source,args)
	TriggerEvent('qb-pdm:enableBuypdm')
end)

RegisterNetEvent("qb-pdm:enableBuypdm")
AddEventHandler("qb-pdm:enableBuypdm", function()
	if exports["qb-business"]:IsEmployedAt("pdm") then
		TriggerEvent('qb-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
