...
RegisterNetEvent("qb-phone:addToCurrentBanks", function(data)
    local addedBank = {
    ...
    RPC.execute("qb-phone:addToCurrentBanks", addedBank)
end)
...
RegisterNUICallback('getCurrentBanks', function(data, cb)
    local banks = RPC.execute('qb-phone:getCurrentBanks')
...
RegisterNUICallback('onPlayerJoinBankBusters', function(data, cb)
    RPC.execute("qb-phone:addPlayerToGroup", tonumber(data.targetCID), data.groupID, tonumber(data.senderID))
end)
...
RegisterNUICallback('addBankbusterGroup', function(data, cb)
    RPC.execute("qb-phone:requestPlayerToGroup", tonumber(data.cid), data.groupID)
end)

RegisterNUICallback('createBankbusterGroup', function(data, cb)
    RPC.execute("qb-phone:createGroup")
end)
    ...
    for k,v in pairs(currentGroups) do
        for j,s in pairs(v.members) do
            if tonumber(s.cid) == tonumber(data.cid) and s.leader then
                RPC.execute("qb-phone:deleteGroup", data.groupID)
            elseif tonumber(s.cid) == tonumber(data.cid) then
                RPC.execute("qb-phone:removePlayerFromGroup", tonumber(data.cid), data.groupID)
            end
        end
    end
end)