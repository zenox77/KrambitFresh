...
RegisterNUICallback('racing:event:start', function(data)
  local canJoinOrErr = exports["qb-racing"]:canJoinOrStartRace()
  if canJoinOrErr ~= true then
    exports["qb-phone"]:phoneNotification("racen",canJoinOrErr,"RACING", 3000)
    return
  end
  print('racing:event:start ',data.id, json.encode(data))
    exports["qb-racing"]:createPendingRace(data.id, data)
    Wait(100)
    SendNUIMessage({
...
RegisterNUICallback("racing:event:join", function(data)
  ...
  print('[racing:event:join] Alias : ',alias)
  if not cid or not alias then return end
  
  exports["qb-racing"]:joinRace(id, alias, cid)
  Wait(100)
  SendNUIMessage({
    openSection = "racing:event:update",
    canJoin = false,
...
RegisterNetEvent("qb-racing:removedActiveRaced")
AddEventHandler("qb-racing:removedActiveRaced", function(id)
    openRacingApp()
...
function openRacingApp()
  ...
  local rank = 5
  local alias = RPC.execute("GetAlias")
  local _races = exports["qb-racing"]:getAllRaces()
  ...
  print('_newRaces: ',json.encode(_newRaces))
  SendNUIMessage({
...
AddEventHandler("qb-racing:api:updatedState", function()
...
RegisterNUICallback("racing:map:save", function(data)
  if creating then
      ...
      if veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
          ...
          TriggerEvent("qb-racing:cmd:racecreate", options)
          closeGui()
...
RegisterNUICallback('racing:map:cancel', function()
  creating = false
  TriggerEvent('qb-racing:cmd:racecreatecancel')
  openRacingApp()
end)
...
RegisterNUICallback('qb-phone:racing:setGPS', function(data)
  local mapID = data.mID
  exports["qb-racing"]:locateRace(mapID)
  phoneNotification("racen","GPS Set to your map.","RACING")
end)

RegisterNUICallback('qb-phone:racing:seePreview', function(data)
  local mapID = data.mID
  if not mapID then return end
  id = tostring(mapID)
  exports["qb-racing"]:previewRace(mapID)
...
RegisterNUICallback('qb-phone:r-users', function(data, cb)
  ...
  if race ~= nil then
    ...
    local racer = RPC.execute('qb-phone:racing:getRacer', raceID)
    cb(racer)
  end
  print(json.encode(newPendingRace[raceID]))
end)

AddEventHandler("qb-racing:api:startingRace", function(startTime)
  TriggerEvent("DoLongHudText", "Starting race in " .. tostring(startTime / 1000) .. " seconds")
end)

AddEventHandler("qb-racing:api:playerJoinedYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " joined your race")
end)

AddEventHandler("qb-racing:api:playerLeftYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " left your race")
end)

RegisterNetEvent('qb-racing:pendingRacenotif')
AddEventHandler("qb-racing:pendingRacenotif", function(race)
  ...
  exports["qb-phone"]:phoneNotification("racen","Pending race available...","From the PM", 12000)
end)
-- Citizen.CreateThread(function()
--   while true do
--     Wait(5000)
