...
function manageGroups(id){
    $.post('http://qb-phone/manageGroup', JSON.stringify({
        GroupID: id
    }));
}
...
function addGroupManage(group,ranking) {
   ...
   $.post('http://qb-phone/retreiveGroupRanks', JSON.stringify({
       bID: globalGroup
   }));
...
$('#group-manage-fire-accept').click(function(e){
    ...
    setTimeout(() => {
        $.post('http://qb-phone/fireEmployee', JSON.stringify({
...
$(".group-submit-hire-rank").click(function (e) {
    ...
    complateInput();
    setTimeout(() => {
        $.post('http://qb-phone/addEmployee', JSON.stringify({
            business: business_id,
            rank: rank,
            state_id: state_id
        }));
...
$("#rankSet-submit-input").click(function (e) {
    ...
    setTimeout(() => {
        $.post('http://qb-phone/editEmployee', JSON.stringify({
            business: business_id,
            rank: rank,
            state_id: state_id
        }));
...
$(".payexternal-submit").click(function(e) {
    ...
    complateInput();
    setTimeout(() => {
        $.post('http://qb-phone/payExternal', JSON.stringify({
            pStateId: stateid,
            pAmount: amt,
...
$(".create-rank-submit-input").click(function(e) {
    ...
    complateInput();
    setTimeout(() => {
        $.post('http://qb-phone/createBusinessRole', JSON.stringify({
            pBusinessId: business_id, 
...
$(".edit-rank-submit-input").click(function(e) {
    ...
    complateInput();
    setTimeout(() => {
        $.post('http://qb-phone/editBusinessRole', JSON.stringify({
            pBusinessId:      business_id, 
            pCurRole:         currole, 
            pRoleName:        rolename, 
            pPermFire:        fire, 
...
$(".delete-rank-submit-input").click(function(e) {
    ...
    complateInput();
    setTimeout(() => {
        $.post('http://qb-phone/deleteRole', JSON.stringify({
            pBusinessID: business_id,
            pRole: rolename
        }))
        setTimeout(() => {
...
$(".btn-d-logs").click(function(e){
    $('.option-main').fadeOut()
    var business_id = $("#business-id").val();
    $.post('http://qb-phone/openBusinessLogs', JSON.stringify({
        business: business_id
    }))
})
...
$("#paycheck-submit-input").click(function (e) {
    ...
    // console.log("SUBMIT SHIT")
    setTimeout(() => {
        $.post('http://qb-phone/payCheckEmployee', JSON.stringify({
            business: business_id,
            amount: amount,
            state_id: state_id
        }));
...
$("#add-cash-submit-input").click(function(e){
    ...
    $('.group-manage-add-cash').fadeOut()
    complateInput();
    $.post('http://qb-phone/addCashBank', JSON.stringify({
            bid: bId,
            amount: amount,
    }));
    setTimeout(() => {
...
$(".groupbill-submit").click(function(e) {
    ...
    $.post('http://qb-phone/chargeCustomer', JSON.stringify({
        GroupID: business_id,
        stateID: stateID,
        amount: amount,
        reason: comment,
