...
function createPendingRace(id, options)
    ...
    RPC.execute("qb-racing:createPendingRace", id, options)
end
...
function startRace(countdown)
    ...
    for k, v in pairs(pendingRaces) do
        ...
        if v.owner == characterId then
            ...
            RPC.execute("qb-racing:startRace", v.id, countdown or v.countdown, v.owner)
            return
        end
    end
end
...
function endRace()
    ...
    if curRace then
        RPC.execute("qb-racing:endRace")
        print("ENDING RACE 1")
    else
        RPC.execute("qb-racing:leaveRace")
    end
end

function joinRace(id, alias, characterId)
    RPC.execute("qb-racing:joinRace", id, alias, characterId)
end
...
function leaveRace()
    ...
    if curRace then
        RPC.execute("qb-racing:dnfRace", curRace.id)
        cleanupRace()
    else
        RPC.execute("qb-racing:leaveRace")
    end
...
function getAllRaces()
    ...
    local res = RPC.execute("qb-racing:getAllRaces")
    races = res.races
...
function getHasRaceUsbAndAlias()
    ...
    local pdRacingUsbItem = exports["qb-inventory"]:GetInfoForFirstItemOfName("racingusb3")
