...
function joinRace(src, id, alias, cid, dontpay)
    ...
    TriggerClientEvent("qb-racing:joinedRace", src, Races[id])
    TriggerClientEvent("qb-racing:addedPendingRace", -1, PendingRaces[id])

    for k, v in pairs(PendingRaces[id]["players"]) do
        TriggerClientEvent("qb-racing:playerJoinedYourRace", v.serverId, cid, alias)
    end
end
...
RegisterNetEvent("qb-racing:updatePositionInfo")
AddEventHandler("qb-racing:updatePositionInfo", function(curRaceid, totalDist)
    local src = source
    local srcString = tostring(src)
    ...
    TriggerClientEvent("qb-racing:updatePosition", src, position)
end)
...
RPC.register("qb-racing:createPendingRace", function(src, ids, optionss)
    ...
    TriggerClientEvent("qb-racing:addedPendingRace", -1, options)
    TriggerClientEvent('qb-racing:pendingRacenotif', -1)
    print("GETTING TO JOIN AFTER CREATING RACE",options)
    joinRace(src, id, alias, cid, true)
end)

RPC.register("qb-racing:joinRace", function(src, ids, aliass, cids)
    local id = ids
    local alias = aliass
    local cid = cids
...
RPC.register("qb-racing:leaveRace", function(src)
    local srcString = tostring(src)
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if cid == PendingRaces[raceId]["owner"] then
        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("qb-racing:endRace", v.serverId)
        end

        PendingRaces[raceId] = nil
        TriggerClientEvent("qb-racing:removedPendingRace", -1, raceId)
    else
        PendingRaces[raceId]["players"][srcString] = nil
        TriggerClientEvent("qb-racing:leftRace", src)
        TriggerClientEvent("qb-racing:addedPendingRace", -1, PendingRaces[raceId])

        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("qb-racing:playerLeftYourRace", v.serverId, cid, alias)
        end
    end
end)

RPC.register("qb-racing:dnfRace", function(src, ids)
    local id = ids
    local srcString = tostring(src)
    ...
    TriggerClientEvent("qb-racing:leftRace", src)
    TriggerClientEvent("qb-racing:updatedActiveRace", -1, ActiveRaces[id])
    ...
    if ActiveRaces[id]["positions"] > countPlayers then
        ...
        TriggerClientEvent("qb-racing:raceHistory", -1, ActiveRaces[id])
        ...
        ActiveRaces[id] = nil
        TriggerClientEvent("qb-racing:removedActiveRace", -1, id)
    end
end)

RPC.register("qb-racing:startRace", function(src, ids, countdowns, ownerID)
    ...
    TriggerClientEvent("qb-racing:addedActiveRace", -1, ActiveRaces[id])

    PendingRaces[id] = nil
    TriggerClientEvent("qb-racing:removedPendingRace", -1, id)
    ...
    for k, v in pairs(ActiveRaces[id]["players"]) do
        ...
        if isStartEnabled then
            TriggerClientEvent("qb-racing:startRace", v.serverId, race, countdown * 1000)
            -- TriggerClientEvent("qb-racing:api:currentRace", v.serverId, race)
            TriggerClientEvent("qb-racing:api:currentRace", race)
        else
            local user = exports['qb-base']:getModule("Player"):GetUser(v.serverId)
            local char = user:getCurrentCharacter()
...
RPC.register("qb-racing:endRace", function(src)
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    ...
    TriggerClientEvent("qb-racing:removedActiveRace", -1, raceId)
end)

RPC.register("qb-racing:finishedRace", function(src, curRaceids, bestLapTimes, totalTimes)
    ...
    TriggerClientEvent("qb-racing:finishedRace", src, ActiveRaces[curRaceid]["players"][srcString]["position"], totalTime, ActiveRaces[curRaceid]["buyIn"])
    TriggerClientEvent("qb-racing:updatedActiveRace", -1, ActiveRaces[curRaceid])
    ...
    if ActiveRaces[curRaceid]["players"][srcString]["position"] == countPlayers then
        ...
        TriggerClientEvent("qb-racing:raceHistory", -1, ActiveRaces[curRaceid])
        ...
        TriggerClientEvent("qb-racing:removedActiveRace", -1, curRaceid)
    elseif ActiveRaces[curRaceid]["dnfPosition"] > 0 and ActiveRaces[curRaceid]["dnfPosition"] == ActiveRaces[curRaceid]["players"][srcString]["position"] then
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            TriggerClientEvent("qb-racing:startDNFCountdown", v.serverId, countdown)
        end
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            if v.position == 0 then
                TriggerClientEvent("qb-racing:dnfRace", v.serverId)
            end
        end
