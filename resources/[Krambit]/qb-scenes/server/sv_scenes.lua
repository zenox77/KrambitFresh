...
RegisterNetEvent("qb-scenes:getScenes")
AddEventHandler("qb-scenes:getScenes", function()
    local src = source
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))

    if not data then
        TriggerClientEvent("qb-scenes:allScenes", src, {})
        return
    end
    ...
    TriggerClientEvent("qb-scenes:allScenes", src, scenes)
end)

RegisterNetEvent("qb-scenes:addScene")
AddEventHandler("qb-scenes:addScene", function(pData)
    local src = source
    ...
    TriggerClientEvent("qb-scenes:refreshScenes", -1, data[1].id, data[1])
end)

RegisterNetEvent("qb-scenes:deleteScene")
AddEventHandler("qb-scenes:deleteScene", function(pCoords)
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))
    for k,v in pairs(data) do
        ...
        if #(acCoord - pCoords) < 2.0 then
            ...
            if delete then
                TriggerClientEvent("qb-scenes:deleteScene", -1, v.id)
                return
            end
        end
    end
