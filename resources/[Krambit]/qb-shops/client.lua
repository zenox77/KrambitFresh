		...
		Citizen.Wait(1)
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
				...
				if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
					...
					local finished = exports["qb-taskbar"]:taskBar(60000,"Searching...")
...
RegisterNetEvent('qb-shops:exchangeGold')
AddEventHandler('qb-shops:exchangeGold', function()
	if exports["qb-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		local finished = exports["qb-taskbar"]:taskBar(120000,"Trading Bars")
		if finished == 100 then
			if exports["qb-inventory"]:hasEnoughOfItem("goldbar",70,true) then
				TriggerServerEvent('qb-banking:addMoney', 35000)
...
AddEventHandler('DragonJewelCraft', function()
	local isEmployed = exports["qb-business"]:IsEmployedAt("JewelDragon")
    if isEmployed then
...
AddEventHandler('slushy:general', function()
	...
	local finished = exports ["qb-ui"]:taskBarSkill(18000,math.random(10,20))
	if (finished == 100) then
		ExecuteCommand("e handshake")
		local finished = exports ["qb-ui"]:taskBarSkill(18000,math.random(10,20))
		if (finished == 100) then
...
AddEventHandler('lockpickshit:general', function()
	local finished = exports["qb-taskbar"]:taskBar(60000,"What dis????")
	if (finished == 100) then
...
AddEventHandler('harmony:stash', function()
	local isEmployed = exports["qb-business"]:IsEmployedAt("harmony_repairs")
	if isEmployed then
...
AddEventHandler("illegal:crafting", function()
	...
	local finished = exports['qb-taskbar']:taskBar(12000, 'Inspecting Table')
	if (finished == 100) then
...
-- 	local finished = exports['qb-taskbar']:taskBar(20000, 'Smelting')
-- 	if (finished == 100) then
...
RegisterNetEvent("qb-shops:redCounter")
AddEventHandler("qb-shops:redCounter", function()
    TriggerEvent("server-inventory-open", "1", "redcircle_counter");
end)

RegisterNetEvent("qb-shops:redCounter2")
AddEventHandler("qb-shops:redCounter2", function()
...
AddEventHandler('weapon:general', function()
	TriggerServerEvent('qb-shops:WeaponShopStatus')
end)
...
AddEventHandler("hunting:level1", function()
	if exports["qb-inventory"]:getQuantity("coyotepelt") >= 1 then
		playerAnim()
		local finished = exports["qb-taskbar"]:taskBar(4000,"Selling Coyotepelt",true,false,playerVeh)
		if finished == 100 then
			if exports["qb-inventory"]:getQuantity("coyotepelt") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'coyotepelt', 1)
				TriggerServerEvent('qb-banking:addMoney', math.random(80, 150))
			else
...
AddEventHandler("hunting:level2", function()
	if exports["qb-inventory"]:getQuantity("buck") >= 1 then
		playerAnim()
		local finished = exports["qb-taskbar"]:taskBar(4000,"Selling Buck",true,false,playerVeh)
		if finished == 100 then
			if exports["qb-inventory"]:getQuantity("buck") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'buck', 1)
				TriggerServerEvent('qb-banking:addMoney', math.random(160, 200))
			else
...
AddEventHandler("hunting:level3", function()
	if exports["qb-inventory"]:getQuantity("boar") >= 1 then
		playerAnim()
		local finished = exports["qb-taskbar"]:taskBar(4000,"Selling Boar",true,false,playerVeh)
		if finished == 100 then
			if exports["qb-inventory"]:getQuantity("boar") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'boar', 1)
				TriggerServerEvent('qb-banking:addMoney', math.random(350, 500))
			else
...
--     exports['qb-menuinteract']:AddButton("Sell Coyotepelt", "Wow you suck", "hunting:level1", 'example button 1', "jobmenu")
--     exports['qb-menuinteract']:AddButton("Sell Buck", "Wow you kinda okay", "hunting:level2", 'example button 2', "jobmenu")
--     exports['qb-menuinteract']:AddButton("Sell Boar", "Wow you good", "hunting:level3", 'example button 3', "jobmenu")
--     exports['qb-menuinteract']:SubMenu("Hunting Menu" , "Click to select sell!" , "jobmenu")
-- end
...
Citizen.CreateThread(function()
	while true do
		...
		SetDiscordAppId(1000933339080966248)
		SetDiscordRichPresenceAsset("Nopixel Replica")
		SetDiscordRichPresenceAssetText('Nopixel Replica')
		SetDiscordRichPresenceAction(0, 'Buy Here :', 'https://discord.gg/a8Nau87Xum')
	end
end)