...
RegisterInterfaceCallback("qb-signin:handler", function(data, cb)
  ...
  if eventData.sign_in then
    if eventData.job == "police" then
      ...
      TriggerServerEvent("qb-signin:duty", eventData.job)
    end
...
RegisterInterfaceCallback("qb-signin:spawnVehicle", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local eventData = data.key
...
AddEventHandler("qb-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  print('[qb-signin:peekAction] MenuData:' ..' ' ,name)
  exports["qb-interface"]:showContextMenu(MenuData[name])
end)

AddEventHandler('qb-island:hideBlips', function(pState)
