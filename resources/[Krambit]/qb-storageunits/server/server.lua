RPC.register("qb-storageunits:get", function()
    return storage_units
end)


RegisterServerEvent("qb-storageunits:purchase", function(args)
    local pSrc = source
...
RPC.register("qb-storageunits:occupied", function(storage_unit_id)
    local pSrc = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(pSrc)
        ...
        if result[1] ~= nil then
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "qb-storageunits:attempt:inventory",
                        args = {
                            id = storage_unit_id,
                        }
                    }
            ...
            
                {
                    ...
                    params = {
                        event = "qb-storageunits:attempt:access",
                        args = {
                            id = storage_unit_id,
            ...
            if tonumber(storage_owner) == char_id then
                TriggerClientEvent('qb-context:sendMenu', pSrc, menu_data)  
            else
                for i=1, #keys do
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "qb-storageunits:attempt:purchase",
                        args = {
                            id = storage_unit_id,
                            price = 70000
                        }
...
RPC.register("qb-storageunits:give:access", function(storage_unit_id, given_cid)
    local pSrc = source
...
RPC.register("qb-storageunits:remove:access", function(storage_unit_id, given_cid, given_name)
    local pSrc = source
...
RegisterServerEvent("qb-storageunit:view", function(storage_unit_id)
    local pSrc = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(pSrc)
    exports["ghmattimysql"]:execute("SELECT `given_access` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        if result[1].given_access ~= nil then
            ...
            for i=1, #current_keys do  
                exports["ghmattimysql"]:execute("SELECT `first_name`, `last_name`, `id` FROM characters WHERE id = @id", {['id'] = current_keys[i]}, function(result)
                    if result[1] ~= nil then
                        for i=1, #result do
                            TriggerClientEvent('qb-context:sendMenu', pSrc, {
                                {
                                    id = result[i].id,
                                    header = "Name: " ..result[i].first_name .. " " .. result[i].last_name,
                                    txt = "CID: " ..result[i].id,
                                    params = {
                                        event = "qb-storageunit:attempt:remove",
                                        args = {
                                            id = storage_unit_id,
...
RPC.register("qb-storageunits:attenpt:transfer", function(storage_unit_id, transfer_cid)
    local pSrc = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
