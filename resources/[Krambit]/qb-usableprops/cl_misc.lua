...
local function benchObjectHandler(pBenchType, pItem)
    ...
    local id = exports['qb-objects']:PlaceAndSaveObject(
...
local itemHandlers = {
    ...
    ['pailshovel'] = function(info)
        ...
        local finished = exports["qb-taskbar"]:taskBar(15000, "Building Sandcastle", false, true, false, false, nil, 5.0, ped)
        ClearPedTasks(ped)
    ...
    ['letterbox'] = function(info)
        ...
        local properties = exports['qb-housing']:getHousingData()
        ...
        local name
        if elements then
            local prompt2 = exports['qb-ui']:OpenInputMenu(elements)
            if not prompt2 then
                return false
            end
            ...
            if mailType == 'property' then
                local housingTable = exports['qb-housing']:retriveHousingTable()
            ...
            elseif mailType == 'business' then
                local zones = exports['qb-polyzone']:GetZones(plyCoords)
                if not zones then
        ...
        local results = exports['qb-objects']:PlaceObject(
            model,
            {
                forceGroundSnap = true,
                zOffset = -0.05,
        ...
        local objId = RPC.execute('qb-usableprops:placeLetterbox', model, results[2].coords, results[2].rotation, mailType, foundInfo, name)
        if objId then
...
local function isPD()
    local job = exports["qb-base"]:getModule("LocalPlayer"):getVar("job")
...
-- Citizen.CreateThread(function()
--     exports["qb-interact"]:AddPeekEntryByModel({ `prop_sign_road_01a` }, {{
--         event = "qb-usableprops:stealStopSign",
--         id = "qb-usableprops:stealStopSign",
--         icon = "hammer",
--         label = "Steal!",
--         parameters = {},
--     }}, {
...
--     exports["qb-interact"]:AddPeekEntryByModel({ 2108567945, -1940238623 }, {{
--         event = "qb-usableprops:payParkingMeter",
--         id = "qb-usableprops:parking_pay",
--         icon = "dollar-sign",
...
--     exports["qb-interact"]:AddPeekEntryByModel({ `prop_skid_tent_01`, `prop_skid_tent_03` }, {{
--         event = "qb-usableprops:pickupTent",
--         id = "qb-usableprops:pickup_tent",
--         icon = "arrow-up",
--         label = "pickup tent",
--         parameters = {},
--     }}, {
--         distance = { radius = 2.0 },
--         isEnabled = function(pEntity)
--           return exports['qb-objects']:GetObjectByEntity(pEntity) ~= nil
--         end,
--     })

--     exports["qb-interact"]:AddPeekEntryByModel({ `prop_bench_04`, `prop_ld_bench01`, `prop_snow_bench_01` }, {{
--         event = "qb-usableprops:pickupBench",
--         id = "qb-usableprops:pickup_bench",
--         icon = "arrow-up",
--         label = "pickup bench",
--         parameters = {},
--     }}, {
...
--     exports["qb-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_letterbox_04`, `prop_postbox_01a` }, {{
--         event = "qb-usableprops:openMail",
--         id = "qb-usableprops:open_mailbox",
--         icon = "envelope",
--         label = "deposit mail",
--         parameters = {},
--     }}, {
...
--     exports["qb-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_postbox_01a` }, {{
--         event = "qb-usableprops:checkMail",
--         id = "qb-usableprops:check_mailbox",
--         icon = "envelope-open",
--         label = "check mail",
--         parameters = {},
--     }}, {
...
--             return (mailType == 'business' and (exports['qb-business']:HasPermission(info, 'stash_access') or isPD())) or
--             (mailType == 'property' and exports['qb-housing']:hasPropertyAccess(info, true)) or
--             (mailType == 'mrpd' and isPD()) or
--             (mailType == 'pillbox' and (job == 'ems' or job == 'doctor'))
--         end,
--     })

--     exports["qb-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03` }, {{
--         event = "qb-usableprops:removeMailbox",
--         id = "qb-usableprops:remove_mailbox",
--         icon = "trash-alt",
...
--         isEnabled = function(pEntity)
--             local info, mailType = getMailboxInfo(pEntity)
--             return mailType == 'business' and (exports['qb-business']:HasPermission(info, 'stash_access')) or
--             (mailType == 'property' and exports['qb-housing']:hasPropertyAccess(info, false))
--         end,
--     })

-- end)

AddEventHandler("qb-usableprops:stealStopSign", function(pArgs, pEntity, pContext)
    ...
    local finished = exports["qb-taskbar"]:taskBar(15000, "Stealing", false, true, false, false, nil, 5.0, ped)
    ClearPedTasks(ped)
    if finished == 100 then
...
RegisterNetEvent("qb-usableprops:deleteEntity", DeleteEntityAtCoords)
...
AddEventHandler("qb-usableprops:payParkingMeter", function(args, entity, context)
    PaidMeters[entity] = true
    local success = RPC.execute('qb-usableprops:payParkingMeter', args.price)
    if success then
        local pos = GetEntityCoords(entity)
        TriggerServerEvent("qb-scenes:addScene", {
...
AddEventHandler("qb-usableprops:pickupTent", function(args, entity, context)
    local tent = exports['qb-objects']:GetObjectByEntity(entity)
    if tent then
        local success = exports['qb-objects']:DeleteObject(tent.id)
        if success then
...
AddEventHandler("qb-usableprops:pickupBench", function(args, entity, context)
    local bench = exports['qb-objects']:GetObjectByEntity(entity)
    if bench then
        local success = exports['qb-objects']:DeleteObject(bench.id)
        if success then
...
local closeData = {}
AddEventHandler("qb-usableprops:openMail", function(args, entity, context)
    local info, mailType, object = getMailboxInfo(entity)
    waitForClose = RPC.execute('qb-usableprops:openMailbox', info, mailType, object.id)
...
-- AddEventHandler("qb-usableprops:checkMail", function(args, entity, context)
--     if isPD() and mailType == 'business' then
--         local prompt = exports['qb-ui']:OpenInputMenu({
...
--     end
--     local info, mailType, object = getMailboxInfo(entity)
--     RPC.execute('qb-usableprops:checkMailbox', info, mailType, object.id)
-- end)

AddEventHandler('qb-inventory:closed', function(pName)
    if waitForClose and pName == 'mailtemp:' .. closeData.mailType .. ':' .. closeData.objId then
        RPC.execute('qb-usableprops:closeMailbox', closeData.info, closeData.mailType, closeData.objId)
        waitForClose = false
        closeData = {}
    end
end)

AddEventHandler('qb-usableprops:removeMailbox', function(args, entity, context)
    TriggerEvent("animation:PlayAnimation","mechanic3")
    local finished = exports['qb-taskbar']:taskBar(15000, "Removing mailbox", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local obj = exports['qb-objects']:GetObjectByEntity(entity)
        if not obj then
            return
        end
        exports["qb-objects"]:DeleteObject(obj.id)
    end
end)

AddEventHandler('qb-objects:objectCreated', function(pObject, handle)
    if pObject.data.metadata.mail then
        FreezeEntityPosition(handle, false)
    end
end)

-- AddEventHandler('qb-spawn:characterSpawned', function(pCharacterId)
--     RPC.execute('qb-usableprops:checkMailAlerts', pCharacterId) -- Do later keeping here so the timeout reminds me : D
-- end)
