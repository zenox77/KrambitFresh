...
RegisterNetEvent("qb-nitro:client:placeNitro")
AddEventHandler("qb-nitro:client:placeNitro", function() 
...
RegisterNetEvent('qb-nitro:client:particlefx')
AddEventHandler('qb-nitro:client:particlefx', function (veh)	
	local exhaustNames = {
...
RegisterNetEvent('qb-nitro:client:particlefisfis')
AddEventHandler('qb-nitro:client:particlefisfis', function (type, veh)
		...
		if nitroactive and pNitrousLevel > 0 and IsPedInVehicle(PlayerPedId(), GetVehiclePedIsIn(PlayerPedId(), false), false) then
			...
			if fisfismode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- fışfış
					TriggerServerEvent('qb-nitro:server:particlefisfis', 'fisfisacc', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
				else
					pNitrousInUse = false
					TriggerServerEvent('qb-nitro:server:particlefisfis', 'fisfiskapatt', nitroveh)
				end
			elseif nitromode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- nitro
					TriggerServerEvent('qb-nitro:server:particlefx', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
...
AddEventHandler('onResourceStart', function (resource)
	pNitrousLevel = 0
	nitroactive = false
	TriggerServerEvent('qb-nitro:server:particlefisfis', 'fisfiskapatt')
	nitroveh = nil
	pNitrousInUse = false 
end)
...
Citizen.CreateThread(function()
    exports["qb-keybinds"]:registerKeyMapping("", "Nitrous", "Change Mode", "+nosMode", "-nosMode", "")

	exports["qb-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Higher", "+changeFlowPlus", "-changeFlowPlus", "")

	exports["qb-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Lower", "+changeFlowMinus", "-changeFlowMinus", "")
end)