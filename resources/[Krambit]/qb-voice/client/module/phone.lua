	...
	Citizen.CreateThread(function()
		...
		while callChannel ~= 0 do
			...
			if NetworkIsPlayerTalking(PlayerId()) and not changed then
				...
				TriggerServerEvent('qb-voice:setTalkingOnCall', true)
			elseif changed and NetworkIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false
				MumbleClearVoiceTargetPlayers(1)
				TriggerServerEvent('qb-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
RegisterNetEvent('qb-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('qb-voice:setTalkingOnCall', function(tgt, enabled)
	if tgt ~= playerServerId then
...
RegisterNetEvent('qb-voice:addPlayerToCall', function(plySource)
	callData[plySource] = false
end)

RegisterNetEvent('qb-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('qb-voice:setPlayerCall', channel)
	callChannel = channel
...
RegisterNetEvent('qb-voice:clSetPlayerCall', function(_callChannel)
	if GetConvarInt('voice_enablePhones', 1) ~= 1 then return end
