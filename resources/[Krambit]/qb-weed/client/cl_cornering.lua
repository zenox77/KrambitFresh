...
local InteractStartCorner = {
    ...
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["qb-inventory"]:hasEnoughOfItem("weedq", 1, false, true)
...
local InteractCornerVehicle = {
    ...
    options = {
        distance = { radius = 5.0 },
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["qb-inventory"]:hasEnoughOfItem("weedpackage", 1, false, true)
            return hasWeed
        end
    }
}
...
Citizen.CreateThread(function()
    exports["qb-interact"]:AddPeekEntryByEntityType(InteractCorner.group, InteractCorner.data, InteractCorner.options)
    exports["qb-interact"]:AddPeekEntryByEntityType(InteractCornerVehicle.group, InteractCornerVehicle.data, InteractCornerVehicle.options)
    exports["qb-interact"]:AddPeekEntryByEntityType(InteractStartCorner.group, InteractStartCorner.data, InteractStartCorner.options)
    exports["qb-interact"]:AddPeekEntryByEntityType(InteractStopCorner.group, InteractStopCorner.data, InteractStopCorner.options)
end)

AddEventHandler("qb-weed:startCornering", function(pContext, pEntity)
    ...
    local canCorner, message, engMessage = RPC.execute("qb-weed:startCorner", CurrentCornerCoords)
    ...
    Citizen.CreateThread(function()
        ...
        while CurrentlyCornering do
            ...
            if not foundPed then
                ...
                RPC.execute("qb-weed:cornerPed", coords, NetworkGetNetworkIdFromEntity(foundPed), NetworkGetNetworkIdFromEntity(CurrentCornerVehicle))
            end
...
AddEventHandler('qb-weed:cornerSale', function(pContext, pEntity)
    ...
    local hasBaggies = exports['qb-inventory']:hasEnoughOfItem('weedq', 1, true)
    ...
    RPC.execute('qb-weed:cornerSyncHandoff', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity))
    ...
    local didSale = RPC.execute('qb-weed:cornerSale', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), CurrentCornerZone)
    if didSale then
      PlayAmbientSpeech1(pEntity, 'Chat_State', 'Speech_Params_Force')
      local rndAmt = math.random(1, 3)
      if rndAmt == 1 and exports['qb-inventory']:hasEnoughOfItem('weedq', 1) then
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 2 and exports['qb-inventory']:hasEnoughOfItem('weedq', 3) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 3 and exports['qb-inventory']:hasEnoughOfItem('weedq', 2) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
    ...
    if didSale and math.random() < 0.05 then
      TriggerEvent('qb-mdt:drugsale')
    end
  end)

AddEventHandler("qb-weed:prepareBaggies", function(pContext, pEntity)
    if CornerConfig.RequireScales then
        local hasScale = exports["qb-inventory"]:hasEnoughOfItem("qualityscales", 1, false) or exports["qb-inventory"]:hasEnoughOfItem("smallscales", 1, false)
    ...
    local hasBaggies = exports["qb-inventory"]:hasEnoughOfItem("emptybaggies", CornerConfig.BaggiesPerBrick / 4, false)
    if not hasBaggies then
    ...
    local finished = exports["qb-taskbar"]:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
    if finished == 100 then
        local weedpackages = exports["qb-inventory"]:getItemsOfType("weedpackage", 1, true)
        if #weedpackages == 0 then return end
        local package = weedpackages[1]
        local info = json.decode(package.information)
        RPC.execute("qb-weed:prepareBaggies", info)
    end
end)

AddEventHandler("qb-weed:stopCornering", function(pContext, pEntity)
    TriggerEvent("DoLongHudText", _L('weed-corner-not-selling', 'No longer selling...'))
    stopCornering()
end)

RegisterNetEvent("qb-weed:addCorneredPed")
AddEventHandler("qb-weed:addCorneredPed", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = false
end)

RegisterNetEvent("qb-weed:cornerSyncHandoff")
AddEventHandler("qb-weed:cornerSyncHandoff", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
...
RegisterNetEvent("qb-weed:cornerPed")
AddEventHandler("qb-weed:cornerPed", function(pPed, pCornerCoords, pVehicle)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = true
    UsedPeds[ped] = true
    if NetworkHasControlOfEntity(ped) then
...
RegisterNetEvent("qb-weed:cleanCornerPeds")
AddEventHandler("qb-weed:cleanCornerPeds", function()
    UsedPeds = {}
end)
...
function stopCornering()
    ...
    RPC.execute("qb-weed:stopCorner")
end
