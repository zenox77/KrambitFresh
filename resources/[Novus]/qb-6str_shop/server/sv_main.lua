...
RegisterServerEvent('qb-6str_shop:CarTableTunerShop')
AddEventHandler('qb-6str_shop:CarTableTunerShop', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('qb-6str_shop:ReturnVehicleTunerTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('qb-6str_shop:FinaceEnabledSV')
AddEventHandler('qb-6str_shop:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qb-6str_shop:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('qb-6str_shop:BuyEnabledSV')
AddEventHandler('qb-6str_shop:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qb-6str_shop:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('qb-6str_shop:RequestTableTunerShopCars')
AddEventHandler('qb-6str_shop:RequestTableTunerShopCars', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('qb-6str_shop:ReturnVehicleTunerTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('qb-6str_shop:ChechMoney')
AddEventHandler('qb-6str_shop:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('qb-6str_shop:FailedPurchase', src)
    end
end)

RegisterServerEvent('qb-6str_shop:BuyVehicle')
AddEventHandler('qb-6str_shop:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qb-cardealer:log')
AddEventHandler('qb-cardealer:log', function(cid, model, price, plate, carDealer)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** FM | Nopixel [Car Dealer] | Car Dealership Log **",
          ["description"] = "Buyer State ID: "..cid.."\n Buyer Steam Name: "..pName.."\n Buyer Steam ID: "..identifiers[1].."\n Vehicle Model: "..model.." \n Purchase Price: $"..price.."\n Vehicle Plate: "..plate.."\n Car Dealership: "..carDealer,
        }
      }
      PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE", function(err, text, headers) end, 'POST', json.encode({username = "np", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)