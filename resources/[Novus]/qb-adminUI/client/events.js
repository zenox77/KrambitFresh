...
function toggleNUI(){
    ...
    SendNuiMessage(JSON.stringify({
        action: 'setKeyBinds',
        selectionMenu: global.exports["qb-keybinds"].getKeyMapping("+enableSelectionMenu"),
        selectionKey: global.exports["qb-keybinds"].getKeyMapping("+adminSelect")
    }))
}
function exitNUI()
{
...
on('__cfx_nui:closeMenu', (data, cb) => {
    ...
    if(data.toSelect){
        global.exports["qb-admin"].enterSelection();
    }
...
on("onResourceStart", (resourceName) => {
    if(GetCurrentResourceName() != 'qb-admin') {
      return;
    }
...
on('__cfx_nui:insideCommand', (data, cb) => {
    ...
    if(data.inside){
        ...
        emit('qb-binds:should-execute',false)
        ...
        emit('qb-binds:should-execute',true)
        if(blockingActions){
...
global.exports["qb-keybinds"].registerKeyMapping("", "zzAdmin", "Selection Menu", "+enableSelectionMenu", "-enableSelectionMenu", "")
