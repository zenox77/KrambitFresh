...
function drink()
    ClenpedSecondaryTask(PlayerPedId())
...
function RoleplayStats()
    ...
    if exports["qb-inventory"]:hasEnoughOfItem("shitlockpick",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["qb-inventory"]:hasEnoughOfItem("jailfood",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["qb-inventory"]:hasEnoughOfItem("methbag",1,false) then
        totalroleplay = totalroleplay + 15
    end

    if exports["qb-inventory"]:hasEnoughOfItem("assphone",1,false) then
        totalroleplay = totalroleplay + 15
    ...
    if exports["qb-inventory"]:hasEnoughOfItem("slushy",1,false) then
        totalroleplay = totalroleplay + 10
    end
...
function InmateHasAll()
    if exports["qb-inventory"]:hasEnoughOfItem("slushy",1,false)
        and
        exports["qb-inventory"]:hasEnoughOfItem("-1810795771",1,false)
        and
        exports["qb-inventory"]:hasEnoughOfItem("methbag",1,false)
        and
        exports["qb-inventory"]:hasEnoughOfItem("assphone",1,false)
        and
        exports["qb-inventory"]:hasEnoughOfItem("slushy",1,false)
    then
        return true
    else
        return false
...
RegisterNetEvent('qb-jail:checkTime')
AddEventHandler('qb-jail:checkTime', function()
...
RegisterNetEvent('qb-jailmenu')
AddEventHandler('qb-jailmenu', function()

	local Jailmenu = {
		{
            title = "List Lawyers",
            description = "Check how many lawyers are in the city!",
            action = "qb-jail:lawyerspog",
        },
        {
            title = "Jail Time",
            description = "Check your jail time",
            action = "qb-jail:checktime2",
        },
        {
            ...
            children = {
				{
                    title = "Yes",
                    action = "qb-jail:swapchar",
                },
				{
                    title = "No",
                    action = "qb-jail:swap_char_no",
                },
            },
        },
    }
    exports["qb-interface"]:showContextMenu(Jailmenu)
end)

RegisterInterfaceCallback('qb-jail:swap_char_no', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
end)

RegisterInterfaceCallback('qb-jail:swapchar', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('swappingCharsLoop')
end)

RegisterInterfaceCallback('qb-jail:checktime2', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qb-jail:checkTime')
end)

RegisterInterfaceCallback('qb-jail:lawyerspog', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('yellowPages:retrieveLawyersOnline')
end)

exports["qb-polytarget"]:AddCircleZone("jail_interactions", vector3(1828.76, 2579.78, 46.51), 0.3, {
    useZ = true
})

exports["qb-interact"]:AddPeekEntryByPolyTarget("jail_interactions", {
    {
        event = "qb-jailmenu",
        id = "jail_interact_3",
        icon = "circle",
