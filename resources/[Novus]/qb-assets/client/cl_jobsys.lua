...
AddEventHandler('jobssystem:current', function(cb)
  LocalPlayer = exports["qb-base"]:getModule("LocalPlayer")
  cb(LocalPlayer:getVar("job"))
end)

local PaycheckSpot, called = false, false

exports["qb-polyzone"]:AddBoxZone("paycheck_collect", vector3(241.63, 226.48, 106.29), 4, 4, {
  heading=0
})
...
AddEventHandler('qb-polyzone:enter', function(name)
  if name == "paycheck_collect" then
    PaycheckSpot = true
    exports["qb-interface"]:showInteraction("[E]Collect Paycheck")
    PaycheckFunction()
  end
end)
  ...
  Citizen.CreateThread(function()
    while PaycheckSpot do
      ...
      if IsControlJustReleased(0, 38) then
        local finished = exports["qb-taskbar"]:taskBar(2000,"Collecting Your Paycheck")
