...
function DPX.Core.ConsoleLog(self, msg, mod, ply)
	...
	if ply and tonumber(ply) then
		TriggerClientEvent("qb-base:consoleLog", ply, msg, mod)
	end
end

AddEventHandler("onResourceStart", function(resource)
	TriggerClientEvent("qb-base:waitForExports", -1)
...
RegisterNetEvent("qb-base:playerSessionStarted")
...
-- 		print("^1[qb-base] Paychecks Sent^0")
-- 	end
-- end)
	...
	exports.oxmysql:execute('SELECT `paycheck` FROM characters WHERE `id`= ?', {cid}, function(data)
		...
		if amount >= 1 then
			...
			local connect = {
				{
				  ["color"] = color,
				  ["title"] = "** FM | Nopixel [Paychecks] **",
				  ["description"] = "State ID: "..cid.." \n First Name: "..pChar.first_name.." Last Name: "..pChar.last_name.." \n Amount: $"..amount,
				}
			  }
			  PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE", function(err, text, headers) end, 'POST', json.encode({username = "np | Paycheck", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
