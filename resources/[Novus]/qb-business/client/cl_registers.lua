...
for id,register in ipairs(activeRegisters) do
        local ptId = "nopixel_business_register_" .. id
        exports["qb-polytarget"]:AddBoxZone(ptId, register.polytarget[1], register.polytarget[2], register.polytarget[3], register.polytarget[4])
        exports['qb-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "qb-business:registerPurchasePrompt",
            id = ptId .. "_purchase",
            icon = "cash-register",
            label = "make payment",
        ...
        exports['qb-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "qb-business:registerChargePrompt",
            id = ptId .. "_charge",
            icon = "credit-card",
...
AddEventHandler('qb-business:registerPurchasePrompt', function(pParameters, pEntity, pContext)
    ...
    local acceptContext = {
        ...
        },
        {
            ...
            action = "qb-business:finishPurchasePrompt",
            icon = 'credit-card',
    ...
    exports['qb-interface']:showContextMenu(acceptContext)
end)

AddEventHandler('qb-business:registerChargePrompt', function(pParameters, pEntity, pContext)
    exports['qb-interface']:openApplication('textbox', {
        callbackUrl = 'qb-interface:business:charge',
...
RegisterNetEvent('qb-business:activePurchase')
AddEventHandler("qb-business:activePurchase", function(data)
    activePurchases[data.registerId] = data
end)

--Remove at registerId pos
RegisterNetEvent('qb-business:closePurchase')
AddEventHandler("qb-business:closePurchase", function(data)
    activePurchases[data.registerId] = nil
end)

RegisterInterfaceCallback('qb-business:finishPurchasePrompt', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local success, message = RPC.execute("qb-business:completePurchase", data.key)  
    if not success then
        TriggerEvent("DoLongHudText", message, 2)
    end 
end)

RegisterInterfaceCallback("qb-interface:business:charge", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    RPC.execute("qb-business:startPurchase", {cost = cost, comment = comment, registerId = data.key.registerId, business = data.key.business})
end)