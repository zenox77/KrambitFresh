...
function openMenu()
    ...
    if myjob == "police" or myjob == 'state' or myjob == 'sheriff' or myjob == 'ems' then
        TriggerServerEvent("dispatch:get-police")
        TriggerServerEvent("dispatch:osurcam-html")
        Citizen.Wait(350)
        SetNuiFocus(true, true)
...
RegisterNetEvent("qb-dispatch:openFull")
AddEventHandler("qb-dispatch:openFull", function()
    openMenu()
end)
...
RegisterNetEvent('qb-dispatch:altercallsign;spawn')
AddEventHandler('qb-dispatch:altercallsign;spawn',function(getCallsign,pStatusType)
    exports['qb-dispatch']:callsign_command(getCallsign,pStatusType)
end)

RegisterNetEvent('qb-dispatch:beepEffect')
AddEventHandler('qb-dispatch:beepEffect', function()
    if exports['isPed']:isPed('myjob') == "police" then
        TriggerServerEvent('InteractSound_SV:PlayOnSource', '10-1314', 0.2)
    end
end)

RegisterNetEvent('qb-dispatch:stolenCar')
AddEventHandler('qb-dispatch:stolenCar', function(plate)
    local playerPed = PlayerPedId()	
    local padCoords = GetEntityCoords(playerPed)
    local alertCoord = vector3(padCoords.x, padCoords.y, padCoords.z)
...
RegisterCommand('911', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        local phone_number = exports["isPed"]:isPed("phone_number")
        TriggerEvent('animation:phonecall')

        local cancelled = exports['qb-taskbar']:taskBar(7000,'Dialing 911')
...
RegisterCommand('311', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        local phone_number = exports["isPed"]:isPed("phone_number")
        TriggerEvent('animation:phonecall')
        local cancelled = exports['qb-taskbar']:taskBar(7000,'Dialing 311')
        if cancelled == 100 then
            dispatchadd("911", "311 Call: "..msg..'', 110)
        end
    else
