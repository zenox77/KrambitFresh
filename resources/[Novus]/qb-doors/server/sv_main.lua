...
RegisterNetEvent('qb-doors:request-lock-state')
AddEventHandler('qb-doors:request-lock-state', function()
    local src = source
    TriggerClientEvent('qb-doors:initial-lock-state', src, doors)
end)

RegisterNetEvent('qb-doors:change-lock-state')
AddEventHandler('qb-doors:change-lock-state', function(pDoorId, pDoorLockState)
    if doors[pDoorId] then
        doors[pDoorId].lock = pDoorLockState
        TriggerClientEvent('qb-doors:change-lock-state', -1, pDoorId, pDoorLockState)
    end
end)
...
-- RegisterNetEvent("qb-doors:save-config")
-- AddEventHandler("qb-doors:save-config", function(pDoorData)
