...
RPC.register("BankWithdraw", function(pSource, pId, pAmount, pComment, pTransId, pDate)
    ...
    --TriggerClientEvent("qb-financials:transferUpdate", pSource, ply, newbankamount)
...
RPC.register("DoTransaction", function(pSource, pId, pAmount, pComment, pTransId, pDate, pTarget)
    ...
    if id == "personal" then
        ...
        local userSender = exports["qb-base"]:getModule("Player"):GetUser(pSource)
        ...
        local userReceiver = exports["qb-base"]:getModule("Player"):GetUser(target)
  ...
  RegisterCommand('givecash', function(source, args)
    ...
    if tonumber(args[1]) == src or tonumber(args[2]) <= 0 then
      ...
      if tonumber(args[2]) then
        ...
        PerformHttpRequest('https://discord.com/api/webhooks/ENTER_HERE', function(err, text, headers) end, 'POST', json.encode({username = "np",  avatar_url = "https://cdn.discordapp.com/attachments/XXXXXXXXXXXXXXXXXX/XXXXXXXX/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
       end
    end
  end)
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["qb-base"]:getModule("Player"):GetUser(src)
  local player = exports["qb-base"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('qb-base:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
RegisterServerEvent('qb-financials:addMoney')
AddEventHandler('qb-financials:addMoney', function(amount)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('qb-financials:removeMoney')
AddEventHandler('qb-financials:removeMoney', function(amount)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("qb-financials:cash:get")
AddEventHandler('qb-financials:cash:get', function(amount)

    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local cash = user:getCash()
    TriggerClientEvent("qb-financials:cash",cash)
end)

RegisterServerEvent("qb-financials:business_money", function(amt, account, case)
...
local Deposit_Log = "https://discord.com/api/webhooks/ENTER_HERE"

RegisterServerEvent('qb-financials:deposit_log')
AddEventHandler('qb-financials:deposit_log', function(pDepositAmt, pDepositComment)
    ...
    PerformHttpRequest(Deposit_Log, function(err, text, headers) end, 'POST', json.encode({username = "np",  avatar_url = "https://cdn.discordapp.com/attachments/ENTER_HERE/ENTER_HERE/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)

local Withdraw_Log = "https://discord.com/api/webhooks/ENTER_HERE"

RegisterServerEvent('qb-financials:withdraw_log')
AddEventHandler('qb-financials:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "16777215",
    ...
    PerformHttpRequest(Withdraw_Log, function(err, text, headers) end, 'POST', json.encode({username = "np",  avatar_url = "https://cdn.discordapp.com/attachments/ENTER_HERE/ENTER_HERE/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)