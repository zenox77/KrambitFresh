...
RegisterNetEvent('qb-heists:start_loop')
AddEventHandler('qb-heists:start_loop', function()
    pickup = true
    TriggerEvent("qb-heists:pick_cash")
    Wait(180000)
    Wait(additionalWait)
    pickup = false
end)

RegisterNetEvent('qb-heists:pick_cash')
AddEventHandler('qb-heists:pick_cash', function()
        ...
        if aDist < 2.0 and not LootedTruck then
            ...
            if IsDisabledControlJustReleased(0, 38) then
                if not LootedTruck then
                    ...
                    TriggerEvent('qb-hud:show_money')
                    local BankTruckBarLoot = exports['qb-taskbar']:taskBar(60000, 'Looting the truck')
                    if BankTruckBarLoot == 100 then
                        FreezeEntityPosition(PlayerPedId(), false)
                        TriggerEvent('qb-heists:banktruck_loot') 
                        TriggerEvent('qb-hud:hide_money') 
                        LootedTruck = true
                        Citizen.Wait(900000) 
                        LootedTruck = false     
                    else
...
RegisterNetEvent('qb-heists:banktruck_loot')
AddEventHandler('qb-heists:banktruck_loot', function()
...
RegisterNetEvent('qb-heists:spawn_peds')
AddEventHandler('qb-heists:spawn_peds', function(veh)
...
RegisterNetEvent('qb-heists:banktruck:cl')
AddEventHandler('qb-heists:banktruck:cl', function()
