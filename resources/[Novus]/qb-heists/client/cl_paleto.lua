...
RegisterNetEvent('qb-paleto:box')
AddEventHandler('qb-paleto:box', function()
    ...
    if distance < 1.5 then
        if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
            ...
            PaletoStep1()
            exports["qb-dispatch"]:dispatchadd('10-90', "Paleto Bank Robbery", "108")
...
RegisterNetEvent('qb:createpaletomoney')
AddEventHandler('qb:createpaletomoney', function(toggle)
    if toggle == true then
...
function CreateMoney()
    ...
    TriggerEvent('qb:CleanClientVaultD')
    TriggerEvent("qb:createpaletomoney" ,true)
    Citizen.Trace("Crates Spawned :  Sucess")
    Citizen.Wait(240000)
    Citizen.Wait(60000)
    TriggerEvent("qb:createpaletomoney" ,false)
end


RegisterNetEvent('qb-paleto:startpaleto')
AddEventHandler('qb-paleto:startpaleto', function()
    local player = GetEntityCoords(PlayerPedId())
...
function OnHackSuccess()
    ...
    TriggerEvent("inventory:removeItem", "heistlaptop2", 1)
    TriggerServerEvent('qb-paleto:startCoolDown')
    loadDicts()
    ...
    TriggerServerEvent('qb-doors:change-lock-state', 45, false) 
    CreateMoney()
end

RegisterNetEvent('qb-paletogold')
AddEventHandler('qb-paletogold', function()
    print('called 2?')
    TriggerServerEvent('qb-paleto:tut_tut')
end)

AddEventHandler("qb-inventory:itemUsed", function(pItem)
    if pItem == "thermitecharge" then
        TriggerEvent('qb-paleto:box')
    elseif pItem == "heistlaptop2" then
        TriggerServerEvent("qb-paleto:hacklaptop")
    end
end)