...
AddEventHandler("qb-inventory:itemUsed", function(pItem)
    if pItem == "thermitecharge" then
        TriggerEvent('qb-vault:firstdoor')
        TriggerEvent('qb-vault:seconddoor')
        TriggerEvent('qb-vault:lowerfirstdoor')
        TriggerEvent('qb-vault:lowerseconddoor')
    end
    if pItem == "heistlaptop4" then
        TriggerEvent('qb-vault:startlowervault')
        TriggerEvent('qb-vault:startvault')
    end
end)

RegisterNetEvent('qb-vault:firstdoor')
AddEventHandler('qb-vault:firstdoor', function()
    ...
    if pd >= 0 then
        if distance < 1.5 then
            if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                TriggerServerEvent('qb-heists:vaultRobberyLog')
                exports["qb-dispatch"]:dispatchadd('10-90', "Pacific Bank Robbery", "429")
                TriggerEvent("client:newStress",true, 10)
                frontdoorvault()
                TriggerEvent('qb-dispatch:bankrobbery')
                exports['qb-thermite']:OpenThermiteGame(function(success)
                    if success then
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('qb-doors:change-lock-state', 46, false) 
                        Citizen.Wait(3000)
...
RegisterNetEvent('qb-vault:seconddoor')
AddEventHandler('qb-vault:seconddoor', function()
    local player = GetEntityCoords(PlayerPedId())
            ...
            if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['qb-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('qb-doors:change-lock-state', 48, false) 
                        Citizen.Wait(3000)
                        FreezeEntityPosition(PlayerPedId(), false)
                    else
                        TriggerEvent('inventory:removeItem', 'thermitecharge', 1)
...
RegisterNetEvent('qb-vault:startvault')
AddEventHandler('qb-vault:startvault', function()
    local player = GetEntityCoords(PlayerPedId())
    local distance = #(vector3(253.25254, 228.45343, 101.68377) - player)
    local pd = exports["isPed"]:isPed("countpolice")
    if pd >= 0 then
        if distance < 1.5 then
            if exports["qb-inventory"]:hasEnoughOfItem("heistlaptop4", 1) then
                exports["qb-dispatch"]:dispatchadd('10-90', "Pacific Bank Robbery", "429")
                StartVaultKid()
                TriggerEvent('qb-dispatch:bankrobbery')
                TriggerEvent("client:newStress",true, 10)
                ...
                function()
                    ...
                    TriggerEvent('qb-heists:vault_door')
                    DeleteObject(bag)
                    DeleteObject(laptop)
...
RegisterNetEvent('qb-heists:vault_door')
AddEventHandler('qb-heists:vault_door', function()
    TriggerEvent('vault:success:panel', closestBank, true)
    CreatenBrakeCrates()
end)

RegisterNetEvent('qb-vault:lowerfirstdoor')
AddEventHandler('qb-vault:lowerfirstdoor', function()
    local player = GetEntityCoords(PlayerPedId())
            ...
            if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['qb-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('qb-doors:change-lock-state', 49, false) 
                        Citizen.Wait(3000)
                        FreezeEntityPosition(PlayerPedId(), false)
                    else
                        TriggerEvent('inventory:removeItem', 'thermitecharge', 1)
...
RegisterNetEvent('qb-vault:lowerseconddoor')
AddEventHandler('qb-vault:lowerseconddoor', function()
    local player = GetEntityCoords(PlayerPedId())
            ...
            if exports["qb-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['qb-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('qb-doors:change-lock-state', 50, false) 
                        Citizen.Wait(3000)
                        FreezeEntityPosition(PlayerPedId(), false)
                    else
                        TriggerEvent('inventory:removeItem', 'thermitecharge', 1)
...
RegisterNetEvent("qb-vault:grab", function()
...
exports["qb-polytarget"]:AddCircleZone("np_vault_lower_interact_1",  vector3(289.29, 227.55, 98.24), 0.25, {
    useZ = true
})

exports["qb-polytarget"]:AddCircleZone("np_vault_lower_interact_2",  vector3(286.92, 227.39, 98.29), 0.2, {
    useZ = true
})

exports["qb-polytarget"]:AddCircleZone("np_vault_lower_interact_3", vector3(284.69, 221.64, 98.24), 0.2, {
    useZ = true
})

exports["qb-polytarget"]:AddCircleZone("np_vault_lower_interact_4", vector3(286.52, 220.06, 98.29), 0.25, {
    useZ = true
})


exports["qb-interact"]:AddPeekEntryByPolyTarget("np_vault_lower_interact_1", {{
    event = "qb-vaultrob:lower:firstpindoor",
    id = "np_vault_lower_interact_1",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
exports["qb-interact"]:AddPeekEntryByPolyTarget("np_vault_lower_interact_2", {{
    event = "qb-vaultrob:lower:secpindoor",
    id = "np_vault_lower_interact_2",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
exports["qb-interact"]:AddPeekEntryByPolyTarget("np_vault_lower_interact_3", {{
    event = "qb-vaultrob:lower:thirdpindoor",
    id = "np_vault_lower_interact_3",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
exports["qb-interact"]:AddPeekEntryByPolyTarget("np_vault_lower_interact_4", {{
    event = "qb-vaultrob:lower:fourthpindoor",
    id = "np_vault_lower_interact_4",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
RegisterNetEvent('qb-loot_gold_tray')
AddEventHandler('qb-loot_gold_tray', function()
    TriggerServerEvent('qb-heists:loot_tray')
end)

RegisterNetEvent('qb-loot_cash_tray')
AddEventHandler('qb-loot_cash_tray', function()
    TriggerServerEvent('qb-heists:cash_tray')
end)

RegisterNetEvent("qb-heists:grab_from_tray")
AddEventHandler("qb-heists:grab_from_tray", function()
    local chance = math.random(120)
    Loot()
    if chance < 25 then
