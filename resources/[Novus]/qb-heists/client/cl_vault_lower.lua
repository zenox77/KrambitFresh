...
-- RegisterNetEvent('qb-vault:startlowervault')
-- AddEventHandler('qb-vault:startlowervault', function()
...
--                 -- if exports['qb-blackout']:Blackout() then
--                     StartLowerVaultKid()
--                     TriggerEvent('qb-dispatch:bankrobbery')
--                     exports['qb-hacking']:hacking(
--                         function()
--                             ClearPedTasksImmediately(PlayerPedId())
--                             TriggerEvent('qb-heists:lower_vault_done')
--                             DeleteObject(bag)
--                             DeleteObject(laptop)
...
RegisterNetEvent('qb-heists:lower_vault_done')
AddEventHandler('qb-heists:lower_vault_done', function()
    TriggerServerEvent('qb-doors:change-lock-state', 278, false) 
    TriggerServerEvent("qb-doors:change-lock-state", 279, false)
end)
...
function CreatenzylozCrates()
  ...
  AddEventHandler('qb-vault:startlowervault', function()
    ...
    if distance < 1.5 then
        exports['qb-vault']:OpenVaultGame(function(success)
    if success then
        TriggerServerEvent("qb-doors:change-lock-state", 325, false) 
    else
        TriggerEvent('phone:addJobNotify', "You messed up the hack !")
    end
  end, 10, 10)
end 
end)
...
exports["qb-polyzone"]:AddBoxZone("vault_lower_entrance", vector3(266.01, 231.64, 97.68), 5.2, 13.4, {
...
RegisterNetEvent('qb-vault:closevaultlower')
AddEventHandler('qb-vault:closevaultlower', function()
...
RegisterNetEvent('qb-vault:blockedvaultlower')
AddEventHandler('qb-vault:blockedvaultlower', function()
    RequestIpl("np_int_placement_ch_interior_6_dlc_casino_vaultmilo")
...
RegisterNetEvent('qb-vault:openvaultlower')
AddEventHandler('qb-vault:openvaultlower', function()
    RequestIpl("np_int_placement_ch_interior_6_dlc_casino_vaultmilo")
...
RegisterNetEvent('qb-vault:openvaultdoor_ITSLOWER')
AddEventHandler('qb-vault:openvaultdoor_ITSLOWER', function()
    TriggerEvent('qb-vault:openvaultlower')
end)


RegisterNetEvent('qb-vaultrob:lower:vaultdoor')
AddEventHandler('qb-vaultrob:lower:vaultdoor', function(s, args)
    local ped = PlayerPedId()
    ...
    if #(PLAYER_COORDS - vaultdoorlowerlocation) < 20.0 and not vaultdoor then
        ...
        TriggerEvent('qb-vault:openvaultdoor_ITSLOWER')
        CreatenzylozCrates()
        Wait(1800000)
        TriggerEvent("qb-vault:LOWERVAULT_REFRESH")
    end
end)

RegisterNetEvent("qb-vault:LOWERVAULT_REFRESH")
AddEventHandler("vault:LOWERVAULT_REFRESH", function()
    ...
    if vaultdoor then
        vaultdoor = false
        TriggerEvent('qb-vault:blockedvaultlower')
        Citizen.Wait(900000)
        TriggerEvent('qb-vault:closevaultlower')
    end
...
RegisterNetEvent("qb-vaultrob:lower:fourpincontrol")
AddEventHandler("qb-vaultrob:lower:fourpincontrol", function()
...
RegisterNetEvent('qb-vaultrob:lower:firstpindoor')
AddEventHandler('qb-vaultrob:lower:firstpindoor', function()
    if not firstpindoor  then
        exports['qb-vault']:OpenVaultGame(function(success)
            if success then
                firstpindoor = true
                TriggerEvent("qb-vaultrob:lower:fourpincontrol")
            else
                firstpindoor = false
            end
        end, 5, 5)
...
RegisterNetEvent('qb-vaultrob:lower:secpindoor')
AddEventHandler('qb-vaultrob:lower:secpindoor', function()
...
RegisterNetEvent('qb-vaultrob:lower:thirdpindoor')
AddEventHandler('qb-vaultrob:lower:thirdpindoor', function()
...
RegisterNetEvent('qb-vaultrob:lower:fourthpindoor')
AddEventHandler('qb-vaultrob:lower:fourthpindoor', function()
