...
function Housing.func.enterBuilding(propertyID,enterOverride,counterPart)
    ...
    local isBuiltCoords,objects = exports["qb-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true)
    if counterPart and Housing.staticObjectRobPoints == nil then
        Housing.staticObjectRobPoints = exports["qb-build"]:getModule("func").getRobLocationsForObjects(model,spawnBuildingLocation,Housing.housingRobTargets.static)
    ...
    if isBuiltCoords then
        ...
        if not counterPart then
            TriggerEvent("qb-editor:buildName", propertyID, objects)
        end
    else
        Housing.currentHousingInteractions = nil
        Housing.currentlyInsideBuilding = false
...
RegisterNetEvent("qb-housing:loadHousingClient")
AddEventHandler("qb-housing:loadHousingClient", function()
    gatherPlayerInfo()
end)

RegisterNetEvent("qb-housing:reset")
AddEventHandler("qb-housing:reset", function()
    -- cl_housing
    Housing.currentOwned = {}
    Housing.currentKeys = {}
    Housing.currentlyEditing = false
...
RegisterNetEvent("qb-housing:refresh")
AddEventHandler("qb-housing:refresh", function()
    gatherPlayerInfo()
end)
