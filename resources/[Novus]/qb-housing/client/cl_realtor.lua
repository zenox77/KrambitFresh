...
RegisterNetEvent("qb-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("qb-housing:getGarages", function()
...
function getIsPropLock()
    ...
    local pp =  RPC.execute("qb-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('qb-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = RPC.execute("qb-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('qb-phone:editingHouse',id,street,price)
    else
        TriggerEvent('qb-phone:sentHouse',nil,id,street,price,category)
...
function enterEdit(pPropertyId)
    ...
    print('enterEdit ProprtyID: ',pPropertyId)
...
function exitEdit(saveChanges)
    ...
    if not Housing.currentlyInsideBuilding then
        Housing.currentHousingInteractions.id = nil -- Could be a issue maybe not .. if we have errors it's this fuck 
...
function setInventory()
    ...
    if Housing.currentlyInsideBuilding then
        ...
        local buildingVector = exports["qb-build"]:getModule("func").currentBuildingVector()
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("qb-editor:destroyName", id)
    end
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["qb-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
AddEventHandler("qb-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
AddEventHandler("qb-housing:sellTo", function(pParams)
	...
	if input["cid"] then
		...
		TriggerServerEvent("qb-housing:sell", cid, pParams.propertyId)
	end
end)

RegisterNetEvent("qb-housing:buy")
AddEventHandler("qb-housing:buy", function(pPropertyId, pSeller)
...
RegisterCommand('rent', function()
    TriggerEvent('qb-housing:rent')
end)

RegisterNetEvent("qb-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
RegisterNetEvent("qb-housing:rent2", function(pParams)
    local complete, info = RPC.execute("qb-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
RegisterNetEvent("qb-housing:buyed", function(pParams)
    print('hello?')
    local complete, info = RPC.execute("qb-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
RegisterNetEvent("qb-housing:edit", function()
end)

RegisterNetEvent("qb-housing:editOptions", function(pParams)
    if pParams.action == "enter" then
        if Housing.currentHousingInteractions ~= nil then
            enterEdit(Housing.currentHousingInteractions.id)
            return
        else
...
AddEventHandler("housing:charLogout", function()
    ...
    if max.canHaveCharSelect then
        ...
        exports["qb-build"]:getModule("func").CleanUpArea()
        local cid = exports["isPed"]:isPed("cid")
        TriggerServerEvent("qb-jobmanager:onCharSwap", cid)
        Citizen.Wait(1000)   
        TriggerEvent("qb-base:clearStates")
        exports["qb-ui"]:sendAppEvent("hud", { display = false })
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["qb-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:internalBackdoor", function()
    ...
    if max.canHaveBackDoor then
        ...
        DoScreenFadeOut(1)
        exports["qb-build"]:getModule("func").exitCurrentRoom(Housing.currentHousingInteractions.backdoor_coordinates.external)
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["qb-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("qb-housing:blips")
AddEventHandler("qb-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = RPC.execute("qb-housing:owned")
...
RegisterNetEvent('qb-housing:logShit')
AddEventHandler('qb-housing:logShit', function(hID, Price)
    TriggerServerEvent('qb-housing:buyLog', hID, Price)
end)