...
RegisterNetEvent("qb-housing:sell")
AddEventHandler("qb-housing:sell", function(pCid, pPropertyId)
    local src = source
    ...
    print('here')
    TriggerClientEvent("qb-housing:buy", pCid, pPropertyId, seller)
end)
...
AddEventHandler("CheckFurniture", function(pData, pPropertyId)
    ...
    if housingEditing[pPropertyId] ~= nil then
        ...
        TriggerClientEvent("qb-editor:loadEditor", src, pData)
    end
end)
...
RPC.register("qb-housing:buy", function(src, pPropertyId, pTotal, pTax, pSeller)
    local user = exports['qb-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    ...
    TriggerClientEvent("qb-housing:loadHousingClient",src)
    TriggerClientEvent('qb-housing:logShit', src, pPropertyId, pTotal)
    return true, "Purchase Successful" 
end)

RPC.register("qb-housing:rent", function(src, pPropertyId, pTotal, pTax)
    local user = exports['qb-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    ...
    --local success, message = exports["qb-financials"]:transaction(accountId, groupBank, pTotal, comment, cid, 5)
    --if not success then
    --    return false, message
    --end

    --exports["qb-financials"]:addTax("Propertys", pTax)
...
RPC.register("updateCurrentSelected", function(src, pPropertyId, pInformation, pOrigin)
    ...
    --exports["qb-vehicles"]:setGarage(name, data, nil, false)
...
RPC.register("qb-housing:isHousingLockedServer", function(pSrc, pHid)
    print(pHid)
...
RPC.register("qb-housing:owned", function(src)
...
RegisterNetEvent("qb-housing:getGarages", function()
    local src = source
    ...
    for i, v in ipairs(houses) do
        ...
        TriggerClientEvent("menu:send:qb-housing:garages", src,json.decode(garagedata),v.hid)
    end
end)