...
AddEventHandler('baseevents:leftVehicle', function ()
    exports['qb-vehicles']:nitroLevelLeave()
    pursuitmode = 0
end)

AddEventHandler('baseevents:enteredVehicle', function (currentVehicle, currentSeat)
    exports['qb-vehicles']:nitroLevelEnter()
    
    pursuitmode = exports['qb-hud']:pursuitmode() 
  end)
...
RegisterNetEvent("qb-hud:EnableHud", function(status)
    isLoggedIn = status
    if status == true then
        HudActive = false
    end
...
RegisterNetEvent("qb-hud:changeRange", function(pRange)
    voice = pRange or 2
end)
...
RegisterNetEvent('qb-weapons:client:setWeaponFireRate', function (pAmount)
--    weaponFireRate = pAmount
end)

RegisterNetEvent("qb-weapons:client:showWeaponFireRate", function (pBool)
    weaponFireRate = pBool
end) 

RegisterNetEvent("qb-hud:harness_values", function(cinturon)
	harness = cinturon
end)

RegisterNetEvent("qb-hud:pursuit_values", function(pursuit)
	pursuitmode = pursuit
    print(pursuit)
end)
...
Citizen.CreateThread(function()
  while true do
    Wait(2000) -- put back to 0 if we have issues
...
Citizen.CreateThread(function ()
	...
	while true do
        ...
        local pRadioActive = exports['qb-voice']:pRadioActive()
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            local pRadio = exports['qb-voice']:pRadioActive()
            ...
            SendNUIMessage({
                ...
                gps = gps,
                gpu = gpu,
                nos = exports['qb-vehicles']:nitroLevel(),
                dev = developer,
                debug = debug,
...
RegisterNetEvent("qb-admin:currentDevmode")
AddEventHandler("qb-admin:currentDevmode", function(devmode)
    isBlocked = devmode
end)
--- CARHUD ---
...
RegisterNetEvent('qb-hud:ChangeThirst', function(amount)
...
RegisterNetEvent('qb-hud:ChangeHunger', function(amount)
	if isLoggedIn == true then
...
RegisterNetEvent("client:newStress", function(status, amount)
    ...
    if status == true then
        ...
        Wait(1000)
        TriggerServerEvent('qb-hud:UpdateStress_SV', pStress)
...
Citizen.CreateThread(function()
    while true do
        if not isBlocked and isLoggedIn then
            if pStress >= 100 then
...
RegisterNetEvent('np:admin:maxstats', function()
    ...
    TriggerServerEvent("ragdoll:reviveSV", source)
...
RegisterNUICallback("time", function(data)
    exports['qb-carhud']:SetTimeEnabled(data.time)
end)

RegisterNUICallback("SaveSettings", function(data)
    local pInfo = exports["qb-base"]:getModule("LocalPlayer")
    ...
    exports['qb-carhud']:SetCompassFPS(tonumber(data.compassFPSValue))
    exports['qb-carhud']:SetSpeedFPS(tonumber(data.speedFPSValue))
    TriggerEvent('setModel', model)
    TriggerEvent('qb-phone:grabBackground', bg)
    TriggerEvent('changeBG', bg)
    TriggerEvent("qb-phone:setSomeShit",bg,model,twatNotify,callNotify)
    local circleTaskbar = GetResourceKvpInt(cid..'-circleTaskbar')
    if circleTaskbar == 1 then
        TriggerEvent("qb-preferences:setPreferences", true)
    elseif circleTaskbar == 0 then
        TriggerEvent("qb-preferences:setPreferences", false)
    end
...
AddEventHandler('qb-preferences:setPreferences', function(bool)
    local data = {
        ["hud.misc.circle.taskbar.enabled"] = bool
    }
    exports["qb-interface"]:sendAppEvent('preferences', data)
end)

RegisterNetEvent("qb-hud:SetValues", function()
    local pInfo = exports["qb-base"]:getModule("LocalPlayer")
    local cid = exports['isPed']:isPed('cid')
    ...
    local model = exports['qb-phone']:getModelForHud()
    local background = exports['qb-phone']:getBackgroundForHud()
    local twat = exports['qb-phone']:getTwatForHud()
    local call = exports['qb-phone']:getCallForHud()
    ...
    exports['qb-carhud']:SetCompassFPS(SetCompassFPSValue)
    exports['qb-carhud']:SetSpeedFPS(SetSpeedFPSValue)
    ...
    if SetTimeToggled == 1 then
        exports['qb-carhud']:SetTimeEnabled(true)
    elseif SetTimeToggled == 0 then
        exports['qb-carhud']:SetTimeEnabled(false)
    end
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('qb-hud:EnableHud', false)
    TriggerEvent('closeInventoryGui')
    TriggerEvent('qb-banking:ResetUI')
    TriggerEvent('qb-radio:ResetUI')
    TriggerEvent('qb-menu:ResetUI')
    TriggerEvent('qb-interact:ResetUI')
    TriggerEvent('qb-mdt:ResetUI')
    TriggerEvent('phone:reset')
    TriggerEvent('clothing:close')
    exports['qb-interface']:showInteraction("Restarting UI .")
    Wait(1000)
    exports['qb-interface']:showInteraction("Restarting UI ..")
    Wait(1000)
    exports['qb-interface']:showInteraction("Restarting UI ...")
    Wait(1000)
    exports['qb-interface']:showInteraction("UI Successfully Restarted")
    Wait(1000)
    exports['qb-interface']:hideInteraction()
...
RegisterNetEvent('qb-hud:boosting_value')
AddEventHandler('qb-hud:boosting_value', function(pValue)
...
RegisterNetEvent('qb-buffs:bulb_buff')
AddEventHandler('qb-buffs:bulb_buff', function()
...
RegisterNetEvent('qb-buffs:swim_buff')
AddEventHandler('qb-buffs:swim_buff', function()
...
RegisterNetEvent('qb-buffs:strength_buff')
AddEventHandler('qb-buffs:strength_buff', function()
...
RegisterNetEvent('qb-buffs:money_boost_buff')
AddEventHandler('qb-buffs:money_boost_buff', function()
