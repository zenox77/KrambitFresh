...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('qb-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("qb-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['qb-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("qb-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("qb-fishing:currentlyRentedBoat", function(handle)
