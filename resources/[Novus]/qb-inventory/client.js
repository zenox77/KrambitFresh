...
on('qb-base:playerSpawned', async (broughtData) => {
    // emitNet('server-request-update', cid); // Since whoever did this shit fucked up spawning players somehow ... SMH [Temp fix]
...
//  $.post("http://qb-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['qb-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
  ...
  //  const apartmentFloor = exports["qb-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['qb-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
       ...
       // let serverCode = exports["qb-config"].GetServerCode();
...
on('qb-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('qb-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("qb-inventory:craftProgression", data);
    emitNet("qb-inventory:craftProgression", data);
});
...
on('__cfx_nui:move', (data, cb) => {
    if (global.exports['qb-taskbar'].pInInv()) {
        emit('DoLongHudText', 'Nice try exploiting mate.', 2)
        emitNet('qb-inventory:exploiterKick')
    } else {
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("qb-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("qb-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('qb-inventory:server:weightChange', cid, weight);
