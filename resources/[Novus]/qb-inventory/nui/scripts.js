...
$(document).ready(function() {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://qb-inventory/UpdateSettings',
            JSON.stringify({
...
function invStack(
    ...
    $.post('https://qb-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://qb-inventory/move', JSON.stringify(arr));
}
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://qb-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://qb-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function ErrorMove() {
    // $.post('https://qb-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://qb-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://qb-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://qb-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://qb-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://qb-inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://qb-inventory/craftProgression', JSON.stringify({
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://qb-inventory/invuse', JSON.stringify(arr));
