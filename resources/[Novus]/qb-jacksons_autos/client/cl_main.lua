...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("qb-jacksons_autos:CarTable",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("qb-jacksons_autos:testdrive")
AddEventHandler("qb-jacksons_autos:testdrive", function()
	if exports["qb-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.9081, -1032.289, 13.719951) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qb-jacksons_autos:EnableBuy")
AddEventHandler("qb-jacksons_autos:EnableBuy", function()
	if #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["qb-business"]:IsEmployedAt('jacksons_autos') then
		return
	end
	...
	TriggerServerEvent("qb-jacksons_autos:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("qb-jacksons_autos:BuyEnabledCL")
AddEventHandler("qb-jacksons_autos:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("qb-jacksons:CommissionPrice")
AddEventHandler("qb-jacksons:CommissionPrice", function(newAmount)
	if eexports["qb-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qb-jacksons_autos:ReturnCarTable")
AddEventHandler("qb-jacksons_autos:ReturnCarTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('qb-jacksons_autos:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #pSpawnLocations do
		if #(vector3(pSpawnLocations[i]["x"],pSpawnLocations[i]["y"],pSpawnLocations[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["qb-business"]:IsEmployedAt('jacksons_autos') then
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("qb-jacksons_autos:RequestCarTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('qb-jacksons_autos:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('qb-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'Jacksons Autos')
			TriggerServerEvent('qb-financials:business_money', price / 10, 'jacksons_autos', 'add')
			exports['qb-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("qb-jacksons_autos:FailedPurchase")
AddEventHandler("qb-jacksons_autos:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "b-class" or this == "s-class" then
		if ownerMenu then
			...
			TriggerServerEvent('qb-jacksons_autos:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["qb-business"]:IsEmployedAt('jacksons_autos') and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commission', function(source, args, raw)
	if exports["qb-business"]:IsEmployedAt('jacksons_autos') then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('qb-jacksons_autos:CommissionPrice', amount)
		else
...
RegisterCommand('testdrive', function(source, args, raw)
	if exports["qb-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('qb-jacksons_autos:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuy', function(source, args, raw)
	if exports["qb-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('qb-jacksons_autos:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)