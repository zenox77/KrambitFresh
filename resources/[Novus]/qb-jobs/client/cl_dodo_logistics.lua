...
RegisterNetEvent('qb-jobs:passed')
AddEventHandler('qb-jobs:passed', function()
    pisInDodoJob = true
    ...
    TriggerEvent('qb-phone:goToLocation')
end)

RegisterNetEvent('qb-jobs:startDodoLogistics')
AddEventHandler('qb-jobs:startDodoLogistics', function()
    TriggerEvent('qb-phone:signInDodo')
    ...
    TriggerEvent('qb-phone:dodoJobOffer')
end)

RegisterNetEvent('qb-jobs:dodoLogisticsPayout')
AddEventHandler('qb-jobs:dodoLogisticsPayout', function()
    if pOnWayDodo == true then
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder"))  then
            if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                TriggerServerEvent('qb-jobs:dodoLogisticsPayout')
    ...
    while true do
        ...
        if isToDropLocation == true then
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 2.5 then
                if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                    exports['qb-interface']:showInteraction('[E] Drop Off Package')
                    if IsControlJustPressed(1,38) then
                        TriggerEvent('qb-phone:goBackDepo')
                        exports['qb-interface']:hideInteraction()
                            ...
                            local aDist = GetDistanceBetweenCoords(setCoords, coordA)
                                if aDist < 15.0 then
                                    ...
                                    if exports['qb-hud']:MoneyBuff() then
                                        TriggerServerEvent('qb-jobs:givePayout', math.random(878, 927), "bank", '[Dodo Logistics | With Buff]')
                                    else
                                        TriggerServerEvent('qb-jobs:givePayout', math.random(650, 778), "bank", '[Dodo Logistics | Without Buff]')
                                    end
                                end
                            end
                        end)
...
RegisterNetEvent("qb-jobs:spawn-truck")
AddEventHandler("qb-jobs:spawn-truck", function()
	Citizen.CreateThread(function()
...
exports["qb-polytarget"]:AddCircleZone("dodo_sign_in", vector3(-437.91, -2795.5, 7.39), 0.6, {
    useZ=true,
})

exports["qb-interact"]:AddPeekEntryByPolyTarget("dodo_sign_in", {{
    event = "qb-jobs:startDodoLogistics",
    id = "dodo_sign_in",
    icon = "circle",
    label = "Sign In",
    parameters = {},
