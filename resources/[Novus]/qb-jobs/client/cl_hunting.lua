...
Citizen.CreateThread(function()
  while true do
    ...
    if dst < 40 and jobSpawned == false then
      TriggerEvent('qb-hunting:spawnJobPed',dumbass, 225.87)
      jobSpawned = true
      refreshPed = true
    end
    if dst >= 41  then
...
AddEventHandler('qb-hunting:skinAnimal', function()
  if exports["qb-inventory"]:hasEnoughOfItem("huntingknife", 1) then
    if DoesEntityExist(baitAnimal) then
        LoadAnimDict('amb@medic@standing@kneel@base')
        LoadAnimDict('anim@gangops@facility@servers@bodysearch@')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        TaskPlayAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@" ,"player_search" ,8.0, -8.0, -1, 48, 0, false, false, false )
        local finished = exports["qb-taskbar"]:taskBar(5000,"Skinning")
  ...
  for k,v in pairs (HuntingZones) do
    if IsEntityInZone(GetPlayerPed(-1),v) then
      if cooldown <= 0 then
        LoadAnimDict('amb@medic@standing@kneel@base')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        local finished = exports["qb-taskbar"]:taskBar(5000,"Placing Bait",false,false,playerVeh)
        Citizen.Wait(100)
...
local function processScope(freeAiming)
    if not isFreeAiming and freeAiming then
        isFreeAiming = true
        exports["qb-ui"]:sendAppEvent("sniper-scope", { show = true })
    elseif isFreeAiming and not freeAiming then
        isFreeAiming = false
        exports["qb-ui"]:sendAppEvent("sniper-scope", { show = false })
    end
end
...
local function sellAnimals()
  ...
  for _, carcass in pairs(carcasses) do
      local qty = exports["qb-inventory"]:getQuantity(carcass.name)
...
AddEventHandler("qb-polyzone:enter", function(name)
  ...
  exports['qb-interface']:showInteraction("[E] Sell Animal Carcass")
  listenForKeypress()
end)
...
Citizen.CreateThread(function()
  exports["qb-polyzone"]:AddCircleZone("huntingsales", vector3(12.28, -1299.32, 29.29), 1.3, {
      useZ = true,
      debugPoly= false,
  })
end)