...
RegisterNetEvent('qb-jobs:tow_trucker_duty')
AddEventHandler('qb-jobs:tow_trucker_duty', function()
	if not clockedon then
		...
		clockedon = true
		TriggerEvent('qb-phone:signInInfoTowing')
		TriggerServerEvent('qb-duty:tow_trucker')
		TriggerEvent('qb-jobs:give_tow_job')
	else
		TriggerEvent('DoLongHudText', 'You\'re already clocked in.', 2)
	end
end)

RegisterNetEvent('qb-jobs_tow_trucker_off_duty')
AddEventHandler('qb-jobs_tow_trucker_off_duty', function()
	if clockedon then
...
RegisterNetEvent('qb-jobs:towing_accept')
AddEventHandler('qb-jobs:towing_accept', function()
	GiveTowJobWanker()
end)
...
function GiveTowJobWanker()
	...
	local vehicle = exports['qb-vehicles']:GetVehicleTable()
	local vehiclehash = vehicle[math.random(1, #vehicle)].model
		...
		Nopixel.Location = locations[math.random(1, #locations)]
		
        pGoToCar = AddBlipForCoord(Nopixel.Location.x, Nopixel.Location.y, Nopixel.Location.z)
		...
		Nopixel.Chosen = false
		towedavehicle = true
		while not Nopixel.Chosen do
			Citizen.Wait(10000)
		...
		if fuckyouputa >= 80 then
			...
			Nopixel.Looking = true
			Citizen.CreateThread(function()
				while Nopixel.Looking do
					Citizen.Wait(2000)
					if #(vector3(Nopixel.Location.x, Nopixel.Location.y, Nopixel.Location.z) - GetEntityCoords(PlayerPedId())) <= 320 then
						isAbleImp = 1
						...
						CanImp = true
						Nopixel.VehicleID = CreateVehicle(vehiclehash, Nopixel.Location.x, Nopixel.Location.y, Nopixel.Location.z, true, false)
						SetEntityHeading(Nopixel.VehicleID, Nopixel.Location.h)
						local plate = GetVehicleNumberPlateText(Nopixel.VehicleID)
						setimpoundveh = plate
                        TriggerEvent('qb-fuel:setFuel', GetVehiclePedIsIn(PlayerPedId()), 100)
						SetVehicleOnGroundProperly(Nopixel.VehicleID)
						SetModelAsNoLongerNeeded(vehiclehash)
						Citizen.Wait(100)
						local netid = NetworkGetNetworkIdFromEntity(Nopixel.VehicleID)
						SetNetworkIdCanMigrate(netid, true)
						NetworkRegisterEntityAsNetworked(VehToNet(Nopixel.VehicleID))	
						SetEntityVisible(Nopixel.VehicleID, true)
						SetVehicleEngineHealth(Nopixel.VehicleID, 850)
						SmashVehicleWindow(Nopixel.VehicleID, 0)
						SmashVehicleWindow(Nopixel.VehicleID, 1)
						SmashVehicleWindow(Nopixel.VehicleID, 2)
						SmashVehicleWindow(Nopixel.VehicleID, 3)
						SmashVehicleWindow(Nopixel.VehicleID, 4)
						
						SetVehicleDoorBroken(Nopixel.VehicleID, math.random(1,2), true)
						SetVehicleDoorBroken(Nopixel.VehicleID, math.random(3,4), true)
						SetVehicleDoorBroken(Nopixel.VehicleID, math.random(5,6), true)
						
						SetVehicleTyreBurst(Nopixel.VehicleID, 1, false, 990.0)
						SetVehicleTyreBurst(Nopixel.VehicleID, 2, false, 990.0)
						SetVehicleTyreBurst(Nopixel.VehicleID, 3, false, 990.0)
						SetVehicleTyreBurst(Nopixel.VehicleID, 4, false, 990.0)
						Nopixel.Looking = false
...
RegisterNetEvent('qb-jobs:give_tow_job')
AddEventHandler('qb-jobs:give_tow_job', function()
	...
	if clockedon then
		TriggerEvent('qb-phone:towing_offer')
	end
end)
...
Citizen.CreateThread(function()
    exports["qb-polyzone"]:AddBoxZone("np_impound_location", vector3(-236.51, -1172.86, 37.6), 20, 27.6, {
        name="np_impound_location",
        heading=270,
		-- debugPoly=false,
		minZ=22.2,
		maxZ=26.2
...
AddEventHandler('qb-polyzone:enter', function(name)
    if name == "np_impound_location" then
        DesirePolyImp = true     
    end
end)
...
RegisterNetEvent('qb-jobs:impound_confirm_shit_idk_anymore')
AddEventHandler('qb-jobs:impound_confirm_shit_idk_anymore', function()
	if DesirePolyImp and CamImp then
		TriggerEvent('qb-jobs:towing_imp')
	end
end)

RegisterNetEvent('qb-jobs:impound_accept')
AddEventHandler('qb-jobs:impound_accept', function()
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		...
		if licensePlate == setimpoundveh then
			...
			if exports['qb-hud']:MoneyBuff() then
				TriggerServerEvent('qb-jobs:givePayout', math.random(1076, 1554), "cash", '[24/7 Delivery Job | With Buff]')
			else
				TriggerServerEvent('qb-jobs:givePayout', math.random(652, 886), "cash", '[24/7 Delivery Job | Without Buff]')
			end
			TriggerEvent('qb-jobs:give_tow_job')
...
RegisterNetEvent('qb-jobs:imp_decline')
AddEventHandler('qb-jobs:imp_decline', function()
...
RegisterNetEvent('qb-jobs:towing_imp')
AddEventHandler('qb-jobs:towing_imp', function()
...
RegisterNetEvent('qb-jobs_tow_menu')
AddEventHandler('qb-jobs_tow_menu', function()
	...
	local TowMenu = {
		...
		{
			title = "Accept",
			action = "qb-jobs:accept_impound_req",
		},
		{
			title = "Decline",
			action = "qb-jobs:decline_impound_req",
		},
	}
	exports["qb-interface"]:showContextMenu(TowMenu)
end)

RegisterInterfaceCallback('qb-jobs:accept_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('qb-jobs:impound_accept')
end)

RegisterInterfaceCallback('qb-jobs:decline_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('qb-jobs:imp_decline')
end)
...
exports["qb-polyzone"]:AddBoxZone("towing_shit", vector3(-191.63, -1166.46, 23.67), 2, 2, {
	name="towing_shit",
	heading=0,
	--debugPoly=false,
	minZ=21.27,
...
AddEventHandler('qb-polyzone:enter', function(name)
	if name == "towing_shit" then
		...
		pSignOnOff()
		if not clockedon then
			exports['qb-interface']:showInteraction('[E] Sign In')
		else
			exports['qb-interface']:showInteraction('[E] Sign Out', 'error')
		end
	end
end)
	...
	Citizen.CreateThread(function()
        while towing_duty do
			...
			if IsControlJustReleased(0, 38) then
				if not clockedon then
					TriggerEvent('qb-jobs:tow_trucker_duty')
					exports['qb-interface']:hideInteraction()
				elseif clockedon then
					DeleteEntity(pVeh)
					TriggerEvent('qb-jobs_tow_trucker_off_duty')
					exports['qb-interface']:hideInteraction()
				end
			end
		end
...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
		    ...
		    local finished = exports["qb-taskbar"]:taskBar(15000,"Hooking up vehicle.")
        	if finished == 100 and aDist < 5.0 then
				...
				if targetVehicle ~= 0 then
					...
					local part2 = exports["qb-taskbar"]:taskBar(7000,"Towing Vehicle")
		    ...
		    TriggerEvent("animation:tow")
		    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck', 0.5)
		    local finished = exports["qb-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then
                hasVehicleHookedUp = false

        		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        		local part2 = exports["qb-taskbar"]:taskBar(7000,"Unhooking Vehicle")
