...
AddEventHandler('server:GroupPayment', function( gangid, cashamount, sauce)
  ...
  RegisterServerEvent('qb-weedbox:payout')
AddEventHandler('qb-weedbox:payout', function(money)
    local source = source
        ...
        for i = 1, #Registers[regID] do
            if Registers[regID][i].regID == regID then
                ...
                if (tonumber(user:getCash()) >= tonumber(amount)) then
                    ...
                    local owner = exports["qb-base"]:getModule("Player"):GetUser(Registers[regID][i].owner)
...
RegisterServerEvent("qb-jobs:cosmic_cannabis:update:pay")
AddEventHandler("qb-jobs:cosmic_cannabis:update:pay", function(cid)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
