...
function DrawFromLootTable(pServerId, pTableName, pDraws)
    ...
    local player = exports["qb-base"]:getModule("Player"):GetUser(pServerId)
...
AddEventHandler("loot:useItem", function(itemId)
    ...
    if itemId == 'newaccountbox' then
        ...
        TriggerEvent("np:loot:system:draw", src, 'event_newaccountbox2', 1)
        TriggerEvent("np:loot:system:draw", src, 'event_newaccountbox3', 1)
        TriggerEvent("np:loot:system:draw", src, 'event_newaccountbox4', 1)
        TriggerEvent("np:loot:system:draw", src, 'event_newaccountbox5', 1)
        TriggerEvent("np:loot:system:draw", src, 'event_newaccountbox6', 1)
    end
end)