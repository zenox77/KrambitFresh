RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("qb-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("qb-musicplayer:stateSound")
AddEventHandler("qb-musicplayer:stateSound", function(state, data)
