;(function () {
  ...
  !(function () {
    ...
    setImmediate(async () => {
      ...
      Number(objectTable.length) === 0 && emitNet('qb-objects:requestObjects')
    })
    onNet('qb-objects:loadObjects', async (pObjectData) => {
      console.log('[qb-objects] Load Objects')
      let objectTable = pObjectData
      Object.entries(objectTable).forEach(([k, v]) => {
        prepareObject(v)
      })
    })
    onNet('np:objects:prepareNewObject', async (newObject) => {
      prepareObject(newObject)
    })
    on('qb-polyzone:enter', async (name, data) => {
      if (name !== 'object_zone') {
        return
      }
      ...
      console.log('[qb-objects] Entered zone, load objects in distance.')
    ...
    on('qb-polyzone:exit', (name, data) => {
      ...
      console.log('[qb-objects] Left zone, unload objects in distance.')
    ...
    onNet('qb-objects:clearObjects', (objectData) => {
    ...
    onNet('qb-objects:updateObjects', (objectData) => {
      Object.entries(objectData).forEach(([k, v]) => {
        let findData = newObjTable.findIndex(
          (fountData) => fountData.id.toString() === v.toString()
        )
    ...
    async function prepareObject(objectData) {
      console.log('[qb-objects] Adding object: ', objectData)
      let coords = {
      ...
      return (
        Object.entries(objectTable).forEach(([k, v]) => {
          if (Number(v.obj) === Number(pEntity)) {
            let findInd = objectTable.findIndex((fountInd) => Number(fountInd.id) === Number(v.id))
    ...
    function DelObject(obj) {
      let deleted = RPC.execute('qb-objects:DeleteObject', obj)
      return deleted
    }
    exports('DelObject', DelObject)
    function UpdateObject(object, updatedObject) {
      RPC.execute('qb-objects:UpdateObject', object, updatedObject)
    }
    ...
    RegisterCommand(
      ...
      (src, args, user) => {
        ...
        console.log('[qb-objects] getObject return: ', consoleObj)
      },
      false
    )
    RegisterCommand(
  ...
  pMetaData
      ...
      const [_0x5447e5, pCoords] = await placeDaObjectBruh(pObjectModel,_0x72d15e,_0x46e724)
      if (!_0x5447e5) {
        return null
      }
      return await RPC.execute('qb-objects:SaveObject',pRando,pObjectModel,pCoords.coords,pCoords.rotation,pHeading,pMetaData)
    }
    exports('PlaceAndSaveObject', PlaceAndSaveObject)
    let _0x49ebb8 = false
    const placeDaObjectBruh = async (iThinkObject, _0x1e1814, _0x65c686 = () => true) => {
      var _0x3deadd, _0x3e8019, _0x54e9e9, _0x1e7782
      if (_0x49ebb8) {
        return [false, null]
      }
      const pObjectModel = typeof iThinkObject === 'string' ? iThinkObject.trim() : iThinkObject
      if (!IsModelValid(pObjectModel)) {
        return [false, null]
      }
      _0x49ebb8 = true
      await ghostCreateModel(pObjectModel)
      const [_0x320bb1, _0xb5f359] = GetModelDimensions(pObjectModel),
      ...
      let _0x2ead42 =
        ...
        _0x137066 = true,
        _0x544df1 = true,
        _0xe78988 = (_0x3e8019 = _0x1e1814.zOffset) !== null && _0x3e8019 !== void 0? _0x3e8019: 0,
        _0x5e482a = false
      const _0x184686 =
    ...
    exports('PlaceObject', placeDaObjectBruh)
    ...
    async function ghostCreateModel(pModel) {
    ...
    function _0x46659e(pDataa, _0x192a18) {
      ...
      _0x538309 = true
      spawnNewObjectGhost(pObjectModell)
    }
    function spawnNewObjectGhost(pData) {
      emitNet(
        'qb-objects:prepareObject',
        _0x4f06bd,
        pData.x,
        pData.y,
        pData.z,
    ...
    function objectCamThing(_0x79df0, _0x3028f9) {
    ...
    function _0x264aed() {
      const [_0x141562, _0x5dbe5f, _0x4e1d48, _0x1112a4, _0x4c46e3] = objectCamThing(1,0)
      Number(_0x5dbe5f) === 1 && (_0x304a5c = _0x4e1d48)
    }
    on('onResourceStop', (_0x4c6c42) => {
      if (_0x4c6c42 !== 'qb-objects') {
        return
      }
