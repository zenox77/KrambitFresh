...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('qb-paleto:getGetDoorStateSV')
...
RegisterNetEvent("qb-paleto:getDoorCheckCL")
AddEventHandler("qb-paleto:getDoorCheckCL", function(result)
...
RegisterNetEvent("qb-paleto:resetDoorState")
AddEventHandler("qb-paleto:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("qb-paleto:lootup_c")
AddEventHandler("qb-paleto:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("qb-paleto:outcome")
AddEventHandler("qb-paleto:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("qb-paleto:startheist", Paleto.Banks[arg], arg)
    elseif not oc then
        TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("qb-paleto:startLoot_c")
AddEventHandler("qb-paleto:startLoot_c", function(data, name)
    currentname = name
            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot) then
                        ...
                        if initiator then
                            TriggerEvent("qb-paleto:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("qb-paleto:stopHeist_c")
AddEventHandler("qb-paleto:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- MAIN DOOR UPDATE --

AddEventHandler("qb-paleto:freezeDoors", function()
    Citizen.CreateThread(function()
...
RegisterNetEvent("qb-paleto:ClosePaletoDoor")
AddEventHandler("qb-paleto:ClosePaletoDoor", function()
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418,6472.1274414062,31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
        ...
        Citizen.Wait(10000)
    until count == 900
    TriggerServerEvent('qb-paleto:getGetDoorStateSVSV', false)
end)


-- RegisterNetEvent("qb-paleto:toggleVault")
...
--             TriggerServerEvent("qb-paleto:updateVaultState", key, Doors[key][1].state)
--         elseif state then
--             local obj = GetClosestObjectOfType(Paleto.Banks[key].doors.startloc.x, Paleto.Banks[key].doors.startloc.y, Paleto.Banks[key].doors.startloc.z, 2.0, GetHashKey(Paleto.vaultdoor), false, false, false)
--             local count = 0
...
RegisterNetEvent("qb-paleto:OpenPaletoDoor")
AddEventHandler("qb-paleto:OpenPaletoDoor", function()
    TriggerServerEvent('qb-paleto:getGetDoorStateSVSV', true)
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418, 6472.1274414062, 31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
...
AddEventHandler("qb-paleto:reset", function(name, data)
    ...
    TriggerServerEvent("qb-paleto:closeDoor")
    -- TriggerServerEvent("qb-paleto:toggleVault", name, true)
    TriggerEvent("qb-paleto:cleanUp", data, name)
end)


AddEventHandler("qb-paleto:startheist", function(data, name)
    disableinput = true
    currentname = name
  --  currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent('qb-paleto:openDoor')
    TriggerServerEvent("qb-paleto:setCooldown", name)
    --TriggerServerEvent("qb-paleto:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

-- AddEventHandler("qb-paleto:cleanUp", function(data, name)
--     Citizen.Wait(10000)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("qb-paleto:startLoot", data, name, missionplayers)
    done = false
end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("qb-paleto:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
RegisterNetEvent('qb-paleto:darkmail')
AddEventHandler('qb-paleto:darkmail', function()
    Citizen.Wait(1000)
...
RegisterNetEvent('qb-paleto:UseBlueLapTop')
AddEventHandler('qb-paleto:UseBlueLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    Paleto.Banks, doors = RPC.execute("qb-paleto:getBanks")
	TriggerEvent("qb-paleto:freezeDoors")
    ...
    for k, v in pairs(Paleto.Banks) do
        if not v.onaction then
            ...
            if dist < 1.0 then
                ...
                if exports["qb-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                    -- if HasItem then
                    ...
                    exports['hacking']:OpenHackingGame(function(Success)
                        if Success then
                            ...
                            Citizen.SetTimeout(timeout, function()
                                TriggerServerEvent("qb-paleto:startcheck", k)
                            end)
...
AddEventHandler('paletogate:doorburn', function()
	TriggerServerEvent('qb-doors:change-lock-state', 498, false)
end)

RegisterNetEvent('paletogate2:doorburn') -- Doorlock
AddEventHandler('paletogate2:doorburn', function()
	TriggerServerEvent('qb-doors:change-lock-state', 517, false)
end)
