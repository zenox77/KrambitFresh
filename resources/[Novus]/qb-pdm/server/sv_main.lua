...
RegisterServerEvent('qb-pdm:CarTablePDM')
AddEventHandler('qb-pdm:CarTablePDM', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('qb-pdm:ReturnPDMTTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('qb-pdm:FinaceEnabledSV')
AddEventHandler('qb-pdm:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qb-pdm:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('qb-pdm:BuyEnabledSV')
AddEventHandler('qb-pdm:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qb-pdm:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('qb-pdm:RequestPDMTTable')
AddEventHandler('qb-pdm:RequestPDMTTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('qb-pdm:ReturnPDMTTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('qb-pdm:ChechMoney')
AddEventHandler('qb-pdm:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('qb-pdm:FailedPurchase', src)
    end
end)

RegisterServerEvent('qb-pdm:BuyVehicle')
AddEventHandler('qb-pdm:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qb-pdm:wtf')
AddEventHandler('qb-pdm:wtf', function(comis)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    TriggerClientEvent("qb-pdm:commissionpdm", src, comis)
end)