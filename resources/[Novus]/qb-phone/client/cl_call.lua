...
RegisterNUICallback('getCallHistory', function()
  local recentCallHistory = RPC.execute("qb-phone:getRecentCallsByCid", exports['isPed']:isPed("cid"))
  SendNUIMessage({
...
Citizen.CreateThread(function()
  while true do
    Wait(10000)
    TriggerServerEvent("qb-phone:sendRecentCalls", recentcalls, exports['isPed']:isPed("cid"))
  end
end)
...
AddEventHandler('phone:otherClientEndCall', function()
  ...
  Citizen.CreateThread(function()
    exports["qb-keybinds"]:registerKeyMapping("", "Player", "Hangup Call", "+hangup", "-hangup", "5")
    exports["qb-keybinds"]:registerKeyMapping("", "Player", "Answer Call", "+answer", "-answer", "6")
  end)
  ...
  AddEventHandler('phone:addToCall', function(voipchannel)
    exports['qb-voice']:addPlayerToCall(tonumber(voipchannel))
  end)
...
RegisterNetEvent('qb-phone:RemoveCall')
AddEventHandler('qb-phone:RemoveCall', function()
    SendNUIMessage({
      openSection = 'callnotifyEnd'
    })
    SendNUIMessage({
...
function endCall()
  ...
  mySourceID = 0
  TriggerEvent('phone:setCallState', isNotInCall)
  TriggerEvent('qb-phone:RemoveCall')
  onhold = false
  mySourceHoldStatus = false
  callid = 0
end
...
function checkForPayPhone()
  ...
  local ph = exports["qb-applications"]:KeyboardInput({
    header = "Payphone",
    rows = {
      {
        id = 1,
