RegisterNUICallback("getDocuments", function(data, cb)
    SendNUIMessage({
        ...
        documentData = RPC.execute("qb-phone:getDocuments", data.pType)
    })
end)

RegisterNUICallback("deleteDocument", function(data, cb)
    RPC.execute("qb-phone:deleteDocument", data.documentID)
end)

RegisterNUICallback("updateDocument", function(data, cb)
    RPC.execute("qb-phone:updateDocument", data.documentID, data.documentTitle, data.documentText)
end)

RegisterNUICallback("createDocument", function(data, cb)
    RPC.execute("qb-phone:createDocument", exports['isPed']:isPed("cid"), "New Document", "Text Goes Here", data.documentType)
end)

RegisterNUICallback("shareDocumentPermanent", function(data)
    RPC.execute("qb-phone:shareDocumentPermanent", data.documentID, data.targetCID)
end)
...
RegisterNUICallback("shareDocumentLocal", function(data)
    ...
    for k, v in pairs(players) do
        RPC.execute("qb-phone:sendDocumentEvent", v, data.documentID)
    end
end)


RegisterNetEvent("qb-phone:sendDocumentEvent", function(documentID)
...
RegisterNUICallback("acceptDocumentReceiveRequest", function(data)
    local pDocShared = RPC.execute("qb-phone:acceptDocumentReceiveRequest", data.documentID)
    SendNUIMessage({
