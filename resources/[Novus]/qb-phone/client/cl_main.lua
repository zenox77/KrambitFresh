...
local passWifi = 0
local selfie_Discord = "https://discordapp.com/api/webhooks/xxxxxxxxxxxxxxxxxxxxxxxxxxxxx-M"
local wifipass1 = ""
...
RegisterNetEvent('qb-base:playerSpawned')
AddEventHandler('qb-base:playerSpawned', function()
  Wait(1000)
...
RegisterCommand('renten', function()
  TriggerEvent('qb-housing:rent_activate_cl')
end)

RegisterNetEvent('qb-housing:rent_activate_cl')
AddEventHandler('qb-housing:rent_activate_cl', function()
...
AddEventHandler('qb-phone:sanitationIn', function()
  -- print("SANITATION SIGN IN")
  sanitation = true
end)

RegisterNetEvent("qb-phone:grabBackground")
AddEventHandler("qb-phone:grabBackground", function(link)
    wallPaper = link
end)
...
RegisterNUICallback('wallpaper', function(data, cb)
  ...
  TriggerEvent('qb-phone:grabBackground', wallPaperSelecionado)
...
RegisterNUICallback('submitSettings', function(data, cb)
  ...
  Wait(5)
  if bg ~= "" then
    TriggerEvent('qb-phone:grabBackground', bg)
...
RegisterNetEvent("qb-phone:setSomeShit", function(bg, model, twatNotify, callNotify)
  SetResourceKvp(exports['isPed']:isPed('cid').."-bg", bg)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-pmodel", model)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ntwat", twatNotify)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ncall", callNotify)
...
RegisterNUICallback('btnHousing', function()
  ...
  allNewHouses = {}
  myAccessHouse = {}
  --local apt = RPC.execute("qb-phone:apt")
  local housing = exports['qb-housing']:housingForPhone()
  local apartment = exports['qb-apartments']:getMyCurrentApartment()
  myHousess = RPC.execute("qb-phone:getCurrentOwned")
  accessHouse = RPC.execute('qb-phone:getAccessHouse_2')
  ...
  for i,v in pairs(myHousess) do
    ...
    table.insert(newHouses, {
      ...
      cid = v.cid,
      status = v.status,
      cat = exports['qb-housing']:getHousingCatFromPropertID(v.hid),
    })
  end
  for l,s in pairs(accessHouse) do
      table.insert(myAccessHouse, {
        ...
        cid = s.cid,
        status = s.status,
        cat = exports['qb-housing']:getHousingCatFromPropertID(s.house),
      })
  end
  SendNUIMessage({
    openSection = "housing",
...
RegisterNUICallback('btnKeyHolders', function(data, cb)
  local myKittens = {}
  local keys = RPC.execute('qb-housing:getKeyHolderz', data.hid)
  SendNUIMessage({
...
RegisterNUICallback('whatthefuckdoesthisshitwork', function(data,cb)
  local cid = data.cid
  local hid = data.hid
  local success,msg = RPC.execute('qb-housing:removeKey:phone',cid, hid)
  if success then
...
RegisterNUICallback('OpenCamera', function(data,cb)
  RPC.execute('qb-phone:openCamera', data.cameraId)
end)
...
--   while true do
--     Wait(3000)
--     local myHouse = RPC.execute("qb-phone:getCurrentOwned")
...
RegisterInterfaceCallback("qb-phone:signInHandler", function(data, cb)
  if data.key.sign_in then
    RPC.execute("qb-phone:addDOJ", data.key.job)
  else
    RPC.execute("qb-phone:removeDOJ", data.key.job)
  end
...
RegisterNUICallback('getActiveDOJ', function(data,cb)
      SendNUIMessage({
        ...
        penisinmybuttholefuckshit = RPC.execute("qb-phone:getDOJ")
      })
end)
...
RegisterNUICallback('abdulSignIn', function(data,cb)
    ...
    if taxiStatus then
      RPC.execute("qb-phone:addAbdulTaxi")
      addAbdulTaxi()
    ...
    elseif not taxiStatus then
      RPC.execute("qb-phone:removeAbdulTaxi")
      removeAbdulTaxi()
      local availableDrivers = getAbdulTaxi()
      SendNUIMessage({
        openSection = "addTaxi",
...
RegisterNetEvent("qb-phone:manualUpdateAbdulTaxies", function(availableDrivers)
  addAbdulTaxi()
  local availableDrivers = getAbdulTaxi()
  SendNUIMessage({
    openSection = "addTaxi",
...
function addAbdulTaxi () 
  currenttable = {}
  local table = RPC.execute("qb-phone:getAbdulTaxies")
--  local currenttable = {}
...
RegisterNUICallback('sellVeh', function(data)
  ...
  TriggerServerEvent('qb-phone:sendVehBill', data.paypal, data.plate, data.amount)
end)

RegisterNetEvent('qb-phone:sellCar')
AddEventHandler('qb-phone:sellCar', function(plate,amount,sId)
  sellCarPlate = plate
...
RegisterNUICallback('purchaseCar', function(data)
  TriggerServerEvent('qb-phone:purchasedCar', sellCarPlate, data.amount,data.senderID)
end)

RegisterNUICallback('purchase_gas', function(data)
  TriggerEvent('qb-fuel:OfferAccepted')
end)

RegisterNetEvent('qb-hud:send_fuel_bill', function(pCost)
...
function SpawnVehicle(vehicle, x,y,z, Fuel, customized, plate, IsViewing, enginehealth, bodyhealth)
  if DoesEntityExist(spawningVeh) then
	...
	Citizen.CreateThread(function()			
        ...
        exports['qb-vehicles']:SetVehicleIdentifier(spawningVeh, vid)
...
RegisterNUICallback('sethGps', function(data)
 local house = exports["qb-housing"]:retrieveHousingTableMapped()
...
RegisterNUICallback('unlockHouses', function(data)
  ...
  if statuus == "Unlock" then
    --TriggerEvent('qb-phone:h-Lock', hId,"Lock")
  else
    --TriggerEvent('qb-phone:h-Lock', hId,"Unlock")
  end
  TriggerServerEvent('qb-phone:house:status',hId,statuus)
  Wait(200)
  TriggerEvent('updateUIHousing')
end)

RegisterNetEvent("qb-phone:h-Lock", function(hId, status)
  TriggerServerEvent("qb-phone:h-Lock:sv", hId, status)
...
RegisterNUICallback('editHousingOwn', function()
  -- if exports['qb-housing']:isNearProperty() then
    if exports["qb-housing"]:canEdit() then
      local isComplete, _propertyID, dist, zone = exports['qb-housing']:findClosetedPerson()
      TriggerEvent("qb-housing:editOptions",{action = "enter", propertyId = _propertyID})
    else
      SendNUIMessage({
...
RegisterNUICallback('sellOwnHouse', function(data)
  local price = RPC.execute('qb-phone:getHousePrice', data.hid)
end)

RegisterNUICallback('giveAccess', function(data)
  local complete = RPC.execute('qb-phone:GiveAccess', data.hid,data.cid)
...
RegisterNUICallback('houseAccess', function(data)
  ...
  local access = RPC.execute('qb-phone:getAccessHouse', data.hid)
  local data = {
      {
        title = "Close",
        action = "qb-housing:close_access",
        key = {},
    },
  }
  local player = {}
  for i,k in pairs(access) do
    data[#data + 1] = {
          ...
          key = {"lockdown"},
          -- action = "qb-ui:apartmentsContext"
          children = {
            {
                title = "Remove Key",
  ...
  exports["qb-contexts"]:showContext(data)
end)

AddEventHandler("qb-housing:close_access", function()
  exports["qb-contexts"]:hideContext()
end)

AddEventHandler('qb-ui:apartmentsContext', function(data)
  RPC.execute("qb-phone:removeKeys",data[2],data[1])
  exports["qb-ui"]:hideContext(data)
end)
...
function hasDecrypt2()
    if exports["qb-inventory"]:hasEnoughOfItem("vpnxj",1,false,true) then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["qb-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["qb-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["qb-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["qb-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["qb-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["qb-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["qb-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      exports["qb-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
      exports["qb-inventory"]:hasEnoughOfItem("stolens8",1,false) or
      exports["qb-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
      exports["qb-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
      exports["qb-inventory"]:hasEnoughOfItem("assphone",1,false) or
      exports["qb-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
function hasRadio()
    if (exports["qb-inventory"]:hasEnoughOfItem("radio",1,false) or exports["qb-inventory"]:hasEnoughOfItem("pdradio",1,false)) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
local function MiniChecks ()
  ...
  return isREAgent,device,decrypt,decrypt2,trucker,canCreateRaces,hasUsbRaces,hasAlias
end

local isBurner, pCitizenID
function openGui(isBurner,cid,pNumber)
   TriggerEvent("qb-spawn:phonerefresh") --Refreshes jobs
  ...
  if hasPhone() then
    ...
    local near = exports['qb-housing']:isNearProperty()
    local house = RPC.execute("qb-phone:getHouses")
    local phoneBGrep = GetResourceKvpString(exports['isPed']:isPed('cid').."-bg")

    if phoneBGrep == " " or phoneBGrep =="" or phoneBGrep == nil or not phoneBGrep then
      phoneBGrep = "https://i.imgur.com/gKCPMcu.png"
    else
    ...
    local isREAgent,device,decrypt,decrypt2,trucker,canCreateRaces,hasUsbRaces,hasAlias = MiniChecks()
    ...
    SendNUIMessage({
      ...
      hasDevice = device, 
      hasDecrypt = decrypt, 
      pNum = RPC.execute("qb-phone:getPhoneNumber") or pNumber, -- Do later 
...
function canCreateRace()
  local racingCreateUsbItem = exports["qb-inventory"]:GetInfoForFirstItemOfName("racingusb0")
...
function canOpenRaceApp()
  local racingUsbItem = exports["qb-inventory"]:GetInfoForFirstItemOfName("racingusb2")
...
function canJoinRaces()
  local hasRaceUsbAndAlias = exports["qb-racing"]:getHasRaceUsbAndAlias()
...
function closeGui()
  ...
  -- TriggerEvent('qb-phone:UpdateStatePhone')
  pPhoneOpen = false
  insideDelivers = false
end

-- RegisterNetEvent('qb-phone:UpdateStatePhone')
-- AddEventHandler('qb-phone:UpdateStatePhone', function()
--     Wait(5)
...
RegisterNUICallback('updateMyWallpaper', function(data, cb)
  wallPaper = ""
  Wait(5)
  TriggerEvent('qb-phone:grabBackground', data.name .."?auto=compress&cs=tinysrgb&h=350")
...
AddEventHandler('phone:casino_paid', function(message)
  if exports['qb-phone']:pOpen() == false then 
    SendNUIMessage({openSection = "emailnotify", pEHandle = 'Payment Received.', pEMessages = message})
    SendNUIMessage({openSection = "phonemedio", timeout = "5200", pOpen = exports['qb-phone']:pOpen()}) 
  end
...
RegisterNetEvent("phone:sendThePM", function(name, msg)
  if exports["qb-inventory"]:hasEnoughOfItem("thepmdongle",1,false,true) then
...
RegisterNetEvent('qb-spawn:confirm_spawned')
AddEventHandler('qb-spawn:confirm_spawned', function()
  serverSpawn = true
end)
...
RegisterNUICallback('jobcenter-setgps', function(data)	
  print('GPSDATA',json.encode(data))
  ...
  elseif data.item == "dodo" then	
    ...
    elseif data.item =='darkmarket' then
      SetNewWaypoint(-225.95, -1329.81)	
      TriggerEvent("DoLongHudText", "GPS Set!")	
  end	
  	
  if data.item == "oxy" then	
    TriggerEvent("DoLongHudText", "GPS Set!")  	
    SetNewWaypoint(-1564.08, -440.30)	
    TriggerServerEvent("qb-deliveries:JobCenter")	
...
RegisterCommand('house', function()
  local isComplete, propertyID, dist, zone = exports['qb-housing']:findClosetedPerson()
  print("HOUSING",isComplete, propertyID, dist, zone)
end)
RegisterNUICallback('checkHouse', function()
  
 -- if exports["qb-housing"]:canEdit() then
    TriggerEvent('qb-housing:phone:check')
 -- else
 --   SendNUIMessage({
 --     openSection = "noProperty"
 --   })
...
RegisterNUICallback('leaveEditMode', function()
  TriggerEvent('qb-housing:editOptions', { action = "exit" })
end)
...
RegisterNUICallback('openFurnitureUI', function()
  ...
  TriggerEvent('qb-housing:editOptions', { action = "furniture" })
end)

RegisterNUICallback('setBackdoorUI', function()
  TriggerEvent('qb-housing:editOptions', { action = "backdoor" })
end)

RegisterNUICallback('setWardrobeUI', function()
  TriggerEvent('qb-housing:editOptions', { action = "wardrobe" })
end)

RegisterNUICallback('setCraftingUI', function()
  TriggerEvent('qb-housing:editOptions', { action = "crafting" })
end)

RegisterNUICallback('setGarageUI', function()
  TriggerEvent('qb-housing:editOptions', { action = "garage" })
end)

RegisterNUICallback('setStashUI', function()
  TriggerEvent('qb-housing:editOptions', { action = "stash" })
end)

RegisterNUICallback('sellsHouse', function()
  TriggerEvent('qb-housing:phone:check',"edit")
end)

AddEventHandler('qb-phone:sentHouse', function(hasPrice,id,street,price,category)
  print("isRealEstateAgent",isRealEstateAgent(),id,street,price)
  SendNUIMessage({
...
AddEventHandler('qb-phone:editingHouse', function(id,street,price)
  SendNUIMessage({
...
RegisterNUICallback('purchaseHouse', function(data)
  print('purchaseHouse',data.name,data.price)
  if rentEnabled then
   --   TriggerEvent('qb-phone:sentHouse')
      local success, msg = RPC.execute("qb-housing:buy", data.hid, data.price, 0, exports['isPed']:isPed("fullname"))
      TriggerEvent("DoLongHudText",msg,1)
     return
  end
...
-- AddEventHandler('qb-phone:noProperty', function()
...
RegisterNUICallback('editHousePrice', function(data)
  TriggerServerEvent('qb-housing:editPrice', data.hid,data.price)
end)

AddEventHandler('qb-phone:EnableSellRent', function()
    exports['qb-ui']:openApplication('textbox', {
    callbackUrl = 'qb-phone:SellRentEnable',
...
RegisterInterfaceCallback("qb-phone:SellRentEnable", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  TriggerServerEvent('qb-housing:rent_activate_sv', data[1].value)
  exports["qb-ui"]:hideContextMenu()
  cb({ data = {}, meta = { ok = true, message = '' } })
end)
...
RegisterNetEvent('qb-phone:signInInfoTowing')
AddEventHandler('qb-phone:signInInfoTowing', function()
  phoneNotification("email", "Grab the towtruck parked in the bay.", "Job Center")
end)

RegisterNetEvent('qb-phone:towing_offer')
AddEventHandler('qb-phone:towing_offer', function()
...
RegisterNUICallback('accept_tow_job', function(data)
  TriggerEvent('qb-jobs:towing_accept')
end)

-- 24/7 Delivery --

RegisterNetEvent('qb-phone:signInInfoDelivery')
AddEventHandler('qb-phone:signInInfoDelivery', function()
  phoneNotification("email", "Signed in as 24/7 Delivery Driver.", "Job Center")
end)

RegisterNetEvent('qb-phone:delivery_offer')
AddEventHandler('qb-phone:delivery_offer', function()
...
RegisterNUICallback('accept_delivery_job', function(data)
    TriggerEvent('qb-jobs:deliveries:getCar')
end)

-- Dodo Logistics --

RegisterNetEvent('qb-phone:signInDodo')
AddEventHandler('qb-phone:signInDodo', function()
  phoneNotification("email", "Signed in as a Dodo Logistics worker.", "Job Center")
end)

RegisterNetEvent('qb-phone:dodoJobOffer')
AddEventHandler('qb-phone:dodoJobOffer', function()
...
RegisterNUICallback('accept_dodo_job', function(data)
    TriggerEvent('qb-jobs:spawn-truck')
end)

RegisterNetEvent('qb-phone:goToLocation')
AddEventHandler('qb-phone:goToLocation', function()
  phoneNotification("email", "Go to the delivery location on the GPS.", "Job Center")
end)

RegisterNetEvent('qb-phone:goBackDepo')
AddEventHandler('qb-phone:goBackDepo', function()
  phoneNotification("email", "Return back to the depo.", "Job Center")
end)

-- House Robberies --

RegisterNetEvent('qb-phone:signInInfoHouseRobbery')
AddEventHandler('qb-phone:signInInfoHouseRobbery', function()
  phoneNotification("email", "Signed in as Criminal.", "Job Center")
end)

RegisterNetEvent('qb-phone:house_rob_offer')
AddEventHandler('qb-phone:house_rob_offer', function()
...
RegisterNUICallback('accept_house_robbery', function(data)
    TriggerEvent('qb-activities:acceptJob')
end)

-- Chopshop --

RegisterNetEvent('qb-phone:signInInfoChopshop')
AddEventHandler('qb-phone:signInInfoChopshop', function()
  phoneNotification("email", "Signed in as Chopshop.", "Job Center")
end)

RegisterNetEvent('qb-phone:chop_shop_offer')
AddEventHandler('qb-phone:chop_shop_offer', function()
...
RegisterNUICallback('accept_chop_shop', function(data)
    TriggerEvent('qb-chopshop:acceptOffer')
end)

-- Oxy Runs --
RegisterNetEvent('qb-phone:oxyRunNoti')
AddEventHandler('qb-phone:oxyRunNoti', function(pText)
  phoneNotification("email", pText, "Job Center")
end)

RegisterNetEvent('qb-phone:oxyrun:offer')
AddEventHandler('qb-phone:oxyrun:offer', function()
...
RegisterNUICallback('accept_oxy_run', function(data)
    TriggerEvent('qb-oxy:Accept:Offer')
end)

-- Paycheck --

RegisterNetEvent('qb-phone:paymentNoti')
AddEventHandler('qb-phone:paymentNoti', function(pAmt, pType)
  phoneNotification("email", "$"..pAmt..".00 has been added to your "..pType.. " Balance", "Job Center")
end)

RegisterNetEvent('qb-phone:businessRegisterNoti')
AddEventHandler('qb-phone:businessRegisterNoti', function(pBusinessName, pText)
  phoneNotification("charge", pText, pBusinessName)
end)

-- Fishing --

RegisterNetEvent('qb-phone:signInFishing')
AddEventHandler('qb-phone:signInFishing', function()
