...
function loadSMS(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("qb-phone:getSMS",cid)
  local _messages = {}
...
function loadSMSBurner(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("qb-phone:getTextsFromNumber",cid)
  local _messages = {}
  for i, v in ipairs(messages) do
      local name = ""
...
local data = message
local app = "sms"
if exports["qb-phone"]:phasPhone() then
  if exports['qb-phone']:pNotify() == true then
