...
RegisterNUICallback('btnTwatter', function()
    ...
    local twattermessages = RPC.execute("qb-phone:getTwatters")
    -- if not getTwat then
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
    ...
    TriggerServerEvent('qb-phone:twatSendNotification', handle, data, data.time)
...
RegisterNUICallback("qb-phone:getTwatter", function(data, cb)
  cb({ data = currentTwats, meta = { ok = success, message = (not success and currentTwats or 'done') } })
end)



RegisterNetEvent('qb-phone:insertTwat')
AddEventHandler('qb-phone:insertTwat', function(name,msg,time,attach)
  table.insert(currentTwats, {
    handle = name,
    message = msg,
    attachment = attach,
