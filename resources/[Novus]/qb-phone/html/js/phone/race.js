...
$('.submit-crace').click(function(){
   ...
   let bubble = $("#bubble-remove").is(":checked");
    ...
    $.post("https://qb-phone/racing:event:start", JSON.stringify({
      id: id,
...
//    console.log("SUBMIT HERE")
   ...
   setTimeout(() => {
      $.post("https://qb-phone/racing:map:save", JSON.stringify({
...
//    console.log("PENDING ENTRIES",pendingRaces.parent().has('.component-paper').length)
   ...
   $('.pGPS').click(function(){
      ...
      $.post('http://qb-phone/qb-phone:racing:setGPS', JSON.stringify({
         mID: mID,
     }));
   })
   $('.rPreview').click(function(){
      ...
      $.post('http://qb-phone/qb-phone:racing:seePreview', JSON.stringify({
         mID: mID,
     }));
   })
...
$('.pending-entries').on('click', '.pJoin',function(){
   ...
   $.post('http://qb-phone/racing:event:join', JSON.stringify({
      id: eID
   }));
});

$('.pending-entries, .active-entries').on('click', '.pLeave', function(){
   $.post('http://qb-phone/racing:event:leave', JSON.stringify({}));
});
...
$('.pending-entries, .active-entries').on('click', '.pEndRace',function(){
   $.post('http://qb-phone/racing:event:end', JSON.stringify({}));
});
