...
$(document).ready(function() {
    window.onkeydown = function(Ghost) {
        var tab = 9
        if (Ghost.keyCode === tab) {
...
$(document).ready(function () {
    ...
    $.post('http://qb-phone/getWeather', JSON.stringify({}));
    ...
    $(".phone-screen").on('click', '#wifi', function (e) {
        if (wifizone){
            if (enablePassword) {
                ...
                $.post('https://qb-phone/connectWifi', JSON.stringify({}));
            }
        }
    });
    setInterval(function () {
        ...
        if (actionButton !== undefined) {
            switch (actionButton) {
                case "home":
                    ...
                    $.post('https://qb-phone/refreshMySMS', JSON.stringify({status: false}));
                    $.post('https://qb-phone/closeTwat', JSON.stringify({}));
                    $.post('https://qb-phone/checkNotify', JSON.stringify({}));
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://qb-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://qb-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://qb-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://qb-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        if (!isBurnah) {
                            $.post('http://qb-phone/' + action, JSON.stringify({}));
                        } else if (isBurnah) {
                            $.post('http://qb-phone/getRecentCallsByCid', JSON.stringify({
                                cid: burnahCid
                            }));
                        }
                    break;
                ...
                case "doj-app":
                    ...
                    $.post('https://qb-phone/getActiveDOJ', JSON.stringify({}))
                    break;
                case "camera-app":
                    $(".camera-inside").empty();
                    openContainer('camera')
                    $.post('https://qb-phone/getCurrentCameras', JSON.stringify({}))
                    break;
                case "lsbn":
                    openContainer('lsbn');
                    break;
                ...
                case "btnBankBusters":
                    ...
                    $.post('https://qb-phone/getCurrentBanks', JSON.stringify({}))
                    setTimeout(() => {
                        $.post('https://qb-phone/inBankBusterGroup', JSON.stringify({}))
                    }, 1500)
//                    $.post('https://qb-phone/getCurrentBanks', JSON.stringify({}))
                 //   $.post('https://qb-phone/sendNotifAlert', JSON.stringify({
                 //       type: "alert"
                 //   }))
                    break;
                case "documents":
                    ...
                    $.post('https://qb-phone/getDocuments', JSON.stringify({
                        pType: txt
                    }))
                    break;
                case "diamond":
                    ...
                    $.post('https://qb-phone/getDiamondSportsEvents', JSON.stringify({}))
                    break;
                default:
                    $.post('http://qb-phone/' + action, JSON.stringify({}));
                    break;
    ...
    $(".document-submit-cid").click(function (e) {
        ...
        setTimeout(() => {
            ...
            $.post('https://qb-phone/shareDocumentPermanent', JSON.stringify({
                documentID: pDocID,
                targetCID: ptCID
            }))        
        })
    ...
    $('.btn-share-local').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://qb-phone/shareDocumentLocal', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-drop-qr-code').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://qb-phone/dropQRCode', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-delete-document').click(function(){
        ...
        setTimeout(() => {
            ...
            $.post('https://qb-phone/deleteDocument', JSON.stringify({
...
function createDiamond(events, isEmployed) {
    $.each(events, function (__, event) {
        ...
        $('#delete-diamondEvent').click(function(){
            ...
            $.post('http://qb-phone/deleteDiamondEvent', JSON.stringify({
...
function createDOJDepartments(doj) {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.openPhone === true) {
            ...
            $.post('http://qb-phone/checkModel', JSON.stringify({}));

            $.post('http://qb-phone/retreivePhoneContacts', JSON.stringify({}));
...
//                $(".send-message").show();
        ...
        switch (item.openSection) {
            ...
            case "messageRead":
                ...
                $.post('https://qb-phone/refreshMySMS', JSON.stringify({status: true}));
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://qb-phone/race:completed', JSON.stringify({
...
function burnahMessages() {
    $.post('https://qb-phone/burnahMessages', JSON.stringify({
        cid: burnahCid
    }))
}
...
$('.btn-racing-clear').click(function() {
    $.post('http://qb-phone/racing:map:removeBlips', JSON.stringify({}));
});
...
function createCamera(cameras) {
    ...
    $(".eye-camera").click(function () {
        ...
        $.post('https://qb-phone/OpenCamera', JSON.stringify({
            cameraId: cameraId
        }));
    })
    ...
    $(".remove-camera").click(function() {
        ...
        $(".removeCamera").empty();
        $.post("https://qb-phone/getCameraAccessPlayers", JSON.stringify({
            cameraId: cameraId
        }))
    })
};
...
$("#add-camera-submit").click(function() {
    ...
    complateInputQuick()
    setTimeout(() => {
        $.post("https://qb-phone/addUserToCamera", JSON.stringify({
            cameraId: camid,
...
$("#remove-camera-submit").click(function() {
    ...
    setTimeout(() => {
        $("#cameraRemoveID").val("");
        $.post("https://qb-phone/removeUserFromCamera", JSON.stringify({
            cameraId: camid,
            targetPhoneNumber: cid
        }))
    }, 1500)
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://qb-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});
...
function gpsJobCenter(data) {
    $.post(
      'https://qb-phone/jobcenter-setgps',
...
">${moment.utc(emails[email].time).local().fromNow()}
    ...
    $.post('http://qb-phone/emailReads', JSON.stringify({}));
}
...
$('.diamondsports-createevent').on('click', '#submit-input', function(){ 
    ...
    $.post('http://qb-phone/submitNewEventDiamondSports', JSON.stringify({
...
function openBrowser(url) {
    ...
    $.post('http://qb-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function purchaseDarkMarket(data) {
    itemamount = 1
    $.post("https://qb-phone/purchase-darkMarket", JSON.stringify({item:data,itemamount}))
}
function trackvehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://qb-phone/vehtrack", JSON.stringify({vehplate}))
}

function spawnVehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://qb-phone/vehspawn", JSON.stringify({vehplate}))
}
...
$('.sellingvehicle').on('click', '#submit-input', function(){
    ...
    complateInput();
    setTimeout(() => {
        $.post("https://qb-phone/sellVeh", JSON.stringify({
            plate: vplate,
            paypal: paypal,
            amount: amount
        }))
...
//     var notiElement = $(`</div><div class="top-notifications-email" style="max-height: 80px; display: flex;  ;"><div class="notification-container-email slideoutnotify slideinnotify" style="display: block; right: 55px;"><div class="app-bar-email"><div class="icon-twitter"><div class="emailicon" title="Email" id="icon-noti" style="background: url('https://gta-assets.Karambit.net/images/phone-icons/conversations.png') 0% 0% / cover no-repeat;height: 18px; width: 18px; bottom: 10px; left: 2px;">
...
//     var notiElement = $(`</div><div class="top-notifications-email" style="max-height: 80px; display: flex;  ;"><div class="notification-container-email slideoutnotify slideinnotify" style="display: block; right: 55px;"><div class="app-bar-email"><div class="icon-twitter"><div class="emailicon" title="Email" id="icon-noti" style="background: url('https://gta-assets.Karambit.net/images/phone-icons/calls.png') 0% 0% / cover no-repeat;height: 18px; width: 18px; bottom: 10px; left: 2px;">
...
$('#submit-input-the-pm').on('click', function() {
    ...
    setTimeout(() => {
        $('#the-pm-input').val("")
        $.post("https://qb-phone/sendThePM", JSON.stringify({
            thePM: misinput
        }))
    }, 1900)
})
...
$("#contact-submit-input").click(function (e) {
    ...
    setTimeout(() => {
        ...
        $.post('http://qb-phone/newContactSubmit', JSON.stringify({
            name: clean,
...
$(document).ready(function () {
    ...
    $('.send-ping').click(function () {
        ...
        // $.post('http://qb-phone/btnCamera', JSON.stringify({}));
        $.post('http://qb-phone/qb-ui:pingSend', JSON.stringify({
            number: targetId
        }));
        $('.send-value').val('');
    });
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://qb-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate", controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://qb-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://qb-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://qb-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://qb-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://qb-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://qb-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://qb-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://qb-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://qb-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-edit-submit").click(function (e) {
    ...
    $.post('http://qb-phone/editContactSubmit', JSON.stringify({
        name: clean,
    ...
    setTimeout(() => {
        $.post('http://qb-phone/refreshContacts', JSON.stringify({}));
    }, 2000);
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://qb-phone/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('https://qb-phone/stocksTradeToPlayer', JSON.stringify({
...
//     event.preventDefault();
//     $.post('http://qb-phone/newTwatSubmit', JSON.stringify({
...
$(".submit-bg").click(function (event){
    if ($('#bg-set-body').val() == '') {
        ...
        $.post('http://qb-phone/submitBg', JSON.stringify({
            bg: bg
        }));
...
$('#message-submit').click(function (event){
    ...
    $.post('http://qb-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://qb-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            console.log('5543')
            ...
            setTimeout(function () {
                if (!isBurnah) {
                    $.post('http://qb-phone/messages', JSON.stringify({}));     
                } else if (isBurnah) {
                    $.post('http://qb-phone/getTextsByCid', JSON.stringify({
                        cid: burnahCid
                    }))
                }
            }, 300);
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://qb-phone/btnEvictHouse', JSON.stringify({}));
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://qb-phone/racing:map:cancel', JSON.stringify({}));
});
...
$('#racing-start-tracks').on('change', function (e) {
    ...
    if(maps[selectedMap] !== undefined) {
        $.post('http://qb-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://qb-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
//     $.post('http://qb-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://qb-phone/racing:map:save', JSON.stringify({
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://qb-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$("#phone-submit-number").click(function () {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            $.post("https://qb-phone/getCallHistory", JSON.stringify({}))
        }, 300)
    }, 900)
})
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://qb-phone/btnNotifyToggle', JSON.stringify({}));
});



$('.account-information-toggle-pager').click(function () {
    $.post('http://qb-phone/btnPagerToggle', JSON.stringify({}));
...
function clickImage(img) {
    $('.selfieBox').click(function(){
        copyToClipboard(img);
        $.post('http://qb-phone/closeSelfieBox', JSON.stringify({}));
    });
}

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://qb-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://qb-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://qb-phone/removeHouseKey', JSON.stringify({
...
$('.group-manage-entries').on('click', '.group-manage-fire', function () {
    ...
    $.post('http://qb-phone/qb-phone:fireEmp', JSON.stringify({ id: $(this).data('id'), group: $(this).data('group'), name: $(this).data('name') }));
});

$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://qb-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://qb-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://qb-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://qb-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://qb-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://qb-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://qb-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://qb-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.jss1481, .calls-container').on('click', '.btn-contacts-call', function () {
    $.post('http://qb-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
...
$('#contacts-delete-submit').click(function (event) {
    ...
    complateInputQuick();
    setTimeout(() => {
        $.post('http://qb-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
    }, 1500)
});
...
$('.delete-yp').click(function (event) {
    ...
    setTimeout(() => {
        ...
        $('#youradhere').hide();
        hasPostedYP = false;
        $.post('http://qb-phone/deleteYP', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
    }, 1500)
});
...
$("#h-check").click(function () {
    ...
    $.post('http://qb-phone/checkHouse', JSON.stringify({}))
})

$("#edit-h").click(function () {
    // console.log("CHECK PROPERTY")
    // $.post('http://qb-phone/editHouse', JSON.stringify({}))
    //noPropty()
})

$("#sell-h").click(function () {
    // console.log("CHECK PROPERTY")
    $.post('http://qb-phone/sellsHouse', JSON.stringify({}))
})
...
$('.h-found').on('click', '.h-found-purchase', function (event) {
    ...
    complateInput();
    $('#twat-modal').modal('close');
    $.post('http://qb-phone/purchaseHouse', JSON.stringify({
...
$('.sell-house-own').on('click', '.confirm-sellHouse', function (event) {
    ...
    $.post('http://qb-phone/sellOwnHouse', JSON.stringify({
        hid: $('#h-p-id').val()
    }))
})
...
$('#submit-h-price').click(function(){
    ...
    $.post('http://qb-phone/editHousePrice', JSON.stringify({
        hid: hid,
        price: price
    }))
    $('.housing-edit-price').css('display', 'none');
...
function openPhoneShellDev() {
    ...
    $('.jss13').css('display', 'flex')
    $('#contacts-container').css('display', 'none')
    $('.status-bar-player-id').text("#Karambit");
    openContainer("home")
    $('#racing-app').css('display', 'block')
    $('.racing-apps').css('display', 'block');
    $('.create-r-m').css('display', 'block')
    ...
    var html = `<div camerahover=1 class="component-paper camera-count" style="width: 100%;">
        <div class="main-container">
            ...
            <div class="details ">
                <div class="title ">
                <p class="MuiTypography-root MuiTypography-body2 MuiTypography-colorTextPrimary"
                    style="word-break: break-word;">Karambit 101</p>
                </div>
            </div>
...
function createLSBNPostsDev() {
        ...
        <div class="main-container">
            ...
            <div class="title ">
                <p class="MuiTypography-root MuiTypography-body2 MuiTypography-colorTextPrimary"
                    style="word-break: break-word; text-align:left">Karambit Test</p>
                </div>
                <div class="description ">
                <p class="MuiTypography-root MuiTypography-body2 MuiTypography-colorTextPrimary"
                    style="word-break: break-word; text-align:right;">Karambit - 10 days ago</p>
                </div>
            </div>
        </div>
        </div>
...
function createDocumentsDev() {
    var html = `<div id="note-1" class="component-paper">
      <div class="main-container">
         <div class="details">
            <div class="title">
               <p class="MuiTypography-root MuiTypography-body2 MuiTypography-colorTextPrimary"
                  style="word-break: break-word;">Karambit OT</p>
            </div>
            <div class="description">
               <p class="MuiTypography-root MuiTypography-body2 MuiTypography-colorTextPrimary"
                  style="word-break: break-word;"></p>
   ...
   $("#note-1").click(function() {
      ...
      setTimeout(() => {
          ...
          document.getElementById("ppclankkv2").value = 'Karambit OT';
          tinymce.get('fuckthisseashit').setContent('Karambit Ipsum');
      }, 800)
    })
}
...
$('.phone-selfie').click(function(){
    $.post('http://qb-phone/phone:selfie', JSON.stringify({}));
    closePhoneShell()
})
...
//                 $.post('http://qb-phone/btnAnswer', JSON.stringify({}));
//             }
//         });
...
//                             setTimeout(() => {
//                                 Notif.addClass('notification-container-fade-out')
//                                 $.post('http://qb-phone/btnHangup', JSON.stringify({}));
//                             }, 1000)
//                         }, 200)
//                     }
//                     // Notif.addClass('notification-container-fade-out')
...
//             }
//             Notif.addClass('notification-container-fade-out')
//             $.post('http://qb-phone/pingEnded', JSON.stringify({}));
//             setTimeout(() => {
//                 Notif.remove()
//                 var count = 0
...
//             $.post('http://qb-phone/qb-ui:pingAccept', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
//             activePing = false
//             $.post('http://qb-phone/qb-ui:pingReject', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
$(".house-entries, .access-house-entries").on('click', '.h-setgps', function (e) {
    ...
    $.post('http://qb-phone/sethGps', JSON.stringify({hid: overid}));
})
...
$(".house-key-entries").on('click', '.h-removeKey', function (e) {
    ...
    $.post('http://qb-phone/whatthefuckdoesthisshitwork', JSON.stringify({cid: overid, hid: overid2}));
})
//a-signin
...
$('.a-signin').click(function(){
    if (cuunt <= 0) {
     ...
     $.post('http://qb-phone/abdulSignIn', JSON.stringify({status: taxiOn}));
 })
...
$(".house-entries, .access-house-entries").on('click', '.h-unlock', function (e) {
    ...
    $.post('http://qb-phone/unlockHouses', JSON.stringify({hid: overid, penisfuck: hstatus}));
 })
 ...
 $(".house-entries").on('click', '.h-edit-s', function (e) {
    ...
    $("#house-keys").hide();
    $("#house-2").hide();
    $.post('http://qb-phone/editHousingOwn', JSON.stringify({}));
 })
 ...
 $(".house-entries").on('click', '.h-access-keys', function (e) {
    ...
    $("#house-2").hide();
    $('.access-house-entries').css('display', 'none');
    $.post('http://qb-phone/btnKeyHolders', JSON.stringify({
        hid: overid
    }));
 ...
 $('.h-leaveedit').click(function(){
    editingHouse = false;
    $.post('http://qb-phone/leaveEditMode', JSON.stringify({}))
    $("#house-2").show();
    $("#house-3").hide();
    $("#house-keys").hide();
    $(".btn-2").show();
 ...
 $('.h-openfurniture').click(function(){
    $.post('http://qb-phone/openFurnitureUI', JSON.stringify({}))
 })

 $('.h-setbackdoor').click(function(){
    $.post('http://qb-phone/setBackdoorUI', JSON.stringify({}))
 })

 $('.h-setwardrobe').click(function(){
    $.post('http://qb-phone/setWardrobeUI', JSON.stringify({}))
 })

 $('.h-setcrafting').click(function(){
    $.post('http://qb-phone/setCraftingUI', JSON.stringify({}))
 })

 $('.h-setgarage').click(function(){
    $.post('http://qb-phone/setGarageUI', JSON.stringify({}))
 })

 $('.h-setstash').click(function(){
    $.post('http://qb-phone/setStashUI', JSON.stringify({}))
 })
 ...
 $('.submit-h-acess').click(function(){
    ...
    $('.phone-acces-house').hide()
    $.post('http://qb-phone/giveAccess', JSON.stringify({
        hid: hid,
        cid: cid
    }));
})
    ...
    $(document).ready(function () {
        $('.submit-settings').click(function(){
            ...
            $('.settings').hide()
                ...
                complateInput();
                $.post('http://qb-phone/submitSettings', JSON.stringify({
                    bg: bg,
                    model: model,
                    twat: twat,
                    call: call
...
function addDebtCars(vehData){ 
    ...
    $('.c-pawynow-button').click(function(){
        ...
        $.post("https://qb-phone/car_paid", JSON.stringify({
            plate:plate,
            payment:payment
        }))
        complateInputJustGreen();
        setTimeout(() => {
            $.post("https://qb-phone/btnDebt", JSON.stringify({}))
        }, 2000);
   })
