...
RPC.register("qb-phone:addToCurrentBanks", function(pSource, pBank)
...
RPC.register("qb-phone:getCurrentBanks", function(pSource)
    return currentBanks
end)

RPC.register("qb-phone:requestPlayerToGroup", function(pSource, pCid, pGroupID)
...
RPC.register("qb-phone:addPlayerToGroup", function(pSource, pCid, pGroupID, pSenderID)
...
RPC.register('qb-phone:removePlayerFromGroup', function(pSource, pCid, pGroupID)
...
RPC.register("qb-phone:createGroup", function(pSource)
...
RPC.register('qb-phone:deleteGroup', function(pSource, pGroupID)
    local jsonDecodedGroups = json.decode(groupTable)
    table.remove(jsonDecodedGroups, pGroupID)
    groupTable = json.encode(jsonDecodedGroups)
end)

RPC.register('qb-phone:getMyGroup', function(pSource, pCid)
    local jsonDecodedGroups = json.decode(groupTable)
    for k,v in pairs(jsonDecodedGroups) do
        local jsonDecodedGroupMembers = json.decode(v.members)
        for j,s in pairs(jsonDecodedGroupMembers) do
