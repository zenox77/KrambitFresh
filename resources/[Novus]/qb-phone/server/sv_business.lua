...
RegisterNetEvent('qb-phone:fireEmp')
AddEventHandler('qb-phone:fireEmp', function(id, job, name)
  local src = source
...
RPC.register("qb-phone:sendChargeCustomerNotif", function(pSource, BusinessName, GroupID, stateID, amount, reason)
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local tax = exports['qb-business']:taxTotal(amount)
    ...
    TriggerClientEvent("qb-phone:chargeCustomerNotif", tSrc, { 
...
RPC.register("qb-phone:chargeCustomerBus", function(pSource, amount, GroupID, stateID, sID)
    local amtTax = amount
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local user = exports['qb-base']:getModule("Player"):GetUser(tSrc)
    local char = user:getCurrentCharacter()
    ...
    if bank >= tonumber(amtTax) then
        ...
        TriggerClientEvent("qb-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They couldn't accept the payment")
        TriggerClientEvent("qb-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Not enough money to complete the payment.")
        return
    end
    TriggerClientEvent("qb-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Payment Successful.")
    TriggerClientEvent("qb-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "You have been deducted " .. amtTax .. " from you're bank account")
    TriggerClientEvent("qb-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They accepted the payment")
    return
end)
...
RegisterServerEvent('qb-phone:business_hired', function(StateID, Rank, pBusiness)
...
RegisterServerEvent('qb-phone:business_edit', function(pRank, pStateID, pPassType)
...
RPC.register("qb:getBusinessLogs", function(pSource, pBusiness)
...
RPC.register('qb-phone:business_paycheck', function(source,bus,p,a)
    local src = source
    local bId = bus
    local cid = p
    local amount = a
...
function getPlayerFromCid(zcid, amountAdded)
    for _, player in ipairs(GetPlayers()) do
        local user = exports["qb-base"]:getModule("Player"):GetUser(tonumber(player))
...
RPC.register("qb-business:payExternal", function(pSource, pCid, pAmount, pBusinessID)
...
RPC.register('qb-phone:bus_addBank', function(source,id,a)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local ply = user:getCurrentCharacter()
    local bId = id
...
RegisterServerEvent('qb-phone:business_fire', function(pBusinessName, pStateID)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
