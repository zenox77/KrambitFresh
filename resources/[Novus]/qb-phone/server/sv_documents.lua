...
local documents = {}
RPC.register('qb-phone:getDocuments', function(pSource, pType)
...
RPC.register("qb-phone:sendDocumentEvent", function(pSource, pTarget, pDocumentID)
    TriggerClientEvent("qb-phone:sendDocumentEvent", pTarget, pDocumentID)
end)

RegisterNetEvent("qb-phone:checkForNonDocumentedLicenses", function(pCid)
    local sex = "sex"
...
RPC.register("qb-phone:shareDocumentPermanent", function(pSource, dID, tCID)
...
RPC.register("qb-phone:acceptDocumentReceiveRequest", function(pSource, pDocumentID)
    local doc = MySQL.query.await([[
        SELECT * FROM __documents WHERE id = ?
    ]], {pDocumentID})
...
RPC.register('qb-phone:deleteDocument', function(pSource, pId)
    exports.oxmysql:execute("DELETE FROM __documents WHERE id = ?",{pId})
end)

RPC.register('qb-phone:updateDocument', function(pSource, pId, pTitle, pText)
...
RPC.register("qb-phone:createDocument", function(pSource, pOwner, pTitle, pText, pType)
    MySQL.query.await([[
