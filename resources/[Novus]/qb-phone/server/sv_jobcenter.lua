...
RPC.register("qb-phone:getIdleGroup", function(source)
    local src = source
    local idle = {}
...
RPC.register("qb-phone:getMyGroup", function(source)
    local src = source
    local idle = {}
    local g = MySQL.query.await([[
        SELECT * FROM job_group
...
RPC.register("qb-phone:c_group", function(source)
    local src = source
...
RPC.register("qb-phone:j_group", function(source, id)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
...
RPC.register("qb-phone:get_groupsMem", function(source, id)
...
RPC.register('qb-phone:g_ready', function(source,stat,id)
    local src = source
    local status = stat
    local gid = id
...
RPC.register('qb-phone:leave_group', function(source,id)
    local src = source
    local gId = id
