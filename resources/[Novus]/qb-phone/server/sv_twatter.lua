...
AddEventHandler('Tweet', function(handle, data, time)
  ...
  TriggerClientEvent('qb-phone:insertTwat', -1, handle,data.text,time,data.attachment)
  -- TriggerClientEvent('Client:UpdateTweet1', src, newtwat,allTweet)
 
end)

RegisterNetEvent('qb-phone:twatSendNotification')
AddEventHandler('qb-phone:twatSendNotification', function(handle, data, time)

  local newtwat = { ['handle'] = handle, ['message'] = data, ['time'] = time}
  TriggerClientEvent('qb-phone:TwatNotify', -1, newtwat)
end)

RegisterNetEvent('qb-phone:openGetTweets')
AddEventHandler('qb-phone:openGetTweets', function()
  local src = source
  exports.oxmysql:execute('SELECT * FROM (SELECT * FROM tweets ORDER BY `time` DESC LIMIT 50) sub ORDER BY time ASC', {}, function(tweets) -- Get most recent 100 tweets
...
RPC.register('qb-phone:getTwatters', function(source)
...
RPC.register("qb-phone:twitterhandle", function(pSource)
  local src = source
  local user = exports["qb-base"]:getModule("Player"):GetUser(src)
