RegisterNetEvent('qb-phone:transferWenmo', function(num,amount,comm)
    local src = source
    local zcid = getIdentifierByPhoneNumber(num)
    local user = exports["qb-base"]:getModule("Player"):GetUser(tonumber(src))
...
RPC.register('qb-phone:getWenmo', function(source)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(tonumber(src))
