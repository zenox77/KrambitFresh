Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['qb-interact']:AddPeekEntryByPolyTarget('qb-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "qb-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('qb-polychair:stopSitting')
    else
        ...
        TriggerEvent('qb-polychair:startSitting')
    end
    exports["qb-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('qb-polychair:chairSit', function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

RegisterNetEvent("qb-emotes:sitOnChair")
AddEventHandler("qb-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
