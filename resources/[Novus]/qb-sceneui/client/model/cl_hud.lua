...
--
local cmdsSet = {}
RegisterNUICallback("qb-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["qb-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["qb-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
    ...
    TriggerEvent("qb-preferences:setPreferences", data)
end)
...
AddEventHandler("qb-ui:watch", function()
...
RegisterNetEvent('qb-inventory:itemCheck')
AddEventHandler('qb-inventory:itemCheck', function (item, hasItem, quantity)
...
AddEventHandler('qb-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
AddEventHandler('qb-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('qb-boosting:client:setBoostCompletions')
AddEventHandler('qb-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('qb-weapons:client:setWeaponFireRate')
AddEventHandler('qb-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('qb-weapons:client:showWeaponFireRate')
AddEventHandler('qb-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('qb-ui:hud:forceShowMinimap', function(pForce)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["qb-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["qb-vehicles"]:CurrentFuel() or 0
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["qb-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["qb-vehicles"]:GetVehicleDegradation(veh)
...
AddEventHandler('qb-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["qb-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["qb-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("qb-island:onIsland", function(pState)
    onIsland = pState
end)
