...
local function drawStatusHUD(state, pValues)
  ...
  local balance = RPC.execute("qb-casino:getCurrentChipCount")
  local values = {}
  isSlotMachinesEnabled, currentJackPot = RPC.execute("qb-slotmachines-sv:checkIfSlotMachinesState")
  ...
  -- exports["qb-textui"]:sendAppEvent("status-hud", {
  --   show = state,
  --   title = title,
  --   values = values
  -- })
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("qb-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "qb-textui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "qb-textui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("qb-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["qb-interface"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["qb-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            elseif exports["qb-inventory"]:hasEnoughOfItem("casinovip", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            else
            ...
            exports["qb-interface"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end

        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("qb-slotmachines-sv:sitDownAtSlotMachine", k)
            break
          end
...
--#[Event Handlers]#--
RegisterNetEvent("qb-slotmachines-cl:getMachineData")
AddEventHandler("qb-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("qb-slotmachines-cl:updateMachineData")
AddEventHandler("qb-slotmachines-cl:updateMachineData", function(machine, ply)
  if currentSlotMachineObjs[machine] ~= nil then
    currentSlotMachineObjs[machine].ply = ply
  end
end)
-- Done by ghost :)
RegisterNetEvent("qb-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("qb-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("qb-slotmachines-cl:playSlotMachineSound")
AddEventHandler("qb-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("qb-slotmachines-cl:stopReels")
AddEventHandler("qb-slotmachines-cl:stopReels", function(reel,rot)
...
RegisterNetEvent("qb-slotmachines-cl:spinFinished")
AddEventHandler("qb-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    -- RPC.execute("qb-casino:winChips", amountWon)
...
RegisterNetEvent("qb-slotmachines-cl:toggleSlotMachines")
AddEventHandler("qb-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("qb-casino:casinoEnteredEvent", function()
  isInsideCasino = true
  isSlotMachinesEnabled, currentAccJackpot = RPC.execute("qb-slotmachines-sv:checkIfSlotMachinesState")
...
AddEventHandler("qb-casino:casinoExitedEvent", function()
  isInsideCasino = false
  isSlotMachinesEnabled = RPC.execute("qb-slotmachines-sv:checkIfSlotMachinesState")
  if not isSlotMachinesEnabled then
    return
  end
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("qb-textui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    local useChipsResult = RPC.execute("qb-casino:useChips", currentBet)
    ...
    --local info = (exports["qb-inventory"]:GetInfoForFirstItemOfName("casinoloyalty") or { information = "{}" })
    TriggerServerEvent(
      "qb-slotmachines-sv:spinMachine",
...
RegisterInterfaceCallback("qb-textui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
