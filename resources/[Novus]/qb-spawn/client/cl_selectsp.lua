...
AddEventHandler('spawn:clientSpawnData', function(spawnData, heheData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				Citizen.Wait(1500)
				TriggerEvent("qb-spawn:firstspawnyo")
			end
			return
		end
		...
		if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["qb-housing"]:retriveHousingTable()
		end
...
RegisterNetEvent("qb-spawn:firstspawnyo")
AddEventHandler("qb-spawn:firstspawnyo", function()
	if triggered then
		triggered = false
		if not exports["qb-inventory"]:hasEnoughOfItem("newaccountbox",1,false) then
			TriggerEvent("player:receiveItem","newaccountbox",1,true)
		end
	end
end)

RegisterNetEvent('qb-spawn:finishedClothingstuff')
AddEventHandler('qb-spawn:finishedClothingstuff', function()
	
end)

RegisterNetEvent('qb-spawn:finishedClothingstuffbish')
AddEventHandler('qb-spawn:finishedClothingstuffbish', function()
	
	if not exports["qb-inventory"]:hasEnoughOfItem("idcard",1,false) then
		TriggerEvent("player:receiveItem","idcard",1,true)
	end
	if not exports["qb-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
		TriggerEvent("player:receiveItem","mobilephone",1)
	end
	if not exports["qb-inventory"]:hasEnoughOfItem("lockpick",1,false)then
		TriggerEvent("player:receiveItem","lockpick",3)
	end
	if not exports["qb-inventory"]:hasEnoughOfItem("sandwich",3,false)then
		TriggerEvent("player:receiveItem","sandwich",1)
	end
	if not exports["qb-inventory"]:hasEnoughOfItem("water",1,false)then
		TriggerEvent("player:receiveItem","water",3)
	end
	if not exports["qb-inventory"]:hasEnoughOfItem("repairkit",1,false)then
...
function Spawn.getDevSpawn()
	...
	local devspawn = exports["storage"]:tryGet("vector4","fmdevspawn")
	if devspawn then
...
function Spawn.getBusinessSpawn(pBusinessCode, pBusinessName)
	...
	local hasPermission = RPC.execute("qb-business:hasPermission", pBusinessCode, "key_access")
...
function Spawn.getRoosterSpawn()
    ...
    local isRooster = exports["qb-business"]:IsEmployedAt("rooster")
	if isRooster then
		spawn = { ["pos"] = vector4(-172.83,331.17,93.76,266.08), ['info'] = ' Rooster Rest'}
	end
...
function Spawn.overwriteSpawn(overwrite)
	...
	elseif overwrite == 'jail' then
		pos = vector4(1802.51,2607.19,46.01,93.0) -- default prison 
	end
	...
	Wait(200)
	TriggerEvent("nopixel:raid_clothes_pedfix")
	DoScreenFadeIn(2500)	
	TriggerEvent('qb-spawn:spawnCharacter')
	-- Login.characterSpawned()
