...
RegisterServerEvent("qb-login:disconnectPlayer")
AddEventHandler("qb-login:disconnectPlayer", function()
    local src = source
    DropPlayer(src, "You DisConnected")
end)

RegisterServerEvent("qb-base:playerSessionStarted")
...
RegisterServerEvent("qb-spawn:licenses")
AddEventHandler("qb-spawn:licenses", function()
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qb-spawn:SetNewRouting')
AddEventHandler('qb-spawn:SetNewRouting', function(pRoutingNum)
    local src = source
    SetPlayerRoutingBucket(src, pRoutingNum)
    print('Spawn routing bucket: '..pRoutingNum)
end)

RegisterServerEvent('qb-spawn:GetRouting')
AddEventHandler('qb-spawn:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('qb-spawn:SetOldRouting')
AddEventHandler('qb-spawn:SetOldRouting', function()
    local src = source
...
RegisterServerEvent('qb-spawn:createCharacter:log')
AddEventHandler('qb-spawn:createCharacter:log', function(pFirstName, pSecondName, pDOB, pGender)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "**FM | Nopixel [qb-spawn] | Create Character Log **",
...
RPC.register('qb-spawn:isAdmin', function(src)
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
