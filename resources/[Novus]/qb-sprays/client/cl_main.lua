...
RegisterNetEvent('ghost_spray:setSprays')
AddEventHandler('ghost_spray:setSprays', function(s)
    SPRAYS = s
    SetSprayTimeCorrectColor()
end)

AddEventHandler('playerSpawned', function()
    TriggerServerEvent('ghost_spray:playerSpawned')
end)
    ...
    while true do
        ...
        if wantedSprayLocation and wantedSprayRotation and IsSpraying then
            ...
            if currentSprayRotation ~= wantedSprayRotation or not currentSprayRotation or ttl < 0 then
                ...
                print('currentComputedRotation',currentComputedRotation)
            end
        end
    end
end)

function GetRotationThruCamera(location, normal) -- This is the issue resolve asap..
    ...
    print('returning rot' , json.encode(rot)) -- this is vector3(0,0,0) ?? 

    if rot ==( vector3(0,0,0) ) then 
        rot = vector3(math.floor(math.random(1,2)),0,0)
        print('It was 0 0 0 now changed..')
        return rot
    end
