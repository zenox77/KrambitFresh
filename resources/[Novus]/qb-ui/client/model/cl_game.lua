...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        SendUIMessage({ source = "qb-nui", app = "character", data = character });
        
        local endpointData = exports["qb-fiber"]:GetGameDetails()
        SendUIMessage({ source = "qb-nui", app = "game", data = endpointData });

        Citizen.Wait(5000)

        TriggerEvent('qb-ui:phoneReady')
        --TriggerServerEvent('qb-ui:phoneReady')
    end)
end
...
function isGrapeBarrel(p2)
  ...
  AddEventHandler("qb-inventory:itemUsed", function(pItem, pData)
  ...
  AddEventHandler("qb-vineyard:pickupBarrel", function(p1, p2, p3)
    ...
    exports['qb-objects']:DeleteObject(p3.meta.id)
    TriggerEvent("player:receiveItem", "vineyardbarrel", 1, false, data, data)
  end)
  
  
  RegisterUICallback("qb-vineyard:changeBarrelName", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    exports["qb-ui"]:closeApplication("textbox")
    RPC.execute("qb-vineyard:changeBarrelName", data.key, data.values.name)
  end)
  
  RegisterUICallback("qb-vineyard:addGrapesToBarrel", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    exports["qb-ui"]:closeApplication("textbox")
    local newData = data.key.newData
    local objectId = data.key.objectId
    local items = exports["qb-inventory"]:getItemsOfType("vineyardgrapes", false, true)
    ...
    RPC.execute("qb-vineyard:changeBarrelData", objectId, newData)
  end)
  
  local function getTimestamp(data)
    local remaining = math.floor((exports["qb-misc-code"]:getCurrentTime() - data.created_at) / 1000)
  ...
  AddEventHandler("qb-vineyard:barrelAction", function(p1, p2, p3)
    if not isGrapeBarrel(p3) then return end
    local data = p3.meta.data.metadata
    ...
    if p1.action == "change_name" then
      exports["qb-ui"]:openApplication("textbox", {
        callbackUrl = "qb-vineyard:changeBarrelName",
        key = objectId,
        items = {
    ...
    if p1.action == "seal" then
      ...
      local newData = { sealed = true, created_at = exports["qb-misc-code"]:getCurrentTime() }
      RPC.execute("qb-vineyard:changeBarrelData", objectId, newData)
      return
    end
    ...
    if p1.action == "add_grapes" then
      ...
      if #items == 0 then return end
      exports["qb-ui"]:openApplication("textbox", {
        callbackUrl = "qb-vineyard:addGrapesToBarrel",
    ...
    if p1.action == "pour_bottles" then
      local hasEnough = exports["qb-inventory"]:hasEnoughOfItem("vineyardwinebottleempty", 1, false, true)
      if not hasEnough then
        TriggerEvent("DoLongHudText", "You need empty wine bottles!", 2)
        return
      end
      TriggerEvent("doAnim","cokecut")
      local finished = exports["qb-taskbar"]:taskBar(10000, "Bottling Wine")
      ClearPedTasks(PlayerPedId())
      if finished ~= 100 then return end
      exports['qb-objects']:DeleteObject(objectId)
  ...
  AddEventHandler("qb-inventory:attachmentsToggle", function(pEnabled, pId)
  ...
  Citizen.CreateThread(function()
    exports["qb-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
      {
        id = "peek_prop_vineyard_barrel",
        event = "qb-vineyard:pickupBarrel",
        icon = "shopping-basket",
        label = "Pick Up",
      },
      {
        id = "peek_prop_vineyard_barrel_details",
        event = "qb-vineyard:barrelAction",
        icon = "info-circle",
        label = "View Label",
    ...
    exports["qb-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
      ...
      isEnabled = function(p1, p2, p3)
        ...
        local hasEnough = exports["qb-inventory"]:hasEnoughOfItem("vineyardbarrelheadopener", 1, false, true)
        return hasEnough
      end,
    })
  end)
