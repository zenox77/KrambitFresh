...
function correct(){
    ...
    $.post('https://qb-ui/qb-ui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
const PostData = (data) => {
    return $.post(`https://qb-ui/dataPost`, JSON.stringify(data))
}
    ...
    switch (action) {
        ...
        case "eye":
            ...
            if (msg.data.action == "refresh") {
                    ...
                    $.each(payload.options, function (index2, item2) {
                        if(index1 == index2 && item2 == true){
                            ...
                            $("#target-"+index1).hover((e)=> {
                                $("#target-"+index1).css("color",e.type === "mouseenter"?"#48D1CC":"white")
                            })
                ...
                $.post('https://qb-ui/closecontext', JSON.stringify({}));
            }
            break;
        case "sniper-scope":
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://qb-ui/qb-ui:targetSelectOption', JSON.stringify({
            entity : entitydata,
            option :{
                event: eventData,
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        ...
        if (eyeintactive){
            eyeintactive = false
            $.post('https://qb-ui/closeeyeint', JSON.stringify({}));
        }
    }
});
        ...
        if (event.target.id == "backmenu") {
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (children) {
                    ...
                    $.post('https://qb-ui/'+action, JSON.stringify({
                        key:key
                    }));
                ...
                $(".buttoncontext").html("");
                $(".containercontext").hide();
                $.post('https://qb-ui/'+subaction, JSON.stringify({
                    key:subkey
                }));
            }
        }
