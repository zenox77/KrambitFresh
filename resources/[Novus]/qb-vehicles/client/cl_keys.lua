...
function HotwireVehicle(cb)
    ...
    for i = 1, 5 do
        ...
        local finished = exports["qb-ui"]:taskBarSkill(skill[1],  skill[2])
    ...
    exports["qb-flags"]:SetVehicleFlag(currentVehicle, "isHotwiredVehicle", true)
end
...
AddEventHandler("keys:give", function()
    ...
    if distance ~= -1 and distance < 5 then
        TriggerServerEvent("qb-vehicles:sendKeys", target, identifier, plate)
    else
...
Citizen.CreateThread( function()
    ...
    exports["qb-keybinds"]:registerKeyMapping("", "Vehicle", "Lock/Unlock Vehicle", "+toggleLock", "-toggleLock", "U")
                ...
                if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
                    if IsEntityDead(pedDriver) then
                        ...
                        if finished == 100 then
                            ...
                            exports["qb-flags"]:SetVehicleFlag(vehicle, "isStolenVehicle", true)
                        else
