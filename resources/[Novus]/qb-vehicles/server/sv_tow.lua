...
RegisterServerEvent("qb-vehicles:checkrepo")
AddEventHandler("qb-vehicles:checkrepo", function(plate)
    local src = source
...
-- RegisterServerEvent("qb-vehicles:repo")
-- AddEventHandler("qb-vehicles:repo", function(plate)
--     local src = source
-- 	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                         ['@repoed'] = "1"
--                     })
--                     TriggerClientEvent("qb-vehicles:repo:success", src)
...
-- RegisterServerEvent("qb-vehicles:judgerepo")
-- AddEventHandler("qb-vehicles:judgerepo", function(plate)
--     local src = source
-- 	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RegisterServerEvent("qb-vehicles:judgerepodelete")
-- AddEventHandler("qb-vehicles:judgerepodelete", function(plate)
--     local src = source
-- 	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ?", {char.id}, function(data)
...
-- RegisterServerEvent("qb-vehicles:repo2")
-- AddEventHandler("qb-vehicles:repo2", function(plate)
--     local src = source
-- 	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                     ['@vehicle_state'] = "In"
--                 })
--                 TriggerClientEvent("qb-vehicles:repo:success2", src)
--             else
--                 TriggerClientEvent("qb-vehicles:repo:success2", src)
--             end
--         end
...
-- RegisterServerEvent("qb-vehicles:release:vehicle")
-- AddEventHandler("qb-vehicles:release:vehicle", function(ServerID, plate)
...
--         ['repoed'] = "0"
--     })
--     exports['qb-exports']:removeCash(SrcID, 500)
...
RegisterCommand('towbill', function(source, args)
	...
	exports.oxmysql:execute('SELECT * FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'towunion' then
			...
			TriggerClientEvent("qb-vehicles:towBill", src, tonumber(args[2]))
		else
		end
	end)
end)