RegisterNetEvent("qb-weapons:attackedByCash")
AddEventHandler("qb-weapons:attackedByCash", function(pAttacker)
    local victim = source
    TriggerClientEvent("qb-weapons:hitPlayerWithCash", pAttacker, victim)
end)

RegisterNetEvent("qb-weapons:processGiveCashAmount")
AddEventHandler("qb-weapons:processGiveCashAmount", function(pTarget, pAmount, pId)
    local attacker = source
    local victim = pTarget
    local victimUser = exports["qb-base"]:getModule("Player"):GetUser(victim)

    victimUser:addMoney(pAmount)
            
    exports["qb-log"]:AddLog("Weapons", 
...
RegisterNetEvent('qb-weapons:getAmmo')
AddEventHandler('qb-weapons:getAmmo', function()
    local ammoTable = {}
    local src = source
	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    ...
    exports.ghmattimysql:execute("SELECT type, ammo FROM characters_weapons WHERE id = @id", {['id'] = char.id}, function(result)
        ...
        TriggerClientEvent('qb-items:SetAmmo', src, ammoTable)
    end)
end)

RegisterNetEvent('qb-weapons:updateAmmo')
AddEventHandler('qb-weapons:updateAmmo', function(newammo,ammoType,ammoTable)
    local src = source
	local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
