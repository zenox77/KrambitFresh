...
function addPlant(pPlant)
    exports["qb-polyzone"]:AddCircleZone("qb-weed:plant", pPlant.coords, 50, {
        --debugPoly = true,
        data = pPlant,
    })
...
function showPlantMenu(pPlantId)
    ...
    if growth < PlantConfig.HarvestPercent then
        context[#context+1] = {
            ...
            key = { id = pPlantId },
            action = 'qb-weed:addWater',
            disabled = not exports["qb-inventory"]:hasEnoughOfItem("water", 1, false),
        }

        context[#context+1] = {
            title  = 'Add Fertilizer',
            disabled = not exports["qb-inventory"]:hasEnoughOfItem("fertilizer", 1, false),
            description = "",
            children = {
                {
                    title = 'Add Fertilizer' .. ' (N)',
                    key = { id = pPlantId, type = "n" }, 
                    action = 'qb-weed:addFertilizer',
                },
                {
    ...
    if getStageFromPercent(growth) < 3 and plant.gender == 0 then
        context[#context+1] = {
            title = 'Add Male Seed',
            key = { id = pPlantId },
            action = 'qb-weed:addMaleSeed',
            description = 'Make the plant preggies',
            disabled = not exports["qb-inventory"]:hasEnoughOfItem("maleseed", 1, false)
        }
    end
    ...
    if myjob == "police" or myjob == "state" or myjob == "judge" then
        context[#context+1] = {
            ...
            description = "", 
            key = { id = pPlantId },
            action = 'qb-weed:removePlant',
        }
    end

    exports["qb-interface"]:showContextMenu(context);
end
...
AddEventHandler("qb-polyzone:enter", function(zone, data)
    if zone == "qb-weed:area" then
        inZone = true
    elseif zone == "qb-weed:plant" then
            ...
            if hit then
                ...
                for matHash,matType in pairs(MaterialHashes) do
                    if materialHash == matHash then
                        ...
                        local finished = exports["qb-taskbar"]:taskBar(15000, 'Planting Seed', false, true, false, false, nil, 5.0, PlayerPedId())
                        ClearPedTasks(PlayerPedId())
                        if finished == 100 then
                            FreezeEntityPosition(PlayerPedId(), false)
                            ClearPedTasks(PlayerPedId())
                            RPC.execute("qb-weed:plantSeed", endCoords, typeMod)
...
RegisterNetEvent("qb-weed:trigger_zone", function (type, pData)
...
AddEventHandler("qb-weed:checkPlant", function(pContext, pEntity)
    local plantId = false
...
RegisterInterfaceCallback("qb-weed:addWater", function (pParams)
    if not exports["qb-inventory"]:hasEnoughOfItem("water", 1, true, true) then return end
    exports['qb-interface']:closeApplication('contextmenu')
    playPourAnimation()
    local finished = exports["qb-taskbar"]:taskBar(2000, "Adding Water", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("qb-weed:addWater", pParams.key.id)
        if not success then
...
RegisterInterfaceCallback("qb-weed:addFertilizer", function (pParams)
    if not exports["qb-inventory"]:hasEnoughOfItem("fertilizer", 1, true, true) then return end
    exports['qb-interface']:closeApplication('contextmenu')
    playPourAnimation()
    local finished = exports["qb-taskbar"]:taskBar(2000, "Adding Fertilizer", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
            local success = RPC.execute("qb-weed:addFertilizer", pParams.key.id, pParams.key.type)
        if not success then
...
RegisterInterfaceCallback("qb-weed:addMaleSeed", function (pParams)
    if not exports["qb-inventory"]:hasEnoughOfItem("maleseed", 1, true, true) then return end
    exports['qb-interface']:closeApplication('contextmenu')
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["qb-taskbar"]:taskBar(3000, "Adding Male Seed", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("qb-weed:addMaleSeed", pParams.key.id)
        TriggerEvent("inventory:removeItem", "maleseed", 1)
    end
    showPlantMenu(pParams.key.id)
end)

RegisterInterfaceCallback("qb-weed:removePlant", function (pParams)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["qb-taskbar"]:taskBar(3000, "Removing", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getFertilizer = getPlantGrowthPercent(weedPlants[pParams.key.id]) > 20.0
        local success = RPC.execute("qb-weed:removePlant", pParams.key.id, getFertilizer)
        if not success then
...
AddEventHandler("qb-weed:pickPlant", function(pContext, pEntity)
    local plantId = false
    ...
    local plyWeight = exports["qb-inventory"]:getCurrentWeight()
    ...
    TriggerEvent("animation:PlayAnimation", "layspike")
    local finished = exports["qb-taskbar"]:taskBar(10000, "Harvesting", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("qb-weed:harvestPlant", plantId)
    end
end)
...
Citizen.CreateThread(function()
    exports["qb-polyzone"]:AddPolyZone("qb-weed:area", {
    ...
    local _plants = RPC.execute("qb-weed:getPlants")
