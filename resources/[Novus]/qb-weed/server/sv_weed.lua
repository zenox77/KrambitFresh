...
RPC.register("qb-weed:getPlants", function(src)
    return weedPlants
end)

RPC.register("qb-weed:plantSeed", function(src, pCoords, pStrain)
    local pGender = 0
    ...
    TriggerClientEvent("qb-weed:trigger_zone", -1, 1, data)
    return true
end)

RPC.register("qb-weed:addWater", function(src, pPlantId)
    ...
    TriggerClientEvent("qb-weed:trigger_zone", -1, 2, weedPlants[pPlantId])
    return true
end)

RPC.register("qb-weed:addFertilizer", function(src, pPlantId, pType)
    local plant = weedPlants[pPlantId]
...
RPC.register("qb-weed:addMaleSeed", function(src, pPlantId)
    local plant = weedPlants[pPlantId]
...
RPC.register("qb-weed:removePlant", function(src, pPlantId, pFertilizer)
    local plant = weedPlants[pPlantId]
    ...
    TriggerClientEvent("qb-weed:trigger_zone", -1, 3, { id = pPlantId })
    return true
end)

RPC.register("qb-weed:harvestPlant", function(src, pPlantId)
    local plant = weedPlants[pPlantId]
        ...
        for id, plant in pairs(weedPlants) do
            if (time - plant.timestamp) >= (PlantConfig.LifeTime * 60) then
                ...
                if affectedRows and affectedRows > 0 then
                    weedPlants[id] = nil
                    TriggerClientEvent("qb-weed:trigger_zone", -1, 3, plant)
                end
            end
        end
