...
function SyncWheelFitment()
    if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            ...
            TriggerServerEvent("qb-wheelfitment_sv:saveWheelfitment", plate, fitment)
        ...
        RPC.execute("qb-wheelfitment_sv:setIsWheelFitmentInUse", false)
    end
end
...
Citizen.CreateThread(function()
    isWheelFitmentInUse = RPC.execute("qb-wheelfitment_sv:getIsWheelFitmentInUse")
    isPlyWhitelisted = RPC.execute("qb-wheelfitment_sv:checkIfWhitelisted")

    exports["qb-polyzone"]:AddBoxZone("qb-wheelfitment:zone1", vector3(136.0, -3030.36, 6.52), 7.8, 4.2, {
        name="qb-wheelfitment:zone1",
        heading=1,
        --debugPoly=false
      })
end)
...
AddEventHandler("qb-polyzone:enter", function(zone, data)
    if zone ~= "qb-wheelfitment:zone1" then return end
    Citizen.CreateThread(function()
        local plyPed = PlayerPedId()
        inZone = true
        while inZone do
                ...
                if not isMenuShowing then
                    ...
                    if isNear(plyPos, cfg_wheelFitmentPos, 2.0) then
                        ...
                        if IsControlJustReleased(1, 38) then -- Key: E
                            ...
                            RPC.execute("qb-wheelfitment_sv:setIsWheelFitmentInUse", true)
...
RegisterNetEvent('qb-fitment:devaleks',function()
    devwheelMenu ()
end)


AddEventHandler("qb-polyzone:exit", function(zone)
    if zone ~= "qb-wheelfitment:zone1" then
        return
    end
    inZone = false
end)
...
AddEventHandler("qb-admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)

-- #[Event Handlers]#--
RegisterNetEvent("qb-wheelfitment_cl:applySavedWheelFitment")
AddEventHandler("qb-wheelfitment_cl:applySavedWheelFitment", function(plyVeh, VehicleWheel)
...
RegisterNetEvent("qb-wheelfitment_cl:forceMenuClose")
AddEventHandler("qb-wheelfitment_cl:forceMenuClose", function()
    if isMenuShowing then
        local plyPed = PlayerPedId()
        local plyVeh = GetVehiclePedIsIn(plyPed, false)
