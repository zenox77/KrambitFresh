...
AddEventHandler("qb-jobmanager:playerBecameJob", function(job, name, notify)
   ...
   -- TriggerServerEvent("qb-items:updateID",job,exports["isPed"]:retreiveBusinesses())
end)

RegisterNetEvent("qb-base:characterLoaded")
AddEventHandler("qb-base:characterLoaded", function(character)
    local LocalPlayer = exports["qb-base"]:getModule("LocalPlayer")
    LocalPlayer:setVar("job", "unemployed")

end)

RegisterNetEvent("qb-base:exportsReady")
