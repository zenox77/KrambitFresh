...
RegisterNetEvent('qb-base:PolyZoneUpdate')
AddEventHandler('qb-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "qb-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('qb-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('qb-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('qb-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('qb-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('qb-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('qb-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('qb-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('qb-polyzone:stop')
            end
        end
