...
function isPed(checkType)
    ...
    if checkType == "countems" then
      if not curEms or curEms == nil then return 0 end
        pass = curEms
    end
...
RegisterNetEvent("job:emscount")
AddEventHandler("job:emscount", function(activeEms)
  curEms = activeEms
end)
...
AddEventHandler('qb-base:addedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)

RegisterNetEvent('qb-base:removedMoney')
AddEventHandler('qb-base:removedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)
