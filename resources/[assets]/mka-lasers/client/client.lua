...
local function calculateCurrentPoint(fromPoint, toPoint, deltaTime, travelTimeBetweenTargets)
  local npdDirection = toPoint - fromPoint
  local npdDirectionDist = #npdDirection
  local percentOfTravelTime = deltaTime / (travelTimeBetweenTargets * 1000)
  local distance = math.min(npdDirectionDist * percentOfTravelTime, npdDirectionDist)
  return fromPoint + (norm(npdDirection) * distance)
end
