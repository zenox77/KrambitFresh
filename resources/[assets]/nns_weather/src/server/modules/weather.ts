...
onNet('nns_weather:admin:changeWeather', (pWeather)  => {
  const weather = pWeather
  ...
  console.log('[ADMIN]' + source + ' Setting weather to : ',pWeather)
...
RegisterCommand('weather', (source: string, args: string[]) => {
  if (!IsPlayerAceAllowed(source, "nns_weather.commands.weather")) {
    return emitNet('DoLongHudText', source, 'You do not have permissions to use this command.')
  }

  if (args.length === 0) {
    return emitNet('DoLongHudText', source, 'Format: /weather [type]')
  }
