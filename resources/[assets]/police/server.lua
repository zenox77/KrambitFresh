...
AddEventHandler('police:targetCheckBank', function(target)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(target)
...
RegisterServerEvent('police:IsTargetCuffed') -- that is qb's code ((sway))
...
function getmonth(month) -- that is qb's code ((sway))
    local months = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"}
    return months[tonumber(month)]
end

function ConvertDate(d) -- that is qb's code ((sway))
...
RegisterServerEvent('police:showPH') -- that is qb's code ((sway))
...
AddEventHandler('police:jailGranted', function(args)
    ...
    local target = exports["qb-base"]:getModule("Player"):GetUser(player)
    local character = target:getCurrentCharacter()
    local playerName = character.first_name .. ' ' .. character.last_name

    local pdunit = exports["qb-base"]:getModule("Player"):GetUser(src)
    if not pdunit:getVar("job") == "police" then
        local steamid, name = pdunit:getVar("name"), pdunit:getVar("steamid")

        exports["qb-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {
            target = playerName,
            reason = reason,
            cid = cid,
    ...
    TriggerEvent("qb:news:newConviction", {
        name = playerName,
        duration = time,
        charges = reason
    })
    ...
    exports["qb-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
...
AddEventHandler("police:deletecrimes", function(target)
    ...
    if not pdunit:getVar("job" == "police") then
        ...
        exports["qb-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {
            target = target,
            src = src
        })
        DropPlayer(src, "")
...
RegisterServerEvent('police:gsrGranted') -- that is qb's code ((sway))
...
RegisterServerEvent('police:hasShotRecently') -- that is qb's code ((sway))
...
AddEventHandler('police:dnaAsk', function(t)
	local src = source
	local user2 = exports["qb-base"]:getModule("Player"):GetUser(t)
...
RegisterServerEvent('police:remmaskGranted') -- that is qb's code ((sway))
AddEventHandler('police:remmaskGranted', function(targetplayer)
    TriggerClientEvent('police:remmaskAccepted', targetplayer)
end)

RegisterServerEvent('unseatAccepted') -- that is qb's code ((sway))
...
AddEventHandler("police:targetCheckInventory", function(target, status)
    local src = source
    local user2 = exports["qb-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('Gimmeurfuknmoney', function(target)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local targetply = exports["qb-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:SeizeCash', function(target)
    ...
    local userz = exports["qb-base"]:getModule("Player"):GetUser(target)
...
RegisterCommand('bill', function(source, args)
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(tonumber(args[1]))
    local character = user:getCurrentCharacter()
    local user2 = exports["qb-base"]:getModule("Player"):GetUser(src)
    local character2 = user2:getCurrentCharacter()
    exports.ghmattimysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
        if result[1].job == 'police' then
            ...
            TriggerServerEvent('qb-financials:business_money', args[2], 'police', 'add')
        end
    end)
end)
...
RegisterCommand('callsign', function(source, args)
    ...
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    print("^5[POLICE] Setting callsign")
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["qb-log"]:AddLog("HC Added Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["qb-log"]:AddLog("HC Removed Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
...
-- 	local commands = exports["qb-base"]:getModule("Commands")
...
function jailUser(player, reason, cid, time, src)
    local jailer = exports["qb-base"]:getModule("Player"):GetUser(src)
    if not jailer then
        return false
    end
    ...
    if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
        ...
        exports["qb-log"]:AddLog("Exploiter", jailer,
    ...
    TriggerEvent("qb-police:playerJailed", character)
...
AddEventHandler("911", function(args, pStreetName)
    ...
    local connect = {
        {
            ["color"] = color,
            ["title"] = "** FM | Nopixel [Police] | 911 Call **",
            ["description"] = "Steam Name:"..pName.. " \n Steam ID: "..identifiers[1].. " \n 911 | " .. char.first_name .. " " .. char.last_name .. " \n Message: " .. tostring(message),
        }
    }
    PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE", function(err, text, headers) end, 'POST', json.encode({username = "qb | 911 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
    ...
    local users = exports["qb-base"]:getModule("Player"):GetUsers()

    local emergencyPlayers = {}

    for k, v in pairs(users) do
        local user = exports["qb-base"]:getModule("Player"):GetUser(v)
        local job = user:getVar("job")
...
AddEventHandler("311", function(args)
    ...
    TriggerClientEvent("chatMessage", src,
        ...
        local connect = {
            {
                ["color"] = color,
                ["title"] = "** FM | Nopixel [Police] | 311 Call **",
                ["description"] = "Steam Name:"..pName.. " \n Steam ID: "..identifiers[1].. " \n 911 | " .. char.first_name .. " " .. char.last_name .. " \n Message: " .. tostring(message),
            }
        }
        PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE", function(err, text, headers) end, 'POST', json.encode({username = "qb | 311 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler("police:targetCheckWeapons", function(playerId,items)
    local src = source
    local char = exports["qb-base"]:getModule("Player"):GetUser(playerId)
...
-- 	end
-- 	local caller = tostring(char.first_name) .. " " .. tostring(char.last_name)
-- 	exports["qb-log"]:AddLog("911 Chat", user, caller .. ": " .. message, {})
...
RegisterCommand('pstate', function(source, args)
    local src = source
    local users = exports["qb-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qb-police:hire_pd')
AddEventHandler('qb-police:hire_pd', function(pCID, pRank, pCallsign, pDept)
...
RegisterServerEvent('qb-police:sv:open_hire_menu')
AddEventHandler('qb-police:sv:open_hire_menu', function()
    local src = source
    local user = exports["qb-base"]:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
...
local JSON = json.decode(LoadResourceFile("qb-hud", "/config/pursuitModes.json"))
