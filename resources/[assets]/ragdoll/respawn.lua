...
AddEventHandler("qb-menu:var:inServerFarm", function(pBool)
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["qb-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
function releaseBody(dropItems)
    ...
    if dropItems then
        ...
        --TriggerServerEvent("qb-ragdoll:removeallitems")
    end
    ...
    local doctors = RPC.execute('qb-jobmanager:jobCount', 'doctor')
    ...
    TriggerServerEvent('qb-healthcare:playerRespawn', hosp)
...
AddEventHandler('ragdoll:sendPing', function()
    if not SentPing then
        SentPing = true
        exports["qb-dispatch"]:policedead('10-47', "Injured Civillian", "429")
        Citizen.Wait(10000)
        SentPing = false
    else
