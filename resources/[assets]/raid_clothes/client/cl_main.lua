...
function Save(save, close, newFadeStyle)
    ...
    TriggerEvent('qb-weapons:getAmmo')
...
RegisterInterfaceCallback("qb-ui:raid_clothes:addOutfitPrompt", function(data, cb)
...
RegisterInterfaceCallback("qb-ui:raid_clothes:addOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback("qb-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key }, 3)
    exports['qb-interface']:closeApplication('contextmenu')
end)

RegisterInterfaceCallback("qb-ui:raid_clothes:deleteOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
...
AddEventHandler("qb-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports['qb-interface']:showInteraction(("[M] %s"):format(currentZone.text))
...
RegisterNetEvent("nopixel:raid_clothes_pedfix")
AddEventHandler("nopixel:raid_clothes_pedfix", function(name,price)
    SetSkin(GetEntityModel(PlayerPedId()), true)
    TriggerServerEvent("clothing:checkIfNew") 
    print("[CLOTHES] Setting PED clothes..")
end)
...
RegisterNetEvent("qb-context:closeglobal")
AddEventHandler("qb-context:closeglobal", function()
    ...
    local data = {
        {
            ...
            description = "",
            key = true,
            action = "qb-ui:raid_clothes:CustomaddOutfitPrompt"
        },
    }
...
RegisterInterfaceCallback("qb-ui:raid_clothes:CustomaddOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['qb-ui']:openApplication('textbox', {
        callbackUrl = 'qb-ui:raid_clothes:addCustomOutfit',
        key = data.key,
        items = {
            {
                icon = "pencil-alt",
...
RegisterInterfaceCallback("qb-ui:raid_clothes:addCustomOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent('nopixel:clothing:admin', function()
    Wait(2000)
    OpenMenu("clothing_shop")
end)
...
RegisterCommand("h1", function(source, args, rawCommand)
    if exports['qb-inventory']:hasEnoughOfItem('hat', 1) then
...
RegisterCommand("m1", function(source, args, rawCommand)
    if exports['qb-inventory']:hasEnoughOfItem('mask', 1) then
...
AddEventHandler("qb-inventory:itemUsed", function(pItem, pInfo)
  storedpItem = pItem
  storedpInfo = pInfo
  local model = chainModels[pItem]
  print('[qb-inventory:itemUsed]',model)
